...
3. **楼层事务**。用户对某一层触发的写入（记忆、世界状态、定时器）必须能撤销。工作区写入走 WAL（楼层号 + 序号），回退就是逆序回放。例外：记忆超容量的异步压缩在 idle 期 `runMaintenance` 执行（`floor=null` 不记 WAL），回退不撤压缩——无损整理，不增删剧情事实。
...
**查平台行为先看官方文档**：<https://deepseek-harness.github.io/deepseek-harness/>。涉及宿主机制（slot、profile、patch、typert、system-prompt 瀑布等）的判断以官方文档和宿主源码为准，不要凭记忆猜。

**查宿主源码看 npm 包**：`npm install` 后直接读 `node_modules/@deepseek-ai/*/` 的 `lib/` 与 `.d.ts`（devDependencies 锁精确版本；本体与 peer 包版本可能不同，先看各包 `package.json`）。本地没装时看 npm registry 的 Code 页（同站换包名）。

运行期角色卡、记忆、会话绑定在 `$DSH_HOME/dsh-tavern/`，不在本仓库。不要把真实卡拷进 git。测试用手写工厂数据。
...
- 设置用 schemastery（`src/node/config.ts`），命名空间 `dsh-tavern`，用户覆盖在 `~/.dsh/settings.yaml`，`applies: 'live'`。键：`locale`（界面语言，默认 en，设置页「界面」子组切换，只影响本插件 UI）、采样（含 thinking 档位）、世界书全局、记忆、`defaults`（新会话默认绑定）、`interactiveCards`、`cardNetworkWhitelist`、`cascadeDeleteEmbeddedBook`（关掉则删卡前把内嵌书抢救到世界书库）。rc.2 起宿主通用设置页也能看到/改这些键（命名空间白名单已移除，宿主行为），插件面板仍是主入口，不要为此改面板。
...
| 通道 | dsh 落点 | 内容 | 稳定性 |
| --- | --- | --- | --- |
| standing | system 段 `tavern:standing`（order 210，工具说明 100–199 之后） | `BOUND_DISCIPLINE` + 角色定义 + 预设骨架 + 常驻世界书 + 静态深度注入（无本轮宏的 in-chat 条目 / depth_prompt） | 按会话 × 生成场景钉死字节（`STANDING_PIN_VERSION` + generationType + 卡/预设/人设指纹 + 资产修订号）。纪律或段布局变更必须递增版本，否则旧钉死挡住新文案。预设/世界书经 `TavernState` 写方法 bump 修订号（`standingRevTags`）；绕开它手改文件不会被捕获。 |
| turn | runtime context `tavern:turn` | 固定 `TURN_PLAYBOOK`（不随 step 变）+ 关键词世界书/记忆/变化层/AN/本轮宏 | dsh 追加成 user 快照（`Current runtime context.`）覆盖同名旧快照；宿主按字节去重——同轮后续步骤快照不变则不再追加（多步零开销）。步骤收口压力走 `【Tavern 步骤】` inject（见下）。 |
...
turn 层预算（`core/turnBudget.ts`）：runtime context 快照对新请求永远是**未缓存前缀**（前缀缓存只认追加点之前的字节），快照里的大体量内容 = 每轮全价重付。因此世界书层用固定 `tokenBudget`（默认 8192，绝对上限，不随历史/窗口缩水；百分比路径折算基数 clamp 到 128K），且只计搭快照通道的条目——落 standing 的常驻（constant 且无本轮宏，`isStandingSafeEntry`）豁免计费与概率掷骰（恒定注入：掷骰已冻结成每会话一次，fork 分支重掷只会打穿 system 前缀缓存）。被裁条目进 `truncated`、快照尾部附 uid 清单（封顶 8 条），模型按条 `tavern_lore_read` 补读——硬顶是分页不是丢信息。世界书与记忆段落落消息带来源标签（standing `【世界书·常驻】`、turn `【世界书·本轮触发】`、记忆 `【检索记忆】`；AN 走 `wiText` 不加），否则裸文本拼接模型难以识别为设定事实。变化层按 `WORLD_DELTA_TURN_BUDGET`（1500）从最新往旧装载，只计本轮实际渲染的条目（判定共用 `isDeltaRenderedInTurn`），更旧的丢给 `tavern_lore_read(source=delta)`；记忆（1200）与 journal（800）各自有顶。被裁不要心疼——工具按条补读是设计内路径。触发日志 `[turn:tail]` 观测尾巴体积、`[standing:pin]` 观测钉位命中/重算。

多步循环，不要为了省 I/O 跳过每步组装：

- 世界书和记忆检索每 turn 只评估一次（定时器以消息数为单位，同轮复用 `wiCache`；`lastCharMessage` 与 `journalText` 随 `wiCache` 同轮冻结——history 增长或 journal 中途编辑不得改变同轮快照字节，否则按字节去重失效）。
...
- 步骤收口通知（`formatTurnStepNotice`，按 `turn:nextStep` 去重）只能在工具执行时注入（`node/tools.ts` 的 `maybeInjectStepNotice`，7 个工具全覆盖）。不要在 `agent/pre-step` 里 inject：要等下一步 preStep 才被认领（晚一步），且 turn 结束判定会把未消费的 nextStep 当成续步理由，强拉一步产生孤儿通知。
- 合成 user 文本（runtime context 快照、同轮写入确认、步骤收口通知）走 `isSyntheticUserText`：不当 `{{lastusermessage}}`，不扫世界书，不计入正则 depth。

采样 / thinking：`agent/request` 透传 `temperature` / `maxTokens` / `stop`，并按模型公布的 reasoning 档写 `reasoningEffort`（disabled → `off`；low/high/max → 公布才显式指定，否则回退自动；enabled → 保留会话已选非 off 档，否则模型默认——默认档思考可能很短，要长思考引导用户选 high/max）。只发送适配器公布的 id。部署把 `llm-deepseek.thinking` 锁成 `disabled` 时插件无法强行打开。
...
| `tavern_memory_write` / `update` | 只记事实与关系/状态变化。去重；超容量标记 `pendingMemoryCompress`，turn 结束后 `runMaintenance` 异步压缩最旧批次（不记 WAL）。写成功 inject 同轮确认。 |
...
npm test           # vitest run：36 个文件、约 510 例（例数随改动浮动，不是精确值）
...
CI（`.github/workflows/ci.yml`）：ubuntu + Node 24 跑 `npm ci` → build → test，然后 `git diff --exit-code lib/` 拦未重建的入库产物，加 `git status --porcelain -- lib/` 拦未跟踪新产物（diff 看不见 untracked），最后解析 `npm pack --dry-run --json` 的 files 列表断言白名单（必含 `lib/index.js` 等，不得含 `src/`/`test/`/`node_modules/`；裸 `--dry-run` 只打印不失败）。行尾靠 `.gitattributes`（`* text=auto`）保证 Linux 可复现。
...
│   ├── util.tsx / styles.ts / i18n.ts / locales.ts / types.ts
│       i18n.ts = 界面语言运行时（useT/t，默认 en）；locales.ts 聚合
│       client/locales/<module>.ts 字典（zh 键全集源，en 同键齐全）
│       types.ts = TavernRemote 契约镜像（改 remote.ts / service.ts 必须同步）
│   └── panel/  设置子面板：characters / presets / lorebooks / lorebookEditor /
│               personas / regex / memory / settings（内分六个子组，二级 pill 导航）
├── index.ts    host 入口
├── agent.ts    agent 面入口
└── remote.ts   typert 契约
```

对应测试在 `test/`（36 个文件，约 510 例）：core/state 纯逻辑，加上 binding、cardFrame、workspace、floorsForkOptions、greetingSeed、stateRuntime、openChild、siblings、presetInstall、i18n 等。host 事件与 client UI 不测。
...
- UI 对齐 dsh 原生：按钮、菜单、弹窗、通知走 `@deepseek-ai/dsh-client-ui-primitives`（封装见 `src/client/util.tsx`）。禁止原生 `<select>`（Windows 上 option 白底白字）。样式只进 `src/client/styles.ts`。颜色只用真实存在的 `--dsw-*` 令牌，不要引用悬空变量；Tavern 品牌层只用 `styles.ts` 顶部定义的 `--tavern-accent-*` 青碧变量，不要在组件里发明新色值。动效用 `--ds-ease-in-out` 加 0.1/0.2/0.3s，遵守 `prefers-reduced-motion`。页签导航用分段控件 `Tabs`（pill track；`size="sm"` 为页内第二级），不是宿主下划线页签。设置行 = 标题 + 说明 + 右侧 36px 胶囊（`SettingsRow`，宽控件用 `stacked`）；一组表单的保存按钮放 `SaveBar`。确认用 `ConfirmDialog`，不用 `window.confirm`；瞬时反馈用 `useToast`；加载用 `Skeleton`；头像用 `Avatar`；可点卡片用 `clickableProps`。
- 展示名永远用 `card.name`。不要把工作区文件夹 `cardId`（净化名 + 8 位 hash）当成角色名。头像走 `getAvatar({ cardId })` 的 PNG dataURL。
- 界面文案一律经 `i18n.ts` 的 `useT()` 取字典（组件内 `const t = useT()`，插值用 `{name}` 占位符），不要在组件里写死任何一种语言。字典按模块分片在 `client/locales/<module>.ts`（键带模块前缀防碰撞；zh 是键全集源，en 同键齐全，`test/i18n.test.ts` 校验 parity、片段碰撞与源码键引用）。slot label 也走这套字典。注释仍用中文。
...
1. 采样只透传 `temperature` / `maxTokens` / `stop`，以及模型公布的 `reasoningEffort`（低/高/最高档仅公布时显式指定）。`top_p` 和 penalty 到不了模型，设置面板仅作记录。
...
3. 会话日志不可删。重新生成/回退/编辑 = fork 前缀 + WAL 回滚 + 子会话续跑，成功后 UI 打开分支会话，并用宿主 `ISessions` 的 `scope → sessionOf → rename` 把分支标题写进会话列表（旧宿主缺这条路径则跳过）。编辑 assistant 正文只换 seed 里的该条消息、不续跑。例外：续写（`continueFloor`）不改历史，不 fork，直接 followup 合成指令。同一父会话 + 同一楼层 fork 出的分支互为兄弟：forkAt 记 `siblings.json`，操作条给 ‹ n/m › 兄弟导航（`getFloorSiblings`，读时按 live/绑定文件过滤已删分支）。rc.2 起会话头另有宿主原生面包屑（按 `meta.parentSession` 算世系，`conversation.session.header.lineage` 由 subagent 插件渲染）：会话级世系，与楼层级 ‹ n/m › 互补，不要替换那个 slot。
...
- **加设置项**：`src/node/config.ts`（schemastery，`applies: 'live'`）+ `src/client/panel/settings.tsx`（六个子组之一：界面 / 默认配置 / 采样与思考 / 世界书引擎 / 记忆 / 卡片与数据；行与保存按钮的排法见「代码风格」）+ slot label 进 `locales.ts`。默认值有跨层引用时放 `src/core/types.ts`。
- **加/改 UI**：样式只进 `src/client/styles.ts`；交互组件走 `dsh-client-ui-primitives`（封装在 `util.tsx`）；禁止原生 `<select>` 与 `window.confirm`；文案进 `client/locales/` 字典，不写死语言。
...
3. 会话头宿主原生面包屑（`meta.parentSession` 世系，`conversation.session.header.lineage`）与楼层级 ‹ n/m › 互补，不要替换那个 slot（详见「平台限制」第 3 条）。
...
7. **非会话写入不记 WAL**。导入/设置改文件时 `WorkspaceFs` 的 floor 为 `null`，跳过快照。但每卡共享句柄 `workspace(cardId).fs` 的 floor 在 turn/start～turn/end 之间**非 null**：面板/服务层写方法（state.ts 的 saveJournal/saveCharacter/saveCharacterLorebook/deleteCharacterLorebook/saveChatLorebook 与导入建索引）一律走 `plainFs`（floor 恒 null），否则生成进行中的用户编辑会被误记进当前楼层 WAL、回退时静默改回旧值；turn 流程内的工具写路径才走共享句柄。不要复活名为 `non-floor` 的 WAL 单元。
...
库资产解析结果有进程内 rev-keyed 缓存：经 `TavernState` 写方法编辑会 bump 修订号并失效缓存；绕开它手改文件不被捕获（与 standing 钉死同一语义，重启即清）。`loadBinding` 走快路径——cardId 仍在时不做全库扫描，卡失效才全量接回。
