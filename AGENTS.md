...
5. **模型请求遵守宿主边界。** standing 是工具说明后的 `tavern:standing`（`getSectionOrder('TOOLS_SDK') + 10`，rc.1 为 5010；旧 210 早于 PTC SDK），turn 是 runtime context `tavern:turn`。绝不使用 complete 段盖掉工具前缀。ST 全量序列是模拟，不能把它称为实际入模消息。input/send、prompt/assemble、prompt/send 的历史改写只用于模拟与代答；live 展示规则是 output/render。
