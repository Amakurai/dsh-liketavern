...
- 也可以走 tarball：作者侧 `npm pack`（`prepack` 会先构建），用户侧 `dsh plugin --profile web add ./dsh-liketavern-0.2.1.tgz`。
...
- **AI 代答**：当前代答操作将结果复制到剪贴板，需要手动粘贴；尚未接入宿主草稿填入能力。
