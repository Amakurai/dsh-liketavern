...
import { BackupError, createBackup, restoreBackup, verifyBackup } from './node/backup.js';
export function parseBackupArguments(args) {
    ...
    if (!values.has('--backup') || (command === 'create' && (!values.has('--home') || values.has('--target')))
        ...
        || (command === 'verify' && (values.has('--home') || values.has('--target') || flags.has('--offline'))))
        throw new BackupError('INVALID_ARGUMENTS');
...
export async function backupMain(args = process.argv.slice(2), io = {
    ...
    let parsed;
    try {
        parsed = parseBackupArguments(args);
    }
    catch {
...
function direct() {
    try {
        return !!process.argv[1] && realpathSync(fileURLToPath(import.meta.url)) === realpathSync(resolve(process.argv[1]));
    }
    catch {
        return false;
    }
...
if (direct())
    void backupMain().then(code => { process.exitCode = code; }, () => { process.stderr.write('dsh-liketavern backup: IO_FAILED\n'); process.exitCode = 2; });
