...
var editorDraftScope = {
  owner: string2().check(_regex(/^[A-Za-z0-9_-]{8,80}$/)),
  key: string2().check(_minLength(1), _maxLength(300))
};
var METHODS = {
  ...
  clearSessionBinding: { req: object({ ...sessionIdField, onlyIfBlank: optional(boolean2()) }), value: anyValue, summary: "\u6E05\u9664\u4F1A\u8BDD\u89D2\u8272\u5361\u7ED1\u5B9A\uFF08\u53EF\u9650\u4E8E\u5C1A\u672A\u5F00\u59CB\u7684\u4F1A\u8BDD\uFF09" },
  // 开场白
  ...
  renderOutputText: {
    req: object({ ...sessionIdField, text: string2(), messageId: optional(int().check(_gte(0))) }),
    value: anyValue,
...
var zh9 = {
  "memory.deleteConfirm": "\u5220\u9664\u540E\uFF0C\u8FD9\u6761\u8BB0\u5FC6\u5C06\u4E0D\u518D\u7528\u4E8E\u5F53\u524D\u72B6\u6001\u7684\u68C0\u7D22\u3002\u786E\u5B9A\u5220\u9664\uFF1F",
  "memory.storyLabel": "\u5267\u60C5 {index} \xB7 {date}",
  "memory.scopeStory": "\u5267\u60C5\u72B6\u6001",
  "memory.scopeInitial": "\u65B0\u4F1A\u8BDD\u521D\u59CB\u72B6\u6001",
  "memory.scopeStoryDesc": "\u4FEE\u6539\u4EC5\u5F71\u54CD\u6240\u9009\u5267\u60C5\uFF0C\u4E0D\u4F1A\u6539\u53D8\u5176\u4ED6\u4F1A\u8BDD\u6216\u5206\u652F\u3002",
  "memory.scopeInitialDesc": "\u4FEE\u6539\u7528\u4E8E\u4EE5\u540E\u65B0\u5EFA\u7684\u4F1A\u8BDD\uFF0C\u4E0D\u4F1A\u6539\u53D8\u5DF2\u7ECF\u5F00\u59CB\u7684\u5267\u60C5\u3002",
  "memory.story": "\u5267\u60C5\u72B6\u6001",
...
var en9 = {
  "memory.deleteConfirm": "This memory will no longer be retrieved in the selected state. Delete it?",
  "memory.storyLabel": "Story {index} \xB7 {date}",
  "memory.scopeStory": "Story state",
  "memory.scopeInitial": "New chat defaults",
  "memory.scopeStoryDesc": "Changes affect only the selected story, not other chats or branches.",
  "memory.scopeInitialDesc": "Changes apply to future chats, not stories already in progress.",
...
var zh15 = {
  ...
  "speech.cardDataTitle": "\u5361\u5185\u4E34\u65F6\u6570\u636E \xB7 \u5907\u4EFD\u4E0E\u6062\u590D",
...
var en15 = {
  ...
  "speech.cardDataTitle": "Temporary card data \xB7 backup and restore",
...
// lib/client/util.js
function Btn(props) {
  return (0, import_jsx_runtime.jsx)(import_dsh_client_ui_primitives.Button, { type: "button", className: "dsh-tavern-btn", variant: props.primary ? "primary" : "outline", size: props.size ?? "sm", disabled: props.disabled, title: props.title, onClick: (e) => {
    e.stopPropagation();
    props.onClick();
...
function FileBtn(props) {
  return (0, import_jsx_runtime.jsx)(import_dsh_client_ui_primitives.Button, { type: "button", variant: "outline", size: "md", disabled: props.disabled, className: "dsh-tavern-file dsh-tavern-btn", onClick: () => {
...
function Tabs(props) {
  ...
  return (0, import_jsx_runtime.jsx)("div", { className: `dsh-tavern-navPills${props.size === "sm" ? " is-sub" : ""}`, role: "tablist", "aria-label": props.label, onKeyDown: (e) => {
    const direction = e.key === "ArrowRight" ? 1 : e.key === "ArrowLeft" ? -1 : 0;
    if (!direction && e.key !== "Home" && e.key !== "End")
      return;
    const buttons = Array.from(e.currentTarget.querySelectorAll('[role="tab"]'));
    const current2 = buttons.indexOf(e.target);
    if (current2 < 0)
      return;
    e.preventDefault();
    const next = e.key === "Home" ? 0 : e.key === "End" ? buttons.length - 1 : (current2 + direction + buttons.length) % buttons.length;
    buttons[next]?.focus();
  }, children: props.items.map((item) => (0, import_jsx_runtime.jsx)("button", { type: "button", role: "tab", id: `${id}-${item.id}`, tabIndex: props.value === item.id ? 0 : -1, className: "dsh-tavern-navPill", "data-active": props.value === item.id ? "true" : "false", "aria-selected": props.value === item.id, "aria-controls": props.panelId, onClick: () => props.onChange(item.id), children: item.label }, item.id)) });
}
function SaveBar(props) {
  return (0, import_jsx_runtime.jsx)("div", { className: `dsh-tavern-saveBar${props.inline ? " is-inline" : ""}`, children: props.children });
}
...
function Err(props) {
  if (!props.message)
    return null;
  return (0, import_jsx_runtime.jsx)("div", { className: "dsh-tavern-errText", role: "alert", children: props.message });
}
...
// lib/core/cardVariables.js
function restoreCardVariableBackup(srcDoc, text) {
  ...
  for (const [key, value] of Object.entries(parsed.scopes)) {
    ...
    const [type, id] = parts;
    if (!["chat", "global", "character", "message", "script", "extension"].includes(type) || typeof id !== "string" && typeof id !== "number" || ["chat", "global", "character"].includes(type) && id !== "" || JSON.stringify(parts) !== key)
      throw new Error("Invalid scope");
  }
...
function installCardVariables(labels, styles = "") {
  const root = window;
  ...
  function scope(option) {
    const opts = option === void 0 ? { type: "chat" } : option;
    if (!record(opts))
    ...
    const id = type === "message" ? opts.message_id ?? "current" : type === "script" ? opts.script_id ?? "current" : type === "extension" ? opts.extension_id : "";
  ...
  function getVariables(option) {
    const value = clone2(holder.scopes[scope(option)] ?? {});
    requestBackup();
    return value;
  }
  function replaceVariables(value, option) {
    const next = clone2(value);
    const scopes = clone2({ ...holder.scopes, [scope(option)]: next });
    clone2({ version: 1, scopes });
  ...
  function insertOrAssignVariables(value, option) {
    return replaceVariables(merge(getVariables(option), clone2(value), false), option);
  }
  function insertVariables(value, option) {
    return replaceVariables(merge(getVariables(option), clone2(value), true), option);
  }
  ...
  function mountBackup() {
    ...
    details2.id = "dsh-tavern-variable-backup";
    if (styles) {
    ...
    details2.append(summary, note, backup, textarea);
    document.body.prepend(details2);
  }
  if (document.readyState === "loading")
...
(function () {
  ...
  (${installCardVariables.toString()})(${escapeScriptJson(options.variableLabels ?? {
    title: "Temporary card data / backup",
    note: "Card variables stay in this frame only. Copy a backup before leaving or refreshing. They are not shared with the host or other conversations.",
    backup: "Select backup text",
    text: "Card variable backup"
...
function wrapCardDocument(html, trustedHead) {
...
function buildCardSrcDoc(html, options) {
  ...
  return wrapCardDocument(html, meta2 + stub);
}
...
// lib/client/drafts.js
var import_jsx_runtime4 = require("react/jsx-runtime");
var import_react5 = require("react");

// lib/client/draftPersistence.js
var import_jsx_runtime3 = require("react/jsx-runtime");
var import_react4 = require("react");
var PersistenceContext = (0, import_react4.createContext)(null);
...
function PersistentEditor(props) {
  const parent = (0, import_react4.useContext)(PersistenceContext);
  ...
  return (0, import_jsx_runtime3.jsx)(DraftLoader, { remote: props.remote, scope: props.scope, children: props.children }, props.scope);
}
function DraftLoader(props) {
  const t2 = useT();
  const [owner] = (0, import_react4.useState)(ownerId);
  const [attempt, retry] = (0, import_react4.useState)(0);
  ...
  if (error)
    return (0, import_jsx_runtime3.jsxs)("div", { className: "dsh-tavern-ui", children: [(0, import_jsx_runtime3.jsx)(Err, { message: `${t2("draft.loadFailed")} ${error}` }), (0, import_jsx_runtime3.jsx)(Btn, { onClick: () => retry((n) => n + 1), children: t2("action.retry") })] });
  if (!loaded)
    return (0, import_jsx_runtime3.jsx)(Skeleton, {});
  return (0, import_jsx_runtime3.jsx)(DraftSession, { remote: props.remote, scope: props.scope, owner, snapshot: loaded.snapshot, children: props.children });
}
function DraftSession(props) {
  const t2 = useT();
  const [writeState, setWriteState] = (0, import_react4.useState)(props.snapshot ? "restored" : "idle");
  const [error, setError] = (0, import_react4.useState)(null);
  const [session] = (0, import_react4.useState)(() => {
    ...
    let mounted = true;
    let seenDirty = false;
    let last = props.snapshot ?? void 0;
    let revision = 0;
    ...
    const flush = () => {
      ...
      void enqueue(queueKey, async () => {
        const result = snapshot ? await props.remote.saveEditorDraft({ owner: props.owner, key: props.scope, value: snapshot }) : await props.remote.deleteEditorDraft({ owner: props.owner, key: props.scope });
        if (!result.ok)
          throw new Error(result.error.message);
        persisted = serialized;
        if (mounted && current2 === revision)
  ...
  return (0, import_jsx_runtime3.jsxs)(PersistenceContext.Provider, { value: session.registry, children: [writeState !== "idle" && (0, import_jsx_runtime3.jsxs)("div", { className: "dsh-tavern-ui dsh-tavern-draftStatus", role: "status", children: [(0, import_jsx_runtime3.jsx)("span", { children: t2(writeState === "restored" ? "draft.restored" : writeState === "saving" ? "draft.autosaving" : writeState === "error" ? "draft.autosaveFailed" : "draft.autosaved") }), error && (0, import_jsx_runtime3.jsxs)(import_jsx_runtime3.Fragment, { children: [(0, import_jsx_runtime3.jsx)(Err, { message: error }), (0, import_jsx_runtime3.jsx)(Btn, { onClick: session.flush, children: t2("action.retry") })] })] }), props.children] });
}
function useDraftState(key, initial) {
  const registry = (0, import_react4.useContext)(PersistenceContext);
  const initialRef = (0, import_react4.useRef)(initial);
  initialRef.current = initial;
  const resolve = () => Object.hasOwn(registry?.restored ?? {}, key) ? registry.restored[key] : typeof initialRef.current === "function" ? initialRef.current() : initialRef.current;
  const [state, setState] = (0, import_react4.useState)(() => ({ key, value: resolve() }));
  let value = state.value;
  if (state.key !== key) {
    value = resolve();
    setState({ key, value });
  }
  const token = (0, import_react4.useRef)({});
  (0, import_react4.useLayoutEffect)(() => {
    registry?.field(key, token.current, value);
    return () => registry?.remove(key, token.current);
  }, [registry, key, value]);
  const setValue = (0, import_react4.useCallback)((next) => {
    setState((current2) => ({ key, value: typeof next === "function" ? next(current2.value) : next }));
  }, [key]);
  return [value, setValue];
}
function useDraftRestored(key) {
  const registry = (0, import_react4.useContext)(PersistenceContext);
  const [restored] = (0, import_react4.useState)(() => Object.hasOwn(registry?.restored ?? {}, key));
  return restored;
}
function usePersistentDraftStatus(id, dirty, busy) {
  const registry = (0, import_react4.useContext)(PersistenceContext);
  (0, import_react4.useEffect)(() => {
...
// lib/client/drafts.js
var DraftContext = (0, import_react5.createContext)(null);
function useDraftGuard(dirty, busy = false, clearChildren) {
  const report = (0, import_react5.useContext)(DraftContext);
  const id = (0, import_react5.useId)();
  const discard = usePersistentDraftStatus(id, dirty, busy);
  const clearDraft = (0, import_react5.useCallback)(() => {
    discard?.();
    clearChildren?.();
  }, [discard, clearChildren]);
  const t2 = useT();
  ...
  const confirmation = (0, import_jsx_runtime4.jsx)(ConfirmDialog, { open: pending !== null, title: t2("draft.leaveTitle"), description: t2("draft.leaveDesc"), confirmLabel: t2("draft.discard"), danger: true, busy, onCancel: () => setPending(null), onConfirm: () => {
    if (busy)
      return;
    setPending(null);
    clearDraft();
...
function DraftScope(props) {
  const [statuses, setStatuses] = (0, import_react5.useState)({});
  const report = (0, import_react5.useCallback)((id, status) => {
    setStatuses((current2) => {
      const next = { ...current2 };
      if (status)
        next[id] = status;
      else
  ...
  const currentStatuses = (0, import_react5.useRef)(statuses);
  currentStatuses.current = statuses;
  const clearChildren = (0, import_react5.useCallback)(() => {
    for (const status of Object.values(currentStatuses.current))
      if (status.dirty)
        status.clearDraft();
  }, []);
  const guard = useDraftGuard(dirty, busy, clearChildren);
  return (0, import_jsx_runtime4.jsxs)(DraftContext.Provider, { value: report, children: [props.children(guard.request, dirty, busy), guard.confirmation] });
}

// lib/client/speech.js
function SpeechHtmlFrame(props) {
  const iframeRef = (0, import_react6.useRef)(null);
  const [frameH, setFrameH] = (0, import_react6.useState)(null);
  const t2 = useT();
  const [restoreOpen, setRestoreOpen] = (0, import_react6.useState)(false);
  const [backup, setBackup] = (0, import_react6.useState)("");
  const [restoreError, setRestoreError] = (0, import_react6.useState)(null);
  const [restored, setRestored] = (0, import_react6.useState)(null);
  ...
  const closeRestore = () => guard.request(() => {
  ...
  const restore = () => {
    try {
      ...
      setRestored((current2) => ({ source: props.srcDoc, doc, revision: (current2?.revision ?? 0) + 1 }));
      setRestoreOpen(false);
      setBackup("");
      setRestoreError(null);
    } catch {
      setRestoreError(t2("speech.cardDataFailed"));
    }
  };
  ...
  const frameStyle = frameH != null ? { height: frameH, minHeight: 0, overflow: "hidden" } : props.compact ? { height: 80, minHeight: 0, overflow: "auto" } : props.widget ? { height: 280, minHeight: 0, overflow: "auto" } : { overflow: "auto" };
  return (0, import_jsx_runtime5.jsxs)(import_jsx_runtime5.Fragment, { children: [(0, import_jsx_runtime5.jsx)("iframe", { ref: iframeRef, className: `dsh-tavern-speechHtml${props.widget || props.compact ? " is-widget" : ""}`, sandbox: "allow-scripts", srcDoc, title: props.title, style: frameStyle }, activeRestore?.revision ?? 0), (0, import_jsx_runtime5.jsx)("div", { className: "dsh-tavern-cardBackupBar", children: (0, import_jsx_runtime5.jsx)(Btn, { onClick: () => {
    setRestoreError(null);
    setRestoreOpen(true);
  }, children: t2("speech.cardDataRestore") }) }), guard.confirmation, restoreOpen && (0, import_jsx_runtime5.jsxs)(Dialog, { open: true, title: t2("speech.cardDataRestore"), onClose: closeRestore, footer: (0, import_jsx_runtime5.jsxs)(import_jsx_runtime5.Fragment, { children: [(0, import_jsx_runtime5.jsx)(Btn, { onClick: closeRestore, children: t2("action.cancel") }), (0, import_jsx_runtime5.jsx)(Btn, { primary: true, disabled: !backup.trim(), onClick: restore, children: t2("action.confirm") })] }), children: [(0, import_jsx_runtime5.jsx)("p", { children: t2("speech.cardDataRestoreDesc") }), (0, import_jsx_runtime5.jsx)("textarea", { className: "dsh-tavern-input dsh-tavern-textarea", "aria-label": t2("speech.cardDataText"), value: backup, onChange: (event) => setBackup(event.target.value) }), (0, import_jsx_runtime5.jsx)(Err, { message: restoreError })] })] });
}
function SpeechBubble(props) {
  return (0, import_jsx_runtime5.jsx)(SpeechBubbleSession, { ...props }, `${props.sessionId}:${props.cardId}`);
}
function SpeechBubbleSession(props) {
  ...
  const rendered = useLoader(() => remote.renderOutputText({ sessionId, text: rawText, messageId: props.messageId }), [sessionId, rawText, props.messageId], Boolean(rawText) && !streaming);
  ...
  const text = !streaming && rendered.state.status === "ready" ? interactive || rendered.state.value.text || rendered.state.value.parts !== void 0 ? rendered.state.value.text : stripDisplayMeta(rawText) : stripDisplayMeta(rawText);
  ...
  const toast = useToast();
  const swipeBusy = (0, import_react6.useRef)(false);
  const [swipeError, setSwipeError] = (0, import_react6.useState)(null);
  const swipeGreeting = async (index) => {
    if (swipeBusy.current)
      return;
    if (!canSwipe) {
      setSwipeError(t2("speech.swipeStarted"));
      return;
    }
    if (!onSwipeGreeting) {
      setSwipeError(t2("speech.navigationUnavailable"));
      return;
    }
  ...
  const storedParts = !streaming && rendered.state.status === "ready" ? rendered.state.value.parts : void 0;
  const visibleParts = storedParts ? storedParts.filter((part) => interactive || part.kind === "markdown") : [...htmls.map((html) => ({ kind: "html", text: html })), ...text ? [{ kind: "markdown", text }] : []];
  if (!visibleParts.length)
    visibleParts.push({ kind: "markdown", text: text || " " });
  const content = visibleParts.map((part, i) => {
    if (part.kind === "markdown")
      return (0, import_jsx_runtime5.jsx)(import_dsh_client_ui_primitives3.MarkdownText, { text: part.text, streaming: Boolean(streaming), labels: markdownLabels }, `text:${i}`);
    const srcDoc = buildCardSrcDoc(part.text, {
      greetings,
      greetingIndex,
      connectHosts: whitelist,
      variableStyles: CARD_VARIABLE_STYLES,
      variableLabels: {
        title: t2("speech.cardDataTitle"),
        note: t2("speech.cardDataNote"),
        backup: t2("speech.cardDataBackup"),
        text: t2("speech.cardDataText")
      }
    });
    const widget = visibleParts.length > 1;
    const frame = (0, import_jsx_runtime5.jsx)(SpeechHtmlFrame, { srcDoc, title: part.title || name2, widget, compact: Boolean(storedParts), onSwipeGreeting: (index) => void swipeGreeting(index) }, `${i}:${part.text.length}`);
    return part.title ? (0, import_jsx_runtime5.jsxs)("details", { className: "dsh-tavern-reason", children: [(0, import_jsx_runtime5.jsx)("summary", { children: part.title }), frame] }, `fold:${i}`) : frame;
  });
  return (0, import_jsx_runtime5.jsxs)("div", { className: "dsh-tavern-speech dsh-tavern-rise", children: [(0, import_jsx_runtime5.jsx)(Avatar, { url: avatarUrl, name: name2, size: 40, className: "dsh-tavern-speechAvatar" }), (0, import_jsx_runtime5.jsxs)("div", { className: "dsh-tavern-speechBody", children: [(0, import_jsx_runtime5.jsx)("div", { className: "dsh-tavern-speechName", children: name2 }), (0, import_jsx_runtime5.jsx)(Err, { message: swipeError }), (0, import_jsx_runtime5.jsx)(Err, { message: rendered.state.status === "error" ? rendered.state.message : null }), content] }), (0, import_jsx_runtime5.jsx)("div", { className: "dsh-tavern-speechCopy", children: (0, import_jsx_runtime5.jsx)(IconBtn, { label: t2("speech.copy"), onClick: () => void onCopy(), children: (0, import_jsx_runtime5.jsx)(import_dsh_client_ui_primitives3.IconCopyOutline16, {}) }) }), toast.node] });
}
...
function ReasoningFold(props) {
  ...
  if (!props.text.trim())
    return null;
  return (0, import_jsx_runtime6.jsxs)("details", { className: "dsh-tavern-reason", children: [(0, import_jsx_runtime6.jsx)("summary", { children: props.streaming ? t2("assistant.thinking") : t2("assistant.thought") }), (0, import_jsx_runtime6.jsx)("pre", { children: props.text })] });
}
function TavernAssistantNode(props) {
  ...
  if (!tavern) {
    return (0, import_jsx_runtime6.jsx)(NativeAssistantFallback, { ...props, streaming, interrupted });
  }
  const locationTurn = node.location && (node.location.kind === "turn" || node.location.kind === "step") ? node.location.turn?.turn : void 0;
  const interruptedActions = interrupted && binding && typeof locationTurn === "number" && sessions ? (0, import_jsx_runtime6.jsx)(TavernInterruptedFloorActions, { remote, sessionId, sessions, turn: locationTurn }) : null;
  if (binding && text && !hasImages) {
    return (0, import_jsx_runtime6.jsxs)("div", { children: [(0, import_jsx_runtime6.jsx)(ReasoningFold, { text: reasoningText, streaming }), (0, import_jsx_runtime6.jsx)(SpeechBubble, { remote, sessionId, cardId: binding.cardId, name: name2 || t2("assistant.characterFallback"), rawText: text, messageId: node.data.finalNode?.seq, streaming: streaming || node.location?.turn?.status === "open", interactiveCards: binding.interactiveCards, onSwipeGreeting: async (index) => {
      if (!sessions)
        throw new Error(t2("speech.navigationUnavailable"));
      const r = await remote.swipeGreeting({ sessionId, index });
      if (!r.ok)
        throw new Error(r.error.message);
      await openChildSession(sessions, r.value.childSessionId, r.value.title);
    } }), interrupted && (0, import_jsx_runtime6.jsx)("div", { className: "dsh-tavern-notice", children: t2("assistant.stopped") }), interruptedActions] });
  }
  return (0, import_jsx_runtime6.jsxs)("div", { children: [(0, import_jsx_runtime6.jsx)(NativeAssistantFallback, { ...props, streaming, interrupted, stripMeta: true }), interruptedActions] });
}
function NativeAssistantFallback(props) {
  ...
  const mentionOwner = (0, import_react7.useMemo)(() => {
  ...
  const mentions = (0, import_react7.useMemo)(() => mentionOwner && fileMentions ? fileMentions(mentionOwner) : void 0, [fileMentions, mentionOwner]);
  const rendered = [];
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (block.kind === "text") {
      ...
      rendered.push((0, import_jsx_runtime6.jsx)(import_dsh_client_ui_primitives4.MarkdownText, { text: shown, streaming, fileMentions: mentions, labels: markdownLabels }, i));
    } else if (block.kind === "reasoning") {
      if (stripMeta)
        continue;
      rendered.push((0, import_jsx_runtime6.jsxs)("details", { className: "dsh-tavern-reason", children: [(0, import_jsx_runtime6.jsx)("summary", { children: streaming ? t2("assistant.thinking") : t2("assistant.thought") }), (0, import_jsx_runtime6.jsx)("pre", { children: block.text })] }, i));
      ...
      if (renderMessageImages) {
        rendered.push((0, import_jsx_runtime6.jsx)(import_react7.Fragment, { children: renderMessageImages({ images: group.map(({ attachment }) => ({ attachment })), align: "start" }) }, i));
      }
    } else if (block.kind === "tool-call") {
      continue;
    } else {
      rendered.push((0, import_jsx_runtime6.jsx)(import_dsh_client_ui_primitives4.JsonBlock, { label: t2("assistant.unknownBlock"), truncatedLabel: (total) => t2("assistant.truncated", { total }), payload: block.block }, i));
    }
  }
  return (0, import_jsx_runtime6.jsxs)("div", { children: [stripMeta ? (0, import_jsx_runtime6.jsx)(ReasoningFold, { text: node.data.blocks.filter((b) => b.kind === "reasoning").map((b) => b.text ?? "").join("\n\n"), streaming }) : null, rendered, interrupted ? (0, import_jsx_runtime6.jsx)("span", { children: t2("assistant.stopped") }) : null] });
}
...
function MemoryEditor(props) {
  ...
  return (0, import_jsx_runtime7.jsxs)("div", { style: { display: "flex", flexDirection: "column", gap: 8, marginTop: 4 }, children: [guard.confirmation, (0, import_jsx_runtime7.jsx)("textarea", { className: "dsh-tavern-input dsh-tavern-textarea", disabled: busy, value: body, onChange: (e) => setBody(e.target.value) }), (0, import_jsx_runtime7.jsxs)("div", { className: "dsh-tavern-fieldRow", children: [(0, import_jsx_runtime7.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime7.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("memory.tags") }), (0, import_jsx_runtime7.jsx)("input", { className: "dsh-tavern-input", disabled: busy, value: tags, onChange: (e) => setTags(e.target.value) })] }), (0, import_jsx_runtime7.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime7.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("memory.keys") }), (0, import_jsx_runtime7.jsx)("input", { className: "dsh-tavern-input", disabled: busy, value: keys, onChange: (e) => setKeys(e.target.value) })] })] }), (0, import_jsx_runtime7.jsx)(Err, { message: error }), (0, import_jsx_runtime7.jsxs)("div", { style: { display: "flex", gap: 8, justifyContent: "flex-end" }, children: [(0, import_jsx_runtime7.jsx)(Btn, { disabled: busy, onClick: cancel, children: t2("action.cancel") }), (0, import_jsx_runtime7.jsx)(Btn, { primary: true, disabled: busy || !body.trim(), onClick: () => void save(), children: t2("action.save") })] })] });
}
function MemorySection(props) {
  ...
  return (0, import_jsx_runtime7.jsx)(PersistentEditor, { remote: props.remote, scope, children: (0, import_jsx_runtime7.jsx)(MemorySelection, { ...props }) });
}
function MemorySelection(props) {
  ...
  const storyId = storedStoryId || void 0;
  return (0, import_jsx_runtime7.jsx)(MemoryContextSection, { remote: props.remote, cardId, storyId, setCardId, setStoryId: (value) => setStoredStoryId(value ?? "") }, JSON.stringify([cardId, storyId ?? null]));
}
function MemoryContextSection(props) {
  ...
  const [error, setError] = (0, import_react8.useState)(null);
  const [busy, setBusy] = (0, import_react8.useState)(false);
  const [deleteId, setDeleteId] = (0, import_react8.useState)(null);
  ...
  const firstJournal = (0, import_react8.useRef)(true);
  ...
  return (0, import_jsx_runtime7.jsxs)(Section, { title: t2("section.memory"), description: t2("memory.desc"), children: [toast.node, guard.confirmation, (0, import_jsx_runtime7.jsx)(ConfirmDialog, { open: deleteId !== null, title: t2("memory.deleteEntry"), description: t2("memory.deleteConfirm"), confirmLabel: t2("action.delete"), danger: true, busy, onCancel: () => {
    if (!busy)
      setDeleteId(null);
  }, onConfirm: () => {
    if (deleteId)
      void deleteMemory(deleteId).then(() => setDeleteId(null));
  } }), (0, import_jsx_runtime7.jsx)(SettingsRow, { title: t2("memory.character"), description: t2("memory.characterDesc"), children: (0, import_jsx_runtime7.jsx)(Select, { size: "md", value: cardId, disabled: busy || editingId !== null, onChange: (value) => guard.request(() => {
    setStoryId(void 0);
    setCardId(value);
  }), options: [{ value: "", label: t2("memory.pickCharacter") }, ...charItems.map((c) => ({ value: c.cardId, label: c.name }))] }) }), cardId && (0, import_jsx_runtime7.jsx)(SettingsRow, { title: t2("memory.story"), description: t2("memory.storyDesc"), children: (0, import_jsx_runtime7.jsx)(Select, { value: storyId ?? "", disabled: busy || editingId !== null, onChange: (value) => guard.request(() => setStoryId(value || void 0)), options: [{ value: "", label: t2("memory.initialState") }, ...stories.state.status === "ready" ? stories.state.value.items.map((story, index) => ({ value: story.id, label: t2("memory.storyLabel", { index: index + 1, date: story.createdAt.slice(0, 10) }) })) : []] }) }), cardId && (0, import_jsx_runtime7.jsxs)("div", { className: "dsh-tavern-storyContext", role: "status", children: [(0, import_jsx_runtime7.jsxs)("div", { className: "dsh-tavern-storyContextTitle", children: [(0, import_jsx_runtime7.jsx)("strong", { children: charItems.find((c) => c.cardId === cardId)?.name ?? t2("memory.character") }), (0, import_jsx_runtime7.jsx)(Badge, { accent: !!storyId, children: t2(storyId ? "memory.scopeStory" : "memory.scopeInitial") })] }), (0, import_jsx_runtime7.jsx)("span", { children: t2(storyId ? "memory.scopeStoryDesc" : "memory.scopeInitialDesc") }), storyId && (0, import_jsx_runtime7.jsx)("span", { className: "dsh-tavern-muted", children: stories.state.status === "ready" ? stories.state.value.items.find((s) => s.id === storyId)?.sessionId ?? storyId : storyId })] }), (0, import_jsx_runtime7.jsx)(Err, { message: stories.state.status === "error" ? stories.state.message : error }), cardId && (0, import_jsx_runtime7.jsxs)(import_jsx_runtime7.Fragment, { children: [(0, import_jsx_runtime7.jsxs)("div", { style: { display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10, margin: "10px 0 14px" }, children: [(0, import_jsx_runtime7.jsxs)("div", { className: "dsh-tavern-filters", children: [(0, import_jsx_runtime7.jsx)("button", { type: "button", className: "dsh-tavern-chip", "data-active": tab === "memory" ? "true" : "false", disabled: busy || editingId !== null, onClick: () => setTab("memory"), children: t2("memory.tab.memory", { count: memoryItems.length }) }), (0, import_jsx_runtime7.jsx)("button", { type: "button", className: "dsh-tavern-chip", "data-active": tab === "delta" ? "true" : "false", disabled: busy || editingId !== null, onClick: () => setTab("delta"), children: t2("memory.tab.delta", { count: deltaItems.length }) }), (0, import_jsx_runtime7.jsx)("button", { type: "button", className: "dsh-tavern-chip", "data-active": tab === "journal" ? "true" : "false", disabled: busy || editingId !== null, onClick: () => setTab("journal"), children: t2("memory.tab.journal") })] }), (0, import_jsx_runtime7.jsx)("span", { style: { flex: 1 } }), tab === "memory" && (0, import_jsx_runtime7.jsx)(Btn, { disabled: busy, onClick: () => void compress(), children: t2("memory.compressOldest") }), tab === "delta" && (0, import_jsx_runtime7.jsx)(Btn, { disabled: busy, onClick: () => void exportBook(), children: t2("memory.exportBook") })] }), tab === "memory" && (0, import_jsx_runtime7.jsxs)("div", { className: "dsh-tavern-list", children: [memories.state.status === "loading" && (0, import_jsx_runtime7.jsxs)(import_jsx_runtime7.Fragment, { children: [(0, import_jsx_runtime7.jsx)(Skeleton, { height: 72 }), (0, import_jsx_runtime7.jsx)(Skeleton, { height: 72 }), (0, import_jsx_runtime7.jsx)(Skeleton, { height: 72 })] }), memories.state.status === "error" && (0, import_jsx_runtime7.jsx)(Err, { message: memories.state.message }), memoryItems.length === 0 && memories.state.status === "ready" && (0, import_jsx_runtime7.jsxs)("div", { className: "dsh-tavern-empty is-compact", children: [(0, import_jsx_runtime7.jsx)("div", { className: "dsh-tavern-emptyTitle", children: t2("memory.emptyMemories") }), (0, import_jsx_runtime7.jsx)("div", { className: "dsh-tavern-emptyDesc", children: t2("memory.emptyMemoriesDesc") })] }), memoryItems.map((m) => (0, import_jsx_runtime7.jsxs)("div", { className: "dsh-tavern-memo", children: [(0, import_jsx_runtime7.jsxs)("div", { className: "dsh-tavern-memoHead", children: [(0, import_jsx_runtime7.jsx)(Badge, { children: m.id }), m.archived ? (0, import_jsx_runtime7.jsx)(Badge, { children: t2("memory.archived") }) : null, m.tags.map((tag) => (0, import_jsx_runtime7.jsx)(Badge, { children: tag }, tag)), (0, import_jsx_runtime7.jsx)("span", { className: "dsh-tavern-memoMeta", children: m.updated }), (0, import_jsx_runtime7.jsxs)("span", { className: "dsh-tavern-memoActions", children: [(0, import_jsx_runtime7.jsx)(IconBtn, { disabled: editingId !== null || busy, label: t2("action.edit"), onClick: () => setEditingId(editingId === m.id ? null : m.id), children: (0, import_jsx_runtime7.jsx)(import_dsh_client_ui_primitives5.IconEditOutline16, {}) }), (0, import_jsx_runtime7.jsx)(IconBtn, { label: t2("memory.deleteEntry"), danger: true, disabled: busy || editingId !== null, onClick: () => setDeleteId(m.id), children: (0, import_jsx_runtime7.jsx)(import_dsh_client_ui_primitives5.IconTrashOutline16, {}) })] })] }), (0, import_jsx_runtime7.jsx)("pre", { className: "dsh-tavern-memoBody dsh-tavern-scroll", children: m.body }), editingId === m.id && (0, import_jsx_runtime7.jsx)(MemoryEditor, { remote, cardId, storyId, entry: m, onDone: () => {
    setEditingId(null);
    memories.reload();
  } })] }, m.id)), (0, import_jsx_runtime7.jsxs)("div", { className: "dsh-tavern-memo is-compose", children: [(0, import_jsx_runtime7.jsx)("textarea", { className: "dsh-tavern-input dsh-tavern-textarea", style: { minHeight: 60 }, placeholder: t2("memory.newPlaceholder"), disabled: busy, value: newBody, onChange: (e) => setNewBody(e.target.value) }), (0, import_jsx_runtime7.jsx)("div", { style: { display: "flex", justifyContent: "flex-end" }, children: (0, import_jsx_runtime7.jsx)(Btn, { primary: true, disabled: busy || !newBody.trim(), onClick: () => void addMemory(), children: t2("memory.addEntry") }) })] })] }), tab === "delta" && (0, import_jsx_runtime7.jsxs)("div", { className: "dsh-tavern-list", children: [deltas.state.status === "loading" && (0, import_jsx_runtime7.jsxs)(import_jsx_runtime7.Fragment, { children: [(0, import_jsx_runtime7.jsx)(Skeleton, { height: 72 }), (0, import_jsx_runtime7.jsx)(Skeleton, { height: 72 }), (0, import_jsx_runtime7.jsx)(Skeleton, { height: 72 })] }), deltas.state.status === "error" && (0, import_jsx_runtime7.jsx)(Err, { message: deltas.state.message }), deltaItems.map((d) => (0, import_jsx_runtime7.jsxs)("div", { className: `dsh-tavern-memo${d.revoked ? " is-revoked" : ""}`, children: [(0, import_jsx_runtime7.jsxs)("div", { className: "dsh-tavern-memoHead", children: [(0, import_jsx_runtime7.jsxs)(Badge, { children: ["#", d.id] }), (0, import_jsx_runtime7.jsx)(Badge, { danger: d.type === "invalidate", children: t2(DELTA_TYPE_KEY[d.type]) }), d.ref ? (0, import_jsx_runtime7.jsxs)(Badge, { children: ["\u2192 ", d.ref] }) : null, d.revoked ? (0, import_jsx_runtime7.jsx)(Badge, { danger: true, children: t2("memory.revoked") }) : null, (0, import_jsx_runtime7.jsx)("span", { className: "dsh-tavern-memoMeta", children: d.ts }), !d.revoked && (0, import_jsx_runtime7.jsx)("span", { className: "dsh-tavern-memoActions", children: (0, import_jsx_runtime7.jsx)(Btn, { size: "sm", disabled: busy, onClick: () => void revoke(d.id), children: t2("memory.revoke") }) })] }), (0, import_jsx_runtime7.jsx)("pre", { className: "dsh-tavern-memoBody dsh-tavern-scroll", children: d.content })] }, d.id)), deltaItems.length === 0 && deltas.state.status === "ready" && (0, import_jsx_runtime7.jsxs)("div", { className: "dsh-tavern-empty is-compact", children: [(0, import_jsx_runtime7.jsx)("div", { className: "dsh-tavern-emptyTitle", children: t2("memory.emptyDeltas") }), (0, import_jsx_runtime7.jsx)("div", { className: "dsh-tavern-emptyDesc", children: t2("memory.emptyDeltasDesc") })] }), (0, import_jsx_runtime7.jsxs)("div", { className: "dsh-tavern-memo is-compose", children: [(0, import_jsx_runtime7.jsxs)("div", { className: "dsh-tavern-fieldRow", children: [(0, import_jsx_runtime7.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime7.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("memory.deltaType") }), (0, import_jsx_runtime7.jsx)(Select, { size: "md", value: deltaType, onChange: (v) => setDeltaType(v), options: [
  ...
  ] })] }), (deltaType === "update" || deltaType === "invalidate") && (0, import_jsx_runtime7.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime7.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("memory.deltaRef") }), (0, import_jsx_runtime7.jsx)("input", { className: "dsh-tavern-input", value: deltaRef, onChange: (e) => setDeltaRef(e.target.value) })] })] }), (0, import_jsx_runtime7.jsx)("textarea", { className: "dsh-tavern-input dsh-tavern-textarea", style: { minHeight: 60, marginTop: 8 }, placeholder: t2("memory.deltaBodyPlaceholder"), disabled: busy, value: deltaContent, onChange: (e) => setDeltaContent(e.target.value) }), (0, import_jsx_runtime7.jsxs)("label", { className: "dsh-tavern-field", style: { marginTop: 8 }, children: [(0, import_jsx_runtime7.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("memory.deltaKeys") }), (0, import_jsx_runtime7.jsx)("input", { className: "dsh-tavern-input", value: deltaKeys, onChange: (e) => setDeltaKeys(e.target.value) })] }), (0, import_jsx_runtime7.jsx)("div", { style: { display: "flex", justifyContent: "flex-end", marginTop: 8 }, children: (0, import_jsx_runtime7.jsx)(Btn, { primary: true, disabled: busy || !deltaContent.trim(), onClick: () => void addDelta(), children: t2("memory.addDelta") }) })] })] }), tab === "journal" && (0, import_jsx_runtime7.jsxs)("div", { className: "dsh-tavern-list", children: [(0, import_jsx_runtime7.jsx)(Muted, { children: t2("memory.journalHint") }), journal.state.status === "ready" ? (0, import_jsx_runtime7.jsx)("textarea", { className: "dsh-tavern-input dsh-tavern-textarea", style: { minHeight: 180 }, disabled: busy, value: journalText, onChange: (e) => setJournalText(e.target.value) }) : journal.state.status === "error" ? (0, import_jsx_runtime7.jsx)(Err, { message: journal.state.message }) : (0, import_jsx_runtime7.jsx)(Skeleton, { height: 180 }), (0, import_jsx_runtime7.jsx)("div", { style: { display: "flex", justifyContent: "flex-end" }, children: (0, import_jsx_runtime7.jsx)(Btn, { primary: true, disabled: busy || !journalDirty || journal.state.status !== "ready" || !cardId, onClick: () => void saveJournal(), children: t2("memory.saveJournal") }) })] })] })] });
}
...
function TavernSeatChip(props) {
  return (0, import_jsx_runtime8.jsxs)("button", { type: "button", className: "dsh-tavern-seat", "aria-haspopup": props.hasPopup ?? "menu", "aria-expanded": props.open, title: props.title ?? props.label, disabled: props.disabled || props.loading, onClick: props.onClick, children: [props.loading ? (0, import_jsx_runtime8.jsxs)(import_jsx_runtime8.Fragment, { children: [(0, import_jsx_runtime8.jsx)("span", { className: "dsh-tavern-seatIcon", children: (0, import_jsx_runtime8.jsx)(Skeleton, { width: 16, height: 16, radius: 999 }) }), (0, import_jsx_runtime8.jsx)("span", { className: "dsh-tavern-seatLabel", children: (0, import_jsx_runtime8.jsx)(Skeleton, { width: 64, height: 13 }) })] }) : (0, import_jsx_runtime8.jsxs)(import_jsx_runtime8.Fragment, { children: [props.avatarUrl ? (0, import_jsx_runtime8.jsx)("span", { className: "dsh-tavern-seatIcon", children: (0, import_jsx_runtime8.jsx)(Avatar, { url: props.avatarUrl, name: props.label, size: 16 }) }) : null, (0, import_jsx_runtime8.jsx)("span", { className: "dsh-tavern-seatLabel", children: props.label })] }), props.chevron !== false ? (0, import_jsx_runtime8.jsx)(import_dsh_client_ui_primitives6.IconChevronDownOutline14, { className: "dsh-tavern-seatChevron" }) : null, props.trailing] });
}
...
function LorebookEditor(props) {
  const fallback = (0, import_react9.useId)();
  const draftKey = targetDraftKey(props.target, fallback);
  const editor = (0, import_jsx_runtime9.jsx)(LorebookEditorContent, { ...props, draftKey }, draftKey);
  if (!props.remote || props.target.kind === "chat" && !props.target.storyId)
    return editor;
  return (0, import_jsx_runtime9.jsx)(PersistentEditor, { remote: props.remote, scope: draftKey, children: editor });
}
function LorebookEditorContent(props) {
  ...
  const savedEntries = (0, import_react9.useRef)(props.entries);
  ...
  const [leaveConfirm, setLeaveConfirm] = (0, import_react9.useState)(false);
  const mark = (next) => {
    setEntries(next);
    setDirty(true);
  };
...
${e.content.slice(0, 400)}`.toLowerCase();
  ...
  return (0, import_jsx_runtime9.jsxs)("div", { children: [(0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-toolbar", children: [(0, import_jsx_runtime9.jsx)(Btn, { size: "md", onClick: askClose, children: target.kind === "chat" ? t2("action.close") : t2("lorebookEditor.backToList") }), (0, import_jsx_runtime9.jsxs)("div", { style: { flex: 1, minWidth: 0 }, children: [(0, import_jsx_runtime9.jsx)("div", { className: "dsh-tavern-cardName", style: { fontSize: 15 }, children: target.name }), (0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-editorMeta", children: [(0, import_jsx_runtime9.jsxs)(Muted, { children: [t2("lorebookEditor.meta", { kind: targetKindLabel(t2, target.kind), total: entries.length, enabled: enabledCount }), constantCount > 0 ? t2("lorebookEditor.metaConstant", { count: constantCount }) : ""] }), dirty ? (0, import_jsx_runtime9.jsx)(Badge, { accent: true, children: t2("lorebookEditor.unsaved") }) : null] })] }), (0, import_jsx_runtime9.jsx)(Btn, { size: "md", onClick: addEntry, children: t2("lorebookEditor.newEntry") }), (0, import_jsx_runtime9.jsx)(Btn, { primary: true, size: "md", disabled: busy || !dirty, onClick: () => void save(), children: t2("action.save") })] }), (0, import_jsx_runtime9.jsx)(Err, { message: error }), (0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-search", style: { margin: "10px 0 12px" }, children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-searchIcon", children: (0, import_jsx_runtime9.jsx)(import_dsh_client_ui_primitives7.IconSearchOutline16, {}) }), (0, import_jsx_runtime9.jsx)("input", { value: query, placeholder: t2("lorebookEditor.searchPlaceholder"), onChange: (e) => {
    setQuery(e.target.value);
    setPage(0);
  } })] }), (0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-filters", children: [[
  ...
  ].map(([id, label]) => (0, import_jsx_runtime9.jsx)("button", { type: "button", className: "dsh-tavern-chip", "data-active": filter === id ? "true" : "false", onClick: () => {
    setFilter(id);
    setPage(0);
  }, children: label }, id)), (0, import_jsx_runtime9.jsx)("span", { style: { flex: 1 } }), (0, import_jsx_runtime9.jsx)(Btn, { size: "sm", onClick: () => applyFilterEnabled(true), disabled: filtered.length === 0, children: t2("lorebookEditor.enableFiltered") }), (0, import_jsx_runtime9.jsx)(Btn, { size: "sm", onClick: () => applyFilterEnabled(false), disabled: filtered.length === 0, children: t2("lorebookEditor.disableFiltered") })] }), filtered.length === 0 ? (0, import_jsx_runtime9.jsx)(Muted, { children: entries.length === 0 ? t2("lorebookEditor.empty") : t2("common.noMatch", { what: t2("lorebookEditor.entryNoun") }) }) : (0, import_jsx_runtime9.jsx)("div", { className: "dsh-tavern-list dsh-tavern-scroll", children: pageItems.map((entry) => {
    const open = openUid === entry.uid;
    return (0, import_jsx_runtime9.jsxs)("div", { className: `dsh-tavern-entry${open ? " is-open" : ""}${entry.enabled ? "" : " is-off"}`, children: [(0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-entryHead", onClick: () => setOpenUid(open ? null : entry.uid), children: [(0, import_jsx_runtime9.jsx)(Toggle, { checked: entry.enabled, title: entry.enabled ? t2("lorebookEditor.entry.disable") : t2("lorebookEditor.entry.enable"), onChange: (enabled) => patch(entry.uid, { enabled }) }), (0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-entryMain", children: [(0, import_jsx_runtime9.jsx)("div", { className: "dsh-tavern-entryTitle", children: entryTitle(t2, entry) }), (0, import_jsx_runtime9.jsx)("div", { className: "dsh-tavern-entrySub", children: entrySub(t2, entry) })] }), (0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-entryBadges", children: [entry.constant ? (0, import_jsx_runtime9.jsx)(Badge, { children: t2("lorebookEditor.constant") }) : null, entry.keys.length > 0 ? (0, import_jsx_runtime9.jsx)(Badge, { children: t2("lorebookEditor.entry.keys", { count: entry.keys.length }) }) : (0, import_jsx_runtime9.jsx)(Badge, { children: t2("lorebookEditor.entry.noKeys") })] }), (0, import_jsx_runtime9.jsx)("span", { className: `dsh-tavern-chevron${open ? " is-open" : ""}`, children: (0, import_jsx_runtime9.jsx)(import_dsh_client_ui_primitives7.IconChevronDownOutline14, {}) })] }), (0, import_jsx_runtime9.jsx)("div", { className: `dsh-tavern-collapse${open ? " is-open" : ""}`, children: (0, import_jsx_runtime9.jsx)("div", { className: "dsh-tavern-collapseInner", children: (0, import_jsx_runtime9.jsx)(EntryForm, { entry, advanced, onAdvanced: setAdvanced, onChange: (partial2) => patch(entry.uid, partial2), onDelete: () => setToDelete(entry.uid) }) }) })] }, entry.uid);
  }) }), pageCount > 1 && (0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-pager", children: [(0, import_jsx_runtime9.jsx)(Btn, { size: "sm", disabled: safePage <= 0, onClick: () => setPage(safePage - 1), children: t2("lorebookEditor.pager.prev") }), (0, import_jsx_runtime9.jsx)(Muted, { children: t2("lorebookEditor.pager.status", { page: safePage + 1, pageCount, count: pageItems.length }) }), (0, import_jsx_runtime9.jsx)(Btn, { size: "sm", disabled: safePage >= pageCount - 1, onClick: () => setPage(safePage + 1), children: t2("lorebookEditor.pager.next") })] }), (0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-stickyBar", children: [(0, import_jsx_runtime9.jsx)(Btn, { size: "md", onClick: addEntry, children: (0, import_jsx_runtime9.jsxs)("span", { style: { display: "inline-flex", alignItems: "center", gap: 6 }, children: [(0, import_jsx_runtime9.jsx)(import_dsh_client_ui_primitives7.IconPlusOutline16, {}), " ", t2("lorebookEditor.newEntry")] }) }), (0, import_jsx_runtime9.jsx)(Btn, { primary: true, size: "md", disabled: busy || !dirty, onClick: () => void save(), children: t2("action.save") }), (0, import_jsx_runtime9.jsx)(Btn, { size: "md", onClick: askClose, children: dirty ? t2("lorebookEditor.discardAndBack") : t2("lorebookEditor.back") })] }), (0, import_jsx_runtime9.jsx)(ConfirmDialog, { open: toDelete !== null, title: t2("lorebookEditor.delete.title"), description: t2("lorebookEditor.delete.desc"), confirmLabel: t2("lorebookEditor.deleteEntry"), danger: true, onCancel: () => setToDelete(null), onConfirm: removeEntry }), (0, import_jsx_runtime9.jsx)(ConfirmDialog, { open: leaveConfirm, title: t2("lorebookEditor.discard.title"), description: t2("lorebookEditor.discard.desc"), confirmLabel: t2("lorebookEditor.discard.confirm"), danger: true, onCancel: () => setLeaveConfirm(false), onConfirm: () => {
    if (busy)
      return;
...
function EntryForm(props) {
  ...
  return (0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-entryBody", onClick: (e) => e.stopPropagation(), children: [(0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("lorebookEditor.form.comment") }), (0, import_jsx_runtime9.jsx)("input", { className: "dsh-tavern-input", style: { width: "100%", height: 36, padding: "0 10px", fontSize: 13, boxSizing: "border-box" }, value: entry.comment, placeholder: t2("lorebookEditor.form.commentPlaceholder"), onChange: (e) => set({ comment: e.target.value }) })] }), (0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("lorebookEditor.form.keys") }), (0, import_jsx_runtime9.jsx)("input", { className: "dsh-tavern-input dsh-tavern-codeFont", style: { width: "100%", height: 36, padding: "0 10px", fontSize: 13, boxSizing: "border-box" }, value: joinKeys(entry.keys), placeholder: t2("lorebookEditor.form.keysPlaceholder"), onChange: (e) => set({ keys: splitKeys(e.target.value) }) })] }), (0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("lorebookEditor.form.content") }), (0, import_jsx_runtime9.jsx)("textarea", { className: "dsh-tavern-input dsh-tavern-textarea", style: { minHeight: 120 }, value: entry.content, placeholder: t2("lorebookEditor.form.contentPlaceholder"), onChange: (e) => set({ content: e.target.value }) })] }), (0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-inlineChecks", children: [(0, import_jsx_runtime9.jsxs)("label", { children: [(0, import_jsx_runtime9.jsx)(Toggle, { checked: entry.constant, onChange: (constant) => set({ constant }) }), t2("lorebookEditor.form.constant")] }), (0, import_jsx_runtime9.jsxs)("label", { children: [(0, import_jsx_runtime9.jsx)(Toggle, { checked: entry.selective, onChange: (selective) => set({ selective }) }), t2("lorebookEditor.form.selective")] }), (0, import_jsx_runtime9.jsxs)("label", { children: [(0, import_jsx_runtime9.jsx)(Toggle, { checked: entry.ignoreBudget, onChange: (ignoreBudget) => set({ ignoreBudget }) }), t2("lorebookEditor.form.ignoreBudget")] })] }), (0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-fieldRow", children: [(0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("lorebookEditor.form.position") }), (0, import_jsx_runtime9.jsx)(Select, { size: "md", value: String(entry.position), onChange: (v) => set({ position: Number(v) }), options: positionOptions(t2) })] }), (0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("lorebookEditor.form.order") }), (0, import_jsx_runtime9.jsx)(NumInput, { value: entry.order, onChange: (order) => set({ order: Math.round(order) }) })] }), entry.position === 4 || entry.position === 7 ? (0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: entry.position === 7 ? t2("lorebookEditor.form.outletName") : t2("lorebookEditor.form.depth") }), entry.position === 7 ? (0, import_jsx_runtime9.jsx)("input", { className: "dsh-tavern-input", style: { width: "100%", height: 36, padding: "0 10px", boxSizing: "border-box" }, value: entry.outletName, onChange: (e) => set({ outletName: e.target.value }) }) : (0, import_jsx_runtime9.jsx)(NumInput, { value: entry.depth, onChange: (depth) => set({ depth: Math.max(0, Math.round(depth)) }) })] }) : null] }), entry.selective ? (0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-fieldRow", children: [(0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("lorebookEditor.form.secondaryKeys") }), (0, import_jsx_runtime9.jsx)("input", { className: "dsh-tavern-input dsh-tavern-codeFont", style: { width: "100%", height: 36, padding: "0 10px", fontSize: 13, boxSizing: "border-box" }, value: joinKeys(entry.secondaryKeys), placeholder: t2("lorebookEditor.form.secondaryKeysPlaceholder"), onChange: (e) => set({ secondaryKeys: splitKeys(e.target.value) }) })] }), (0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("lorebookEditor.form.selectiveLogic") }), (0, import_jsx_runtime9.jsx)(Select, { size: "md", value: String(entry.selectiveLogic), onChange: (v) => set({ selectiveLogic: Number(v) }), options: logicOptions(t2) })] })] }) : null, (0, import_jsx_runtime9.jsx)(Btn, { size: "sm", onClick: () => props.onAdvanced(!props.advanced), children: props.advanced ? t2("lorebookEditor.form.advanced.hide") : t2("lorebookEditor.form.advanced.show") }), props.advanced ? (0, import_jsx_runtime9.jsxs)(import_jsx_runtime9.Fragment, { children: [(0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-fieldRow", children: [(0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("lorebookEditor.form.probability") }), (0, import_jsx_runtime9.jsx)(NumInput, { value: entry.probability, onChange: (probability) => set({ probability }) })] }), (0, import_jsx_runtime9.jsx)("label", { className: "dsh-tavern-inlineChecks", style: { paddingTop: 22 }, children: (0, import_jsx_runtime9.jsxs)("span", { children: [(0, import_jsx_runtime9.jsx)(Toggle, { checked: entry.useProbability, onChange: (useProbability) => set({ useProbability }) }), " ", t2("lorebookEditor.form.useProbability")] }) }), entry.position === 4 ? (0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("lorebookEditor.form.role") }), (0, import_jsx_runtime9.jsx)(Select, { size: "md", value: String(entry.role), onChange: (v) => set({ role: Number(v) }), options: ROLE_OPTIONS })] }) : null] }), (0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-inlineChecks", children: [(0, import_jsx_runtime9.jsxs)("label", { children: [(0, import_jsx_runtime9.jsx)(Toggle, { checked: entry.excludeRecursion, onChange: (excludeRecursion) => set({ excludeRecursion }) }), t2("lorebookEditor.form.excludeRecursion")] }), (0, import_jsx_runtime9.jsxs)("label", { children: [(0, import_jsx_runtime9.jsx)(Toggle, { checked: entry.preventRecursion, onChange: (preventRecursion) => set({ preventRecursion }) }), t2("lorebookEditor.form.preventRecursion")] })] }), (0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-fieldRow", children: [(0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("lorebookEditor.form.delayUntilRecursion") }), (0, import_jsx_runtime9.jsx)(NumInput, { value: entry.delayUntilRecursion, onChange: (delayUntilRecursion) => set({ delayUntilRecursion: Math.max(0, Math.round(delayUntilRecursion)) }) })] }), (0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: "sticky" }), (0, import_jsx_runtime9.jsx)(NullableNumInput, { value: entry.sticky, onChange: (sticky) => set({ sticky }) })] }), (0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: "cooldown" }), (0, import_jsx_runtime9.jsx)(NullableNumInput, { value: entry.cooldown, onChange: (cooldown) => set({ cooldown }) })] }), (0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: "delay" }), (0, import_jsx_runtime9.jsx)(NullableNumInput, { value: entry.delay, onChange: (delay) => set({ delay }) })] })] }), (0, import_jsx_runtime9.jsxs)("div", { className: "dsh-tavern-fieldRow", children: [(0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("lorebookEditor.form.scanDepth") }), (0, import_jsx_runtime9.jsx)(NullableNumInput, { value: entry.scanDepth, onChange: (scanDepth) => set({ scanDepth }) })] }), (0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("lorebookEditor.form.caseSensitive") }), (0, import_jsx_runtime9.jsx)(Select, { size: "md", value: triValue(entry.caseSensitive), onChange: (v) => set({ caseSensitive: triFrom(v) }), options: triStateOptions(t2) })] }), (0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("lorebookEditor.form.matchWholeWords") }), (0, import_jsx_runtime9.jsx)(Select, { size: "md", value: triValue(entry.matchWholeWords), onChange: (v) => set({ matchWholeWords: triFrom(v) }), options: triStateOptions(t2) })] }), (0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("lorebookEditor.form.group") }), (0, import_jsx_runtime9.jsx)("input", { className: "dsh-tavern-input", style: { width: "100%", height: 36, padding: "0 10px", boxSizing: "border-box" }, value: entry.group, onChange: (e) => set({ group: e.target.value }) })] }), (0, import_jsx_runtime9.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime9.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("lorebookEditor.form.groupWeight") }), (0, import_jsx_runtime9.jsx)(NumInput, { value: entry.groupWeight, onChange: (groupWeight) => set({ groupWeight: Math.max(0, Math.round(groupWeight)) }) })] })] }), (0, import_jsx_runtime9.jsx)("div", { className: "dsh-tavern-inlineChecks", children: (0, import_jsx_runtime9.jsxs)("label", { children: [(0, import_jsx_runtime9.jsx)(Toggle, { checked: entry.groupOverride, onChange: (groupOverride) => set({ groupOverride }) }), t2("lorebookEditor.form.groupOverride")] }) })] }) : null, (0, import_jsx_runtime9.jsx)("div", { style: { display: "flex", justifyContent: "flex-end" }, children: (0, import_jsx_runtime9.jsx)(IconBtn, { label: t2("lorebookEditor.deleteEntry"), danger: true, onClick: props.onDelete, children: (0, import_jsx_runtime9.jsx)(import_dsh_client_ui_primitives7.IconTrashOutline16, {}) }) })] });
}
...
function PreDialog(props) {
  return (0, import_jsx_runtime10.jsx)(Dialog, { open: true, title: props.title, onClose: props.onClose, width: "lg", children: (0, import_jsx_runtime10.jsx)("pre", { className: "dsh-tavern-modalPre", children: props.text }) });
}
...
function PromptPreviewDialog(props) {
  const { data } = props;
  const t2 = useT();
  const tabsId = (0, import_react10.useId)();
  const [tab, setTab] = (0, import_react10.useState)("actual");
  const toast = useToast();
...
${data.messages.map((m) => JSON.stringify(m)).join("\n\n")}`;
  ...
  return (0, import_jsx_runtime10.jsxs)(Dialog, { open: true, title: t2("chip.preview.title"), onClose: props.onClose, width: "lg", children: [(0, import_jsx_runtime10.jsx)(Muted, { children: t2("chip.preview.notice") }), (0, import_jsx_runtime10.jsxs)(Muted, { children: [t2("chip.preview.budget", { used: wi.used, limit: wi.limit }), wi.overflowed ? ` \xB7 ${t2("chip.preview.overflowed")}` : "", " \xB7 ", t2("chip.preview.assemble", { after: assemble.tokensAfter, before: assemble.tokensBefore }), assemble.trimmedSections.length > 0 ? ` \xB7 ${t2("chip.preview.trimmed", { sections: assemble.trimmedSections.join(t2("chip.preview.listSep")) })}` : ""] }), (0, import_jsx_runtime10.jsxs)("div", { className: "dsh-tavern-filters", style: { margin: "10px 0 12px" }, children: [(0, import_jsx_runtime10.jsx)(Tabs, { id: tabsId, panelId: `${tabsId}-panel`, label: t2("chip.preview.title"), size: "sm", value: tab, onChange: (id) => setTab(id), items: [
  ...
  ] }), (0, import_jsx_runtime10.jsx)("span", { style: { flex: 1 } }), (0, import_jsx_runtime10.jsx)(IconBtn, { label: t2("chip.preview.copyView"), onClick: () => void copyBody(), children: (0, import_jsx_runtime10.jsx)(import_dsh_client_ui_primitives8.IconCopyOutline16, {}) })] }), (0, import_jsx_runtime10.jsx)("pre", { id: `${tabsId}-panel`, role: "tabpanel", "aria-labelledby": `${tabsId}-${tab}`, tabIndex: 0, className: "dsh-tavern-modalPre", children: body }), toast.node] });
}
function TavernHeaderChip(props) {
  ...
  const [lists, setLists] = (0, import_react10.useState)(null);
  const [draft, setDraft] = (0, import_react10.useState)(null);
  const [error, setError] = (0, import_react10.useState)(null);
  const toast = useToast();
  const [view, setView] = (0, import_react10.useState)(null);
  const [previewData, setPreviewData] = (0, import_react10.useState)(null);
  const [chatLore, setChatLore] = (0, import_react10.useState)(null);
  const [memoryOpen, setMemoryOpen] = (0, import_react10.useState)(false);
  const [confirmUnbind, setConfirmUnbind] = (0, import_react10.useState)(false);
  const [unbindBusy, setUnbindBusy] = (0, import_react10.useState)(false);
  const characterRequest = (0, import_react10.useRef)(0);
...
${log.lines.join("\n")}` : t2("chip.log.empty")
  ...
  return (0, import_jsx_runtime10.jsxs)("span", { className: "dsh-tavern-ui", style: { display: "inline-flex" }, children: [(0, import_jsx_runtime10.jsx)(TavernSeatChip, { label: binding ? name2 ?? listedName ?? t2("chip.characterFallback") : t2("hero.pickCharacter"), title: t2("hero.pickCharacter"), avatarUrl: avatar, open, loading: bindingLoader.state.status === "loading", hasPopup: "dialog", onClick: () => setOpen(!open) }), (0, import_jsx_runtime10.jsx)(Dialog, { open, title: t2("chip.dialog.title"), onClose: () => setOpen(false), width: "full", footer: draft ? (0, import_jsx_runtime10.jsxs)("div", { className: "dsh-tavern-footActions", children: [binding ? (0, import_jsx_runtime10.jsx)(Btn, { danger: true, size: "md", onClick: () => {
    setConfirmUnbind(true);
  }, children: t2("chip.unbind.action") }) : null, (0, import_jsx_runtime10.jsx)("span", { className: "dsh-tavern-footSpacer" }), (0, import_jsx_runtime10.jsxs)("span", { className: "dsh-tavern-footGroup", children: [(0, import_jsx_runtime10.jsx)(Btn, { size: "md", disabled: !binding?.storyId || draft?.cardId !== binding.cardId, onClick: () => void openChatLore(), children: t2("chip.chatLore.edit") }), (0, import_jsx_runtime10.jsx)(Btn, { size: "md", disabled: !binding?.storyId, onClick: () => setMemoryOpen(true), children: t2("section.memory") }), (0, import_jsx_runtime10.jsx)(Btn, { primary: true, size: "md", onClick: () => void saveBinding(), children: t2("binding.save") })] })] }) : void 0, children: (0, import_jsx_runtime10.jsxs)("div", { className: "dsh-tavern-binding dsh-tavern-bindingWide", children: [(0, import_jsx_runtime10.jsx)(Err, { message: error }), !lists && (0, import_jsx_runtime10.jsxs)("div", { className: "dsh-tavern-panelCard", children: [(0, import_jsx_runtime10.jsx)(Skeleton, { height: 14, width: "24%" }), (0, import_jsx_runtime10.jsx)(Skeleton, { height: 38, radius: 18 }), (0, import_jsx_runtime10.jsx)(Skeleton, { height: 38, radius: 18 }), (0, import_jsx_runtime10.jsx)(Skeleton, { height: 38, radius: 18 })] }), lists && (0, import_jsx_runtime10.jsxs)(import_jsx_runtime10.Fragment, { children: [draft ? (0, import_jsx_runtime10.jsxs)(import_jsx_runtime10.Fragment, { children: [(0, import_jsx_runtime10.jsxs)("div", { className: "dsh-tavern-panelCard", children: [(0, import_jsx_runtime10.jsx)("div", { className: "dsh-tavern-groupHead", children: t2("chip.group.binding") }), (0, import_jsx_runtime10.jsx)(Field, { label: t2("chip.field.character"), children: (0, import_jsx_runtime10.jsx)(Select, { width: "100%", value: draft.cardId, onChange: (cardId) => {
    if (!cardId)
      return;
  ...
  ] }) }), (0, import_jsx_runtime10.jsx)(Field, { label: t2("chip.field.preset"), children: (0, import_jsx_runtime10.jsx)(Select, { width: "100%", value: draft.presetId ?? "", onChange: (v) => setDraft({ ...draft, presetId: v || null }), options: [{ value: "", label: t2("chip.field.builtinPreset") }, ...lists.presets.map((p) => ({ value: p.id, label: p.regexCount > 0 ? t2("settings.defaults.presetRegexCount", { name: p.name, count: p.regexCount }) : p.name }))] }) }), (0, import_jsx_runtime10.jsx)(Field, { label: t2("chip.field.persona"), children: (0, import_jsx_runtime10.jsx)(Select, { width: "100%", value: draft.personaId ?? "", onChange: (v) => setDraft({ ...draft, personaId: v || null }), options: [{ value: "", label: t2("chip.field.none") }, ...lists.personas.map((p) => ({ value: p.id, label: p.name }))] }) })] }), (0, import_jsx_runtime10.jsxs)("div", { className: "dsh-tavern-panelCard", children: [(0, import_jsx_runtime10.jsx)("div", { className: "dsh-tavern-groupHead", children: t2("section.lorebooks") }), (0, import_jsx_runtime10.jsx)(Field, { label: t2("chip.field.mainLore"), children: (0, import_jsx_runtime10.jsx)(Select, { width: "100%", value: draft.characterLorebookId ?? "", onChange: (v) => setDraft({ ...draft, characterLorebookId: v || null }), options: [
    { value: "", label: embeddedBookLabel },
    ...lists.lorebooks.map((n) => ({ value: n, label: n }))
  ] }) }), (0, import_jsx_runtime10.jsx)(Field, { label: t2("chip.field.globalLore"), children: lists.lorebooks.length === 0 ? (0, import_jsx_runtime10.jsx)(Muted, { children: t2("chip.field.noLorebooks") }) : (0, import_jsx_runtime10.jsx)(CheckChips, { ariaLabel: t2("chip.field.globalLoreAria"), options: lists.lorebooks.map((n) => ({ value: n, label: n })), selected: draft.lorebookIds, onChange: (lorebookIds) => setDraft({ ...draft, lorebookIds }) }) })] }), (0, import_jsx_runtime10.jsxs)("div", { className: "dsh-tavern-panelCard", children: [(0, import_jsx_runtime10.jsx)("div", { className: "dsh-tavern-groupHead", children: t2("chip.group.turnInject") }), (0, import_jsx_runtime10.jsx)(Field, { label: t2("chip.field.authorNote"), children: (0, import_jsx_runtime10.jsx)("textarea", { className: "dsh-tavern-input dsh-tavern-textarea", style: { minHeight: 96 }, value: draft.authorNote ?? "", onChange: (e) => setDraft({ ...draft, authorNote: e.target.value }) }) }), (0, import_jsx_runtime10.jsx)("div", { className: "dsh-tavern-inlineChecks", children: (0, import_jsx_runtime10.jsxs)("label", { children: [(0, import_jsx_runtime10.jsx)(Toggle, { checked: draft.injectJournal === true, onChange: (injectJournal) => setDraft({ ...draft, injectJournal }) }), t2("chip.field.injectJournal")] }) })] })] }) : (0, import_jsx_runtime10.jsxs)("div", { className: "dsh-tavern-panelCard", children: [(0, import_jsx_runtime10.jsx)("div", { className: "dsh-tavern-groupHead", children: t2("chip.group.binding") }), (0, import_jsx_runtime10.jsx)(Field, { label: t2("chip.field.character"), children: (0, import_jsx_runtime10.jsx)(Select, { width: "100%", value: "", onChange: (cardId) => {
    if (!cardId)
      return;
    ...
    { value: "", label: t2("chip.field.selectCharacter") },
    ...lists.characters.map((c) => ({ value: c.cardId, label: c.name }))
  ] }) })] }), (0, import_jsx_runtime10.jsxs)("div", { className: "dsh-tavern-panelCard", children: [(0, import_jsx_runtime10.jsx)("div", { className: "dsh-tavern-groupHead", children: t2("chip.group.greetingDebug") }), (0, import_jsx_runtime10.jsxs)("div", { className: "dsh-tavern-bindingActions", children: [(0, import_jsx_runtime10.jsx)(Btn, { disabled: !binding, onClick: () => void insertGreeting(), children: t2("binding.greeting") }), (0, import_jsx_runtime10.jsx)(Btn, { disabled: !binding || !canSwipeGreeting, onClick: () => void swipeBy(-1), children: t2("chip.greeting.prev") }), (0, import_jsx_runtime10.jsx)(Btn, { disabled: !binding || !canSwipeGreeting, onClick: () => void swipeBy(1), children: t2("chip.greeting.next") }), (0, import_jsx_runtime10.jsx)(Btn, { onClick: () => void showTriggerLog(), children: t2("binding.triggerLog") }), (0, import_jsx_runtime10.jsx)(Btn, { onClick: () => void preview(), children: t2("binding.preview") })] })] }), usage && (0, import_jsx_runtime10.jsxs)("div", { className: "dsh-tavern-usageBar", children: [typeof usage.percent === "number" && (0, import_jsx_runtime10.jsx)("div", { className: "dsh-tavern-meter", role: "progressbar", "aria-label": t2("chip.usage.aria"), "aria-valuenow": Math.round(usage.percent), "aria-valuemin": 0, "aria-valuemax": 100, children: (0, import_jsx_runtime10.jsx)("span", { className: "dsh-tavern-meterFill", "data-warn": usage.percent >= 90 ? "true" : "false", style: { width: `${Math.min(100, Math.max(0, usage.percent))}%` } }) }), (0, import_jsx_runtime10.jsxs)(Muted, { children: [t2("chip.usage.label"), usage.contextWindow !== null && usage.pressureTokens !== null ? t2("chip.usage.full", { used: fmtTokens(usage.pressureTokens), window: fmtTokens(usage.contextWindow), percent: usage.percent ?? "?" }) : t2("chip.usage.approx", { tokens: fmtTokens(usage.surfaceTokens) }), usage.messageTokens !== null ? ` \xB7 ${t2("chip.usage.breakdown", { system: fmtTokens(usage.systemTokens), tools: fmtTokens(usage.toolsTokens), messages: fmtTokens(usage.messageTokens) })}` : ""] })] })] })] }) }), toast.node, view && (0, import_jsx_runtime10.jsx)(PreDialog, { title: view.title, text: view.text, onClose: () => setView(null) }), previewData && (0, import_jsx_runtime10.jsx)(PromptPreviewDialog, { data: previewData, onClose: () => setPreviewData(null) }), chatLore && (0, import_jsx_runtime10.jsx)(DraftScope, { children: (request) => (0, import_jsx_runtime10.jsx)(Dialog, { open: true, width: "xl", title: t2("chip.chatLore.title"), onClose: () => request(() => setChatLore(null)), children: (0, import_jsx_runtime10.jsx)(LorebookEditor, { remote, target: { kind: "chat", cardId: chatLore.cardId, storyId: chatLore.storyId, name: t2("chip.chatLore.title") }, entries: chatLore.entries, onClose: () => setChatLore(null), onSaved: () => {
    toast.show(t2("chip.chatLore.saved"));
    setChatLore(null);
  }, save: (json) => remote.saveChatLorebook({ cardId: chatLore.cardId, storyId: chatLore.storyId, json }) }) }) }), memoryOpen && binding?.storyId && (0, import_jsx_runtime10.jsx)(DraftScope, { children: (request) => (0, import_jsx_runtime10.jsx)(Dialog, { open: true, width: "xl", title: t2("section.memory"), onClose: () => request(() => setMemoryOpen(false)), children: (0, import_jsx_runtime10.jsx)(MemorySection, { remote, initialContext: { cardId: binding.cardId, storyId: binding.storyId } }, binding.storyId) }) }), (0, import_jsx_runtime10.jsx)(ConfirmDialog, { open: confirmUnbind, title: t2("chip.unbind.title"), description: t2("chip.unbind.desc"), confirmLabel: t2("chip.unbind.action"), danger: true, busy: unbindBusy, onCancel: () => {
    setConfirmUnbind(false);
...
function CardAvatar(props) {
  ...
  return (0, import_jsx_runtime11.jsx)(Avatar, { url, name: props.name, size: props.size });
}
function CharacterCard(props) {
  ...
  return (0, import_jsx_runtime11.jsxs)("article", { className: "dsh-tavern-charCard", ...clickableProps(() => props.onOpen(props.item.cardId)), children: [(0, import_jsx_runtime11.jsx)("div", { className: "dsh-tavern-charCardCover", children: url ? (0, import_jsx_runtime11.jsx)("img", { src: url, alt: "" }) : (0, import_jsx_runtime11.jsx)("span", { className: "dsh-tavern-charCardInitial", children: initial }) }), (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-charCardBar", children: [(0, import_jsx_runtime11.jsx)("div", { className: "dsh-tavern-charCardName", children: props.item.name }), meta2 ? (0, import_jsx_runtime11.jsx)("div", { className: "dsh-tavern-charCardMeta", children: meta2 }) : null] }), (0, import_jsx_runtime11.jsx)("div", { className: "dsh-tavern-charCardActions", children: (0, import_jsx_runtime11.jsx)(import_dsh_client_ui_primitives9.Tooltip, { label: t2("characters.card.delete"), side: "bottom", children: (0, import_jsx_runtime11.jsx)("button", { type: "button", "aria-label": t2("characters.card.delete"), className: "dsh-tavern-coverBtn is-danger", disabled: props.busy, onClick: (e) => {
    e.stopPropagation();
    props.onDelete(props.item);
  }, children: (0, import_jsx_runtime11.jsx)(import_dsh_client_ui_primitives9.IconTrashOutline16, {}) }) }) })] });
}
function LabeledArea(props) {
  return (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime11.jsx)("span", { className: "dsh-tavern-fieldLabel", children: props.label }), (0, import_jsx_runtime11.jsx)("textarea", { className: "dsh-tavern-input dsh-tavern-textarea", style: { minHeight: props.minHeight ?? 64 }, value: props.value, onChange: (e) => props.onChange(e.target.value) })] });
}
function CharacterDetailDialog(props) {
  ...
  const [cardOpen, setCardOpen] = (0, import_react11.useState)(false);
  ...
  const preserveRestored = (0, import_react11.useRef)(restored && draft !== null);
  ...
  return (0, import_jsx_runtime11.jsxs)(Dialog, { open: true, width: "xl", title: t2("characters.detail.title", { name: detail?.name ?? cardId }), onClose: close, footer: detail ? (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-ui dsh-tavern-footActions", children: [(0, import_jsx_runtime11.jsx)("span", { className: "dsh-tavern-muted", role: "status", children: dirty ? t2("draft.unsaved") : "" }), (0, import_jsx_runtime11.jsx)("span", { className: "dsh-tavern-footSpacer" }), (0, import_jsx_runtime11.jsx)(Btn, { size: "md", disabled: busy, onClick: close, children: t2("action.close") }), (0, import_jsx_runtime11.jsx)(Btn, { primary: true, size: "md", disabled: busy || !dirty, onClick: () => void save(), children: t2(busy ? "draft.saving" : "action.save") })] }) : void 0, children: [toast.node, guard.confirmation, state.status === "loading" && (0, import_jsx_runtime11.jsxs)("div", { style: { display: "flex", flexDirection: "column", gap: 8 }, children: [(0, import_jsx_runtime11.jsx)(Skeleton, { height: 48 }), (0, import_jsx_runtime11.jsx)(Skeleton, { height: 14, width: "60%" }), (0, import_jsx_runtime11.jsx)(Skeleton, { height: 90 })] }), state.status === "error" && (0, import_jsx_runtime11.jsx)(Err, { message: state.message }), detail && (0, import_jsx_runtime11.jsxs)("fieldset", { disabled: busy, className: "dsh-tavern-editorFields dsh-tavern-dialogStack", style: { fontSize: 13 }, children: [(0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-panelCard", style: { flexDirection: "row", alignItems: "center", gap: 14 }, children: [(0, import_jsx_runtime11.jsx)(CardAvatar, { remote, cardId, name: detail.name, size: 52 }), (0, import_jsx_runtime11.jsxs)("div", { style: { flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 8 }, children: [(0, import_jsx_runtime11.jsx)(Field, { label: t2("characters.detail.displayName"), children: (0, import_jsx_runtime11.jsx)("input", { className: "dsh-tavern-input", style: { width: "100%" }, value: detail.name, onChange: (e) => set({ name: e.target.value }) }) }), (0, import_jsx_runtime11.jsxs)(Muted, { children: [detail.spec, " \xB7 v", detail.characterVersion || "?", " \xB7 ", detail.creator || t2("characters.detail.unknownCreator"), detail.hasCharacterBook ? ` \xB7 ${detail.characterBookName ? t2("characters.card.embeddedBookNamed", { name: detail.characterBookName }) : t2("characters.card.embeddedBook")}` : ""] })] })] }), (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-panelCard", children: [(0, import_jsx_runtime11.jsx)("div", { className: "dsh-tavern-groupHead", children: t2("characters.detail.groupPersona") }), (0, import_jsx_runtime11.jsx)(LabeledArea, { label: t2("characters.detail.description"), minHeight: 88, value: detail.description, onChange: (v) => set({ description: v }) }), (0, import_jsx_runtime11.jsx)(LabeledArea, { label: t2("characters.detail.personality"), value: detail.personality, onChange: (v) => set({ personality: v }) }), (0, import_jsx_runtime11.jsx)(LabeledArea, { label: t2("characters.detail.scenario"), value: detail.scenario, onChange: (v) => set({ scenario: v }) })] }), (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-panelCard", children: [(0, import_jsx_runtime11.jsx)("div", { className: "dsh-tavern-groupHead", children: t2("characters.detail.groupGreetings") }), (0, import_jsx_runtime11.jsx)(LabeledArea, { label: t2("characters.detail.greeting"), minHeight: 88, value: detail.firstMes, onChange: (v) => set({ firstMes: v }) }), (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime11.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("characters.detail.altGreetings") }), detail.alternateGreetings.map((greeting, index) => (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-greetingEntry", children: [(0, import_jsx_runtime11.jsxs)("label", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime11.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("characters.detail.greetingNumber", { index: index + 1 }) }), (0, import_jsx_runtime11.jsx)("textarea", { className: "dsh-tavern-input dsh-tavern-textarea", value: greeting, disabled: busy, onChange: (e) => set({ alternateGreetings: detail.alternateGreetings.map((text, i) => i === index ? e.target.value : text) }) })] }), (0, import_jsx_runtime11.jsx)(Btn, { disabled: busy, onClick: () => set({ alternateGreetings: detail.alternateGreetings.filter((_, i) => i !== index) }), children: t2("action.delete") })] }, index)), (0, import_jsx_runtime11.jsx)(Btn, { disabled: busy, onClick: () => set({ alternateGreetings: [...detail.alternateGreetings, ""] }), children: t2("characters.detail.addGreeting") })] }), (0, import_jsx_runtime11.jsx)(LabeledArea, { label: t2("characters.detail.mesExample"), value: detail.mesExample, onChange: (v) => set({ mesExample: v }) })] }), (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-panelCard", children: [(0, import_jsx_runtime11.jsx)("div", { className: "dsh-tavern-groupHead", children: t2("characters.detail.groupAdvanced") }), (0, import_jsx_runtime11.jsx)(LabeledArea, { label: t2("characters.detail.systemPrompt"), value: detail.systemPrompt, onChange: (v) => set({ systemPrompt: v }) }), (0, import_jsx_runtime11.jsx)(LabeledArea, { label: t2("characters.detail.postHistory"), value: detail.postHistoryInstructions, onChange: (v) => set({ postHistoryInstructions: v }) }), (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime11.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("characters.detail.depthPrompt") }), (0, import_jsx_runtime11.jsx)("textarea", { className: "dsh-tavern-input dsh-tavern-textarea", style: { minHeight: 64 }, value: detail.depthPrompt?.prompt ?? "", onChange: (e) => set({
  ...
  }) }), detail.depthPrompt ? (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-fieldRow", style: { marginTop: 6 }, children: [(0, import_jsx_runtime11.jsx)(Field, { label: t2("characters.detail.depth"), children: (0, import_jsx_runtime11.jsx)(NumInput, { value: detail.depthPrompt.depth, onChange: (depth) => set({ depthPrompt: { ...detail.depthPrompt, depth: Math.max(0, Math.round(depth)) } }) }) }), (0, import_jsx_runtime11.jsx)(Field, { label: t2("characters.detail.role"), children: (0, import_jsx_runtime11.jsx)(Select, { value: detail.depthPrompt.role, onChange: (role) => set({ depthPrompt: { ...detail.depthPrompt, role } }), options: [
    { value: "system", label: "system" },
    { value: "user", label: "user" },
    { value: "assistant", label: "assistant" }
  ] }) })] }) : null] })] }), (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-panelCard", children: [(0, import_jsx_runtime11.jsx)("div", { className: "dsh-tavern-groupHead", children: t2("characters.detail.groupMetadata") }), (0, import_jsx_runtime11.jsx)(LabeledArea, { label: t2("characters.detail.creatorNotes"), value: detail.creatorNotes, onChange: (v) => set({ creatorNotes: v }) }), (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-fieldRow", children: [(0, import_jsx_runtime11.jsx)(Field, { label: t2("characters.detail.creator"), children: (0, import_jsx_runtime11.jsx)("input", { className: "dsh-tavern-input", value: detail.creator, onChange: (e) => set({ creator: e.target.value }) }) }), (0, import_jsx_runtime11.jsx)(Field, { label: t2("characters.detail.version"), children: (0, import_jsx_runtime11.jsx)("input", { className: "dsh-tavern-input", value: detail.characterVersion, onChange: (e) => set({ characterVersion: e.target.value }) }) })] }), (0, import_jsx_runtime11.jsx)(Field, { label: t2("characters.detail.tags"), children: (0, import_jsx_runtime11.jsx)("input", { className: "dsh-tavern-input", style: { width: "100%" }, value: detail.tags.join(", "), onChange: (e) => set({ tags: e.target.value.split(/[，,]/).map((s) => s.trim()).filter(Boolean) }) }) })] }), (0, import_jsx_runtime11.jsx)(Err, { message: error }), (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-footActions", style: { marginTop: 2 }, children: [(0, import_jsx_runtime11.jsx)(IconBtn, { label: t2("characters.detail.exportPng"), disabled: busy, onClick: () => void exportCard("png"), children: (0, import_jsx_runtime11.jsx)(import_dsh_client_ui_primitives9.IconDownloadOutline16, {}) }), interactiveHtml !== null && (0, import_jsx_runtime11.jsx)(Btn, { size: "md", onClick: () => setCardOpen(true), children: t2("interactive.open") }), (0, import_jsx_runtime11.jsx)("span", { className: "dsh-tavern-footSpacer" }), (0, import_jsx_runtime11.jsx)(Btn, { size: "md", disabled: busy, onClick: () => void exportCard("json"), children: t2("characters.detail.exportJson") })] })] }), cardOpen && interactiveHtml !== null && (0, import_jsx_runtime11.jsx)(Dialog, { open: true, width: "lg", title: t2("characters.detail.interactiveTitle", { name: detail?.name ?? "" }), onClose: () => setCardOpen(false), children: (0, import_jsx_runtime11.jsx)("iframe", { sandbox: "allow-scripts", srcDoc: withCsp(interactiveHtml), title: t2("characters.detail.interactiveFrame"), style: { width: "100%", height: "60vh", border: "1px solid var(--dsw-alias-border-l2, rgba(128,128,128,.25))", borderRadius: 16, background: "var(--dsw-alias-bg-base, #111)" } }) })] });
}
function CharactersSection(props) {
  return (0, import_jsx_runtime11.jsx)(PersistentEditor, { remote: props.remote, scope: "characters", children: (0, import_jsx_runtime11.jsx)(CharactersSectionContent, { ...props }) });
}
function CharactersSectionContent(props) {
  ...
  const [toDelete, setToDelete] = (0, import_react11.useState)(null);
  ...
  return (0, import_jsx_runtime11.jsxs)(Section, { title: t2("section.characters"), description: t2("characters.section.desc"), children: [toast.node, createGuard.confirmation, (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-toolbar", children: [(0, import_jsx_runtime11.jsx)(FileBtn, { accept: ".png,.json", disabled: busy, onFile: (file) => void onImportFile(file), children: t2("characters.importFile") }), (0, import_jsx_runtime11.jsx)(Btn, { size: "md", disabled: busy, onClick: () => setCreating(true), children: t2("characters.newCard") }), (0, import_jsx_runtime11.jsx)(Btn, { size: "md", onClick: reload, disabled: busy, children: t2("action.refresh") }), items.length >= 5 && (0, import_jsx_runtime11.jsx)(SearchInput, { label: t2("characters.searchLabel"), value: query, onChange: setQuery, placeholder: t2("characters.searchPlaceholder"), width: 220 })] }), state.status === "loading" && (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-charGrid", children: [(0, import_jsx_runtime11.jsx)(Skeleton, { height: 198, radius: 18 }), (0, import_jsx_runtime11.jsx)(Skeleton, { height: 198, radius: 18 }), (0, import_jsx_runtime11.jsx)(Skeleton, { height: 198, radius: 18 })] }), state.status === "error" && (0, import_jsx_runtime11.jsx)(Err, { message: state.message }), (0, import_jsx_runtime11.jsx)(Err, { message: error }), items.length === 0 && state.status === "ready" && (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-empty", children: [(0, import_jsx_runtime11.jsx)("div", { className: "dsh-tavern-emptyIcon", children: (0, import_jsx_runtime11.jsx)(import_dsh_client_ui_primitives9.IconUserOutline16, { size: 32 }) }), (0, import_jsx_runtime11.jsx)("div", { className: "dsh-tavern-emptyTitle", children: t2("hero.noCharacters") }), (0, import_jsx_runtime11.jsx)("div", { className: "dsh-tavern-emptyDesc", children: t2("characters.emptyDesc") })] }), q !== "" && filtered.length === 0 && state.status === "ready" && (0, import_jsx_runtime11.jsx)(SearchEmpty, { what: t2("characters.what"), query: query.trim(), onClear: () => setQuery("") }), (0, import_jsx_runtime11.jsx)("div", { className: "dsh-tavern-charGrid", children: filtered.map((item) => (0, import_jsx_runtime11.jsx)(CharacterCard, { remote, item, busy, onOpen: setDetailId, onDelete: setToDelete }, item.cardId)) }), detailId && (0, import_jsx_runtime11.jsx)(CharacterDetailDialog, { remote, cardId: detailId, onClose: () => setDetailId(null), onSaved: reload }, detailId), pending && (0, import_jsx_runtime11.jsx)(Dialog, { open: true, title: t2("characters.importBook.title"), description: pending.preview.characterBookName ? t2("characters.importBook.descNamed", { name: pending.preview.name, book: pending.preview.characterBookName, count: pending.preview.entryCount }) : t2("characters.importBook.desc", { name: pending.preview.name, count: pending.preview.entryCount }), onClose: () => setPending(null), footer: (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-modalActions", children: [(0, import_jsx_runtime11.jsx)(import_dsh_client_ui_primitives9.Button, { type: "button", variant: "outline", size: "md", disabled: busy, onClick: () => void doImport(pending.name, pending.dataBase64, false), children: t2("characters.importBook.skip") }), (0, import_jsx_runtime11.jsx)(import_dsh_client_ui_primitives9.Button, { type: "button", variant: "primary", size: "md", disabled: busy, onClick: () => void doImport(pending.name, pending.dataBase64, true), children: t2("characters.importBook.import") })] }), children: (0, import_jsx_runtime11.jsx)("p", { style: { margin: 0, fontSize: 13, lineHeight: "20px", color: "var(--dsw-alias-label-secondary)" }, children: t2("characters.importBook.skipNote") }) }), (0, import_jsx_runtime11.jsx)(ConfirmDialog, { open: toDelete !== null, title: t2("characters.delete.title"), description: toDelete ? t2("characters.delete.desc", { name: toDelete.name }) : "", confirmLabel: t2("action.delete"), danger: true, busy, onCancel: () => setToDelete(null), onConfirm: () => void onDelete() }), (0, import_jsx_runtime11.jsx)(Dialog, { open: creating, title: t2("characters.create.title"), description: t2("characters.create.desc"), onClose: closeCreate, footer: (0, import_jsx_runtime11.jsxs)("div", { className: "dsh-tavern-modalActions", children: [(0, import_jsx_runtime11.jsx)(Btn, { size: "md", disabled: busy, onClick: closeCreate, children: t2("action.cancel") }), (0, import_jsx_runtime11.jsx)(Btn, { primary: true, size: "md", disabled: busy || !newName.trim(), onClick: () => {
  ...
  }, children: t2("characters.create.confirm") })] }), children: (0, import_jsx_runtime11.jsx)("input", { className: "dsh-tavern-input", style: { width: "100%", height: 36, borderRadius: 8, padding: "0 10px", fontSize: 13, boxSizing: "border-box" }, disabled: busy, value: newName, placeholder: t2("characters.create.namePlaceholder"), onChange: (e) => setNewName(e.target.value) }) })] });
}

// lib/client/characterPicker.js
var recentByRemote = /* @__PURE__ */ new WeakMap();
function rememberCharacter(remote, cardId) {
...
function PickerItem(props) {
  ...
  return (0, import_jsx_runtime12.jsxs)("button", { type: "button", className: "dsh-tavern-pickerItem", "aria-pressed": props.selected, disabled: props.busy, onClick: props.onPick, children: [(0, import_jsx_runtime12.jsx)(Avatar, { name: item.name, url: avatar.state.status === "ready" ? avatar.state.value.dataUrl : null, size: 52 }), (0, import_jsx_runtime12.jsxs)("span", { className: "dsh-tavern-pickerText", children: [(0, import_jsx_runtime12.jsx)("strong", { children: item.name }), item.hasCharacterBook && (0, import_jsx_runtime12.jsx)("span", { children: item.characterBookName || t2("hero.picker.embeddedBook") }), props.recent && (0, import_jsx_runtime12.jsx)("span", { children: t2("hero.picker.recent") })] }), props.selected && (0, import_jsx_runtime12.jsx)(Badge, { accent: true, children: t2("hero.picker.selected") })] });
}
function CharacterPicker(props) {
  const t2 = useT();
  const [query, setQuery] = (0, import_react12.useState)("");
  const [limit, setLimit] = (0, import_react12.useState)(24);
  const [managing, setManaging] = (0, import_react12.useState)(false);
...
${c.characterBookName ?? ""}`.toLocaleLowerCase().includes(q)).sort((a, b) => {
    const rank = (id) => {
  ...
  return (0, import_jsx_runtime12.jsx)(DraftScope, { children: (request) => (0, import_jsx_runtime12.jsx)(Dialog, { open: true, width: "lg", title: t2(managing ? "section.characters" : "hero.pickCharacter"), onClose: () => {
    if (!props.busy)
      request(props.onClose);
  }, children: managing ? (0, import_jsx_runtime12.jsxs)(import_jsx_runtime12.Fragment, { children: [(0, import_jsx_runtime12.jsx)(Btn, { onClick: () => request(() => setManaging(false)), children: t2("hero.picker.back") }), (0, import_jsx_runtime12.jsx)(CharactersSection, { remote: props.remote })] }) : (0, import_jsx_runtime12.jsxs)("div", { className: "dsh-tavern-section", children: [(0, import_jsx_runtime12.jsxs)("div", { className: "dsh-tavern-toolbar", children: [(0, import_jsx_runtime12.jsx)(SearchInput, { label: t2("characters.searchLabel"), value: query, width: "100%", onChange: (value) => {
    setQuery(value);
    setLimit(24);
  } }), (0, import_jsx_runtime12.jsx)(Btn, { disabled: props.busy, onClick: () => setManaging(true), children: t2("hero.picker.manage") })] }), (0, import_jsx_runtime12.jsx)(Err, { message: props.error ?? (chars.state.status === "error" ? chars.state.message : null) }), chars.state.status === "error" && (0, import_jsx_runtime12.jsx)(Btn, { onClick: chars.reload, children: t2("action.retry") }), chars.state.status === "loading" && (0, import_jsx_runtime12.jsxs)(import_jsx_runtime12.Fragment, { children: [(0, import_jsx_runtime12.jsx)(Skeleton, { height: 84 }), (0, import_jsx_runtime12.jsx)(Skeleton, { height: 84 })] }), chars.state.status === "ready" && items.length === 0 && (0, import_jsx_runtime12.jsxs)("div", { className: "dsh-tavern-empty is-compact", children: [(0, import_jsx_runtime12.jsx)("div", { className: "dsh-tavern-emptyTitle", children: t2("hero.noCharacters") }), (0, import_jsx_runtime12.jsx)("div", { className: "dsh-tavern-emptyDesc", children: t2("hero.picker.empty") }), (0, import_jsx_runtime12.jsx)(Btn, { primary: true, onClick: () => setManaging(true), children: t2("hero.picker.manage") })] }), chars.state.status === "ready" && items.length > 0 && filtered.length === 0 && (0, import_jsx_runtime12.jsx)(SearchEmpty, { what: t2("characters.what"), query, onClear: () => setQuery("") }), (0, import_jsx_runtime12.jsx)("div", { className: "dsh-tavern-pickerList", "aria-busy": props.busy, children: filtered.slice(0, limit).map((item) => (0, import_jsx_runtime12.jsx)(PickerItem, { item, remote: props.remote, selected: props.selectedId === item.cardId, recent: recent.includes(item.cardId), busy: props.busy, onPick: () => props.onPick(item.cardId) }, item.cardId)) }), filtered.length > limit && (0, import_jsx_runtime12.jsx)(Btn, { onClick: () => setLimit(limit + 24), children: t2("hero.picker.more") })] }) }) });
}

// lib/core/template.js
var hasEjs = (text) => text.includes("<%");

// lib/core/persona.js
var DEFAULT_USER_NAME = "User";

// lib/client/hero.js
function TemplateGreetingPreview(props) {
  ...
  if (preview.state.status === "error")
    return (0, import_jsx_runtime13.jsx)(Err, { message: preview.state.message });
  if (preview.state.status !== "ready")
    return (0, import_jsx_runtime13.jsx)(Skeleton, {});
  return (0, import_jsx_runtime13.jsx)(import_jsx_runtime13.Fragment, { children: preview.state.value.text });
}
...
function TavernHeroCharacter(props) {
  return (0, import_jsx_runtime13.jsx)(HeroCharacterSession, { ...props }, props.sessionId);
}
function HeroCharacterSession(props) {
  ...
  const [entered, setEntered] = (0, import_react13.useState)(false);
  const alive = (0, import_react13.useRef)(true);
  ...
  const dockRef = (0, import_react13.useRef)(null);
  const [chipHost, setChipHost] = (0, import_react13.useState)(null);
  ...
  const picking = (0, import_react13.useRef)(false);
  const clearing = (0, import_react13.useRef)(false);
  const sweptSession = (0, import_react13.useRef)(null);
  (0, import_react13.useEffect)(() => {
  ...
  const startConversation = async () => {
    ...
    try {
      ...
      const entered2 = await remote.ensureGreeting({ sessionId });
      const enterErr = errOf(entered2);
      if (enterErr)
        setError(enterErr);
      else if (entered2.ok) {
        if (entered2.value.created || entered2.value.conversationStarted) {
          invalidateSessionBinding(sessionId);
          window.dispatchEvent(new CustomEvent(BINDING_CHANGED_EVENT, { detail: sessionId }));
          setEntered(true);
          try {
  ...
  const chip = (0, import_jsx_runtime13.jsx)("span", { "data-tavern-hero-seat": "", className: "dsh-tavern-ui", children: (0, import_jsx_runtime13.jsx)(TavernSeatChip, { label: chipLabel, title: t2("hero.pickCharacter"), avatarUrl: avatar, open, hasPopup: "dialog", loading: bindingLoader.state.status === "loading", disabled: busy, onClick: () => {
    setError(null);
    setOpen(true);
  } }) });
  return (0, import_jsx_runtime13.jsxs)("div", { ref: dockRef, "data-tavern-hero-root": "", className: "dsh-tavern-ui", children: [chipHost ? (0, import_react_dom.createPortal)(chip, chipHost) : chip, open && (0, import_jsx_runtime13.jsx)(CharacterPicker, { remote, selectedId: binding?.cardId, busy, error, onPick: (id) => void pickCharacter(id), onClose: () => setOpen(false) }), error && (0, import_jsx_runtime13.jsx)("div", { className: "dsh-tavern-hero-error", children: error }), bindingLoader.state.status === "error" && (0, import_jsx_runtime13.jsxs)("div", { role: "alert", className: "dsh-tavern-hero-error", children: [bindingLoader.state.message, (0, import_jsx_runtime13.jsx)(Btn, { onClick: bindingLoader.reload, disabled: busy, children: t2("action.retry") })] }), binding && detailLoader.state.status === "loading" && (0, import_jsx_runtime13.jsxs)("div", { className: "dsh-tavern-hero-preview", "aria-busy": "true", children: [(0, import_jsx_runtime13.jsxs)("div", { className: "dsh-tavern-hero-previewHead", children: [(0, import_jsx_runtime13.jsx)(Skeleton, { width: 28, height: 28, radius: 999 }), (0, import_jsx_runtime13.jsx)(Skeleton, { width: 140, height: 14 })] }), (0, import_jsx_runtime13.jsx)(Skeleton, { height: 14, style: { marginTop: 14 } }), (0, import_jsx_runtime13.jsx)(Skeleton, { height: 14, width: "72%", style: { marginTop: 8 } }), (0, import_jsx_runtime13.jsx)(Skeleton, { height: 14, width: "48%", style: { marginTop: 8 } }), (0, import_jsx_runtime13.jsx)(Skeleton, { height: 32, width: 104, radius: 18, style: { marginTop: 14 } })] }), binding && detailLoader.state.status !== "loading" && (0, import_jsx_runtime13.jsxs)("div", { className: "dsh-tavern-hero-preview dsh-tavern-rise", tabIndex: variants.length > 1 ? 0 : void 0, onKeyDown: (e) => {
  ...
  }, children: [(0, import_jsx_runtime13.jsxs)("div", { className: "dsh-tavern-hero-previewHead", children: [(0, import_jsx_runtime13.jsx)(Avatar, { url: avatar, name: chipLabel, size: 40, className: "dsh-tavern-speechAvatar" }), (0, import_jsx_runtime13.jsxs)("div", { className: "dsh-tavern-hero-previewHeadText", children: [(0, import_jsx_runtime13.jsx)("div", { className: "dsh-tavern-hero-previewName", children: chipLabel }), metaParts.length > 0 && (0, import_jsx_runtime13.jsx)("div", { className: "dsh-tavern-hero-previewMeta", children: metaParts.join(" \xB7 ") })] })] }), detailLoader.state.status === "error" ? (0, import_jsx_runtime13.jsx)("div", { className: "dsh-tavern-hero-previewText", children: t2("hero.detailLoadFailed") }) : greetingText ? (0, import_jsx_runtime13.jsx)("div", { className: "dsh-tavern-hero-quote", children: hasEjs(greetingText) ? (0, import_jsx_runtime13.jsx)(TemplateGreetingPreview, { remote, sessionId, text: greetingText }) : greetingText }) : hasAnyGreeting ? (0, import_jsx_runtime13.jsx)("div", { className: "dsh-tavern-hero-previewText", children: t2("hero.emptyVariantHint") }) : (0, import_jsx_runtime13.jsx)("div", { className: "dsh-tavern-hero-previewText", children: t2("hero.noGreetingHint") }), (0, import_jsx_runtime13.jsxs)("div", { className: "dsh-tavern-hero-actions", children: [(0, import_jsx_runtime13.jsx)(Btn, { primary: true, size: "md", disabled: busy || !greetingText.trim() || !detail, onClick: () => void startConversation(), children: t2("hero.start") }), variants.length > 1 && (0, import_jsx_runtime13.jsxs)("div", { className: "dsh-tavern-hero-swipe", children: [(0, import_jsx_runtime13.jsx)("button", { type: "button", className: "dsh-tavern-hero-swipeBtn", disabled: busy, title: t2("hero.prevGreeting"), onClick: () => void swipe(-1), children: (0, import_jsx_runtime13.jsx)(import_dsh_client_ui_primitives10.IconChevronLeftOutline14, {}) }), (0, import_jsx_runtime13.jsxs)("span", { className: "dsh-tavern-hero-swipeIdx", children: [greetingIndex + 1, "/", variants.length] }), (0, import_jsx_runtime13.jsx)("button", { type: "button", className: "dsh-tavern-hero-swipeBtn", disabled: busy, title: t2("hero.nextGreeting"), onClick: () => void swipe(1), children: (0, import_jsx_runtime13.jsx)(import_dsh_client_ui_primitives10.IconChevronRightOutline14, {}) }), (0, import_jsx_runtime13.jsx)("span", { className: "dsh-tavern-hero-swipeHint", children: t2("hero.swipeHint") })] })] })] })] });
}
...
function LorebooksSection(props) {
  return (0, import_jsx_runtime14.jsx)(PersistentEditor, { remote: props.remote, scope: "lorebooks", children: (0, import_jsx_runtime14.jsx)(LorebooksSectionContent, { ...props }) });
}
function LorebooksSectionContent(props) {
  ...
  const [opening, setOpening] = (0, import_react14.useState)(false);
  const [opened, setOpened] = useDraftState("lorebooks:opened", null);
  const [toDelete, setToDelete] = (0, import_react14.useState)(null);
  const [toDeleteEmbedded, setToDeleteEmbedded] = (0, import_react14.useState)(null);
  ...
  if (opened) {
    return (0, import_jsx_runtime14.jsxs)(Section, { title: t2("section.lorebooks"), description: t2("lorebooks.editorDesc"), children: [toast.node, (0, import_jsx_runtime14.jsx)(LorebookEditor, { remote, target: opened.target, entries: opened.entries, onClose: () => setOpened(null), onSaved: () => {
  ...
  return (0, import_jsx_runtime14.jsxs)(Section, { title: t2("section.lorebooks"), description: t2("lorebooks.listDesc"), children: [toast.node, createGuard.confirmation, (0, import_jsx_runtime14.jsxs)("div", { className: "dsh-tavern-toolbar", children: [(0, import_jsx_runtime14.jsx)(FileBtn, { accept: ".json", disabled: busy, onFile: (file) => void onImportFile(file), children: t2("lorebooks.importJson") }), (0, import_jsx_runtime14.jsx)(Btn, { size: "md", disabled: busy, onClick: () => setCreating(true), children: t2("lorebooks.newEmpty") }), (0, import_jsx_runtime14.jsx)(Btn, { size: "md", onClick: () => {
    reload();
    chars.reload();
  }, disabled: busy, children: t2("action.refresh") }), totalBooks >= 5 && (0, import_jsx_runtime14.jsx)(SearchInput, { label: t2("lorebooks.searchLabel"), value: query, onChange: setQuery, placeholder: t2("lorebooks.searchPlaceholder"), width: 220 })] }), (state.status === "loading" || opening) && (0, import_jsx_runtime14.jsxs)("div", { className: "dsh-tavern-list", children: [(0, import_jsx_runtime14.jsx)(Skeleton, { height: 70, radius: 16 }), (0, import_jsx_runtime14.jsx)(Skeleton, { height: 70, radius: 16 }), (0, import_jsx_runtime14.jsx)(Skeleton, { height: 70, radius: 16 })] }), state.status === "error" && (0, import_jsx_runtime14.jsx)(Err, { message: state.message }), (0, import_jsx_runtime14.jsx)(Err, { message: error }), q !== "" && shownCharBooks.length === 0 && shownNames.length === 0 && state.status === "ready" && chars.state.status === "ready" && (0, import_jsx_runtime14.jsx)(SearchEmpty, { what: t2("section.lorebooks"), query: query.trim(), onClear: () => setQuery("") }), shownCharBooks.length > 0 && (0, import_jsx_runtime14.jsxs)(import_jsx_runtime14.Fragment, { children: [(0, import_jsx_runtime14.jsx)("div", { className: "dsh-tavern-groupHead", children: t2("lorebooks.groupEmbedded") }), (0, import_jsx_runtime14.jsx)("div", { className: "dsh-tavern-list", children: shownCharBooks.map((item) => (0, import_jsx_runtime14.jsxs)("div", { className: "dsh-tavern-tile", ...clickableProps(() => void openCharacter(item)), children: [(0, import_jsx_runtime14.jsx)("span", { className: "dsh-tavern-tileIcon", children: (0, import_jsx_runtime14.jsx)(import_dsh_client_ui_primitives11.IconFolderOpenOutline16, { size: 18 }) }), (0, import_jsx_runtime14.jsxs)("div", { className: "dsh-tavern-tileMain", children: [(0, import_jsx_runtime14.jsxs)("div", { className: "dsh-tavern-tileTitleRow", children: [(0, import_jsx_runtime14.jsx)("span", { className: "dsh-tavern-tileName", children: item.characterBookName || item.name }), (0, import_jsx_runtime14.jsx)(Badge, { children: t2("lorebooks.badgeEmbedded") })] }), (0, import_jsx_runtime14.jsx)("span", { className: "dsh-tavern-tileSub", children: typeof item.characterBookEntryCount === "number" && item.characterBookEntryCount > 0 ? t2("lorebooks.fromCharacterWithCount", { name: item.name, count: item.characterBookEntryCount }) : t2("lorebooks.fromCharacter", { name: item.name }) })] }), (0, import_jsx_runtime14.jsxs)("div", { className: "dsh-tavern-tileActions", children: [(0, import_jsx_runtime14.jsx)(IconBtn, { label: t2("lorebooks.editEntries"), onClick: () => void openCharacter(item), children: (0, import_jsx_runtime14.jsx)(import_dsh_client_ui_primitives11.IconEditOutline16, {}) }), (0, import_jsx_runtime14.jsx)(IconBtn, { label: t2("lorebooks.deleteEmbedded"), danger: true, onClick: () => setToDeleteEmbedded(item), children: (0, import_jsx_runtime14.jsx)(import_dsh_client_ui_primitives11.IconTrashOutline16, {}) })] })] }, item.cardId)) })] }), (0, import_jsx_runtime14.jsx)("div", { className: "dsh-tavern-groupHead", children: t2("lorebooks.groupLibrary") }), shownNames.length === 0 && state.status === "ready" ? q === "" ? (0, import_jsx_runtime14.jsxs)("div", { className: "dsh-tavern-empty", children: [(0, import_jsx_runtime14.jsx)("div", { className: "dsh-tavern-emptyIcon", children: (0, import_jsx_runtime14.jsx)(import_dsh_client_ui_primitives11.IconFolderOpenOutline16, { size: 32 }) }), (0, import_jsx_runtime14.jsx)("div", { className: "dsh-tavern-emptyTitle", children: t2("lorebooks.emptyTitle") }), (0, import_jsx_runtime14.jsx)("div", { className: "dsh-tavern-emptyDesc", children: charBooks.length > 0 ? t2("lorebooks.emptyDescEmbeddedAbove") : t2("lorebooks.emptyDesc") })] }) : null : (0, import_jsx_runtime14.jsx)("div", { className: "dsh-tavern-list", children: shownNames.map((name2) => (0, import_jsx_runtime14.jsxs)("div", { className: "dsh-tavern-tile", ...clickableProps(() => void openLibrary(name2)), children: [(0, import_jsx_runtime14.jsx)("span", { className: "dsh-tavern-tileIcon", children: (0, import_jsx_runtime14.jsx)(import_dsh_client_ui_primitives11.IconFolderOpenOutline16, { size: 18 }) }), (0, import_jsx_runtime14.jsxs)("div", { className: "dsh-tavern-tileMain", children: [(0, import_jsx_runtime14.jsxs)("div", { className: "dsh-tavern-tileTitleRow", children: [(0, import_jsx_runtime14.jsx)("span", { className: "dsh-tavern-tileName", children: name2 }), (0, import_jsx_runtime14.jsx)(Badge, { children: t2("lorebooks.badgeLibrary") })] }), (0, import_jsx_runtime14.jsx)("span", { className: "dsh-tavern-tileSub", children: t2("lorebooks.libraryFileSub") })] }), (0, import_jsx_runtime14.jsxs)("div", { className: "dsh-tavern-tileActions", children: [(0, import_jsx_runtime14.jsx)(IconBtn, { label: t2("lorebooks.editEntries"), onClick: () => void openLibrary(name2), children: (0, import_jsx_runtime14.jsx)(import_dsh_client_ui_primitives11.IconEditOutline16, {}) }), (0, import_jsx_runtime14.jsx)(IconBtn, { label: t2("lorebooks.exportJson"), disabled: busy, onClick: () => void exportBook(name2), children: (0, import_jsx_runtime14.jsx)(import_dsh_client_ui_primitives11.IconDownloadOutline16, {}) }), (0, import_jsx_runtime14.jsx)(IconBtn, { label: t2("lorebooks.deleteBook"), danger: true, onClick: () => setToDelete(name2), children: (0, import_jsx_runtime14.jsx)(import_dsh_client_ui_primitives11.IconTrashOutline16, {}) })] })] }, name2)) }), (0, import_jsx_runtime14.jsx)(ConfirmDialog, { open: toDelete !== null, title: t2("lorebooks.confirmDeleteTitle"), description: toDelete ? t2("lorebooks.confirmDeleteDesc", { name: toDelete }) : "", confirmLabel: t2("action.delete"), danger: true, busy, onCancel: () => setToDelete(null), onConfirm: () => void remove() }), (0, import_jsx_runtime14.jsx)(ConfirmDialog, { open: toDeleteEmbedded !== null, title: t2("lorebooks.confirmDeleteEmbeddedTitle"), description: toDeleteEmbedded ? t2("lorebooks.confirmDeleteEmbeddedDesc", {
  ...
  }) : "", confirmLabel: t2("action.delete"), danger: true, busy, onCancel: () => setToDeleteEmbedded(null), onConfirm: () => void removeEmbedded() }), (0, import_jsx_runtime14.jsx)(Dialog, { open: creating, title: t2("lorebooks.createTitle"), description: t2("lorebooks.createDesc"), onClose: closeCreate, footer: (0, import_jsx_runtime14.jsxs)("div", { className: "dsh-tavern-modalActions", children: [(0, import_jsx_runtime14.jsx)(Btn, { size: "md", disabled: busy, onClick: closeCreate, children: t2("action.cancel") }), (0, import_jsx_runtime14.jsx)(Btn, { primary: true, size: "md", disabled: busy || !newName.trim(), onClick: () => void createEmpty(), children: t2("lorebooks.create") })] }), children: (0, import_jsx_runtime14.jsx)("input", { className: "dsh-tavern-input", style: { width: "100%", height: 36, borderRadius: 8, padding: "0 10px", fontSize: 13, boxSizing: "border-box" }, value: newName, disabled: busy, placeholder: t2("lorebooks.namePlaceholder"), onChange: (e) => setNewName(e.target.value) }) })] });
}
...
function PersonasSection(props) {
  ...
  const [toDelete, setToDelete] = (0, import_react15.useState)(null);
  ...
  const openPersona = (p) => guard.request(() => {
    setEditing({ ...p });
  ...
  return (0, import_jsx_runtime15.jsxs)(Section, { title: t2("section.personas"), description: t2("personas.sectionDesc"), children: [toast.node, guard.confirmation, (0, import_jsx_runtime15.jsxs)("div", { className: "dsh-tavern-toolbar", children: [(0, import_jsx_runtime15.jsx)(Btn, { size: "md", onClick: () => guard.request(createNew), children: t2("personas.new") }), (0, import_jsx_runtime15.jsx)(Btn, { size: "md", onClick: reload, disabled: busy, children: t2("action.refresh") }), items.length >= 5 && (0, import_jsx_runtime15.jsx)(SearchInput, { label: t2("personas.searchLabel"), value: query, onChange: setQuery, placeholder: t2("personas.searchPlaceholder"), width: 220 })] }), state.status === "loading" && (0, import_jsx_runtime15.jsxs)("div", { className: "dsh-tavern-list", children: [(0, import_jsx_runtime15.jsx)(Skeleton, { height: 70, radius: 16 }), (0, import_jsx_runtime15.jsx)(Skeleton, { height: 70, radius: 16 }), (0, import_jsx_runtime15.jsx)(Skeleton, { height: 70, radius: 16 })] }), state.status === "error" && (0, import_jsx_runtime15.jsx)(Err, { message: state.message }), (0, import_jsx_runtime15.jsx)(Err, { message: error }), items.length === 0 && state.status === "ready" && (0, import_jsx_runtime15.jsxs)("div", { className: "dsh-tavern-empty", children: [(0, import_jsx_runtime15.jsx)("div", { className: "dsh-tavern-emptyIcon", children: (0, import_jsx_runtime15.jsx)(import_dsh_client_ui_primitives12.IconUserOutline16, { size: 32 }) }), (0, import_jsx_runtime15.jsx)("div", { className: "dsh-tavern-emptyTitle", children: t2("personas.emptyTitle") }), (0, import_jsx_runtime15.jsx)("div", { className: "dsh-tavern-emptyDesc", children: t2("personas.emptyDesc") })] }), q !== "" && filtered.length === 0 && state.status === "ready" && (0, import_jsx_runtime15.jsx)(SearchEmpty, { what: t2("personas.entity"), query: query.trim(), onClear: () => setQuery("") }), (0, import_jsx_runtime15.jsx)("div", { className: "dsh-tavern-list", style: { marginBottom: 12 }, children: filtered.map((p) => (0, import_jsx_runtime15.jsxs)("div", { className: "dsh-tavern-tile", ...clickableProps(() => openPersona(p)), children: [(0, import_jsx_runtime15.jsxs)("div", { className: "dsh-tavern-tileMain", children: [(0, import_jsx_runtime15.jsxs)("div", { className: "dsh-tavern-tileTitleRow", children: [(0, import_jsx_runtime15.jsx)("span", { className: "dsh-tavern-tileName", children: p.name }), p.lorebookId ? (0, import_jsx_runtime15.jsx)(Badge, { children: p.lorebookId }) : null] }), (0, import_jsx_runtime15.jsx)("span", { className: "dsh-tavern-tileSub", children: p.description.trim() || t2("personas.noDescription") })] }), (0, import_jsx_runtime15.jsxs)("div", { className: "dsh-tavern-tileActions", children: [(0, import_jsx_runtime15.jsx)(IconBtn, { label: t2("action.edit"), onClick: () => openPersona(p), children: (0, import_jsx_runtime15.jsx)(import_dsh_client_ui_primitives12.IconEditOutline16, {}) }), (0, import_jsx_runtime15.jsx)(Btn, { size: "sm", disabled: busy, onClick: () => void setAsDefault(p.id), children: t2("personas.setDefault") }), (0, import_jsx_runtime15.jsx)(IconBtn, { label: t2("personas.delete"), danger: true, onClick: () => setToDelete(p), children: (0, import_jsx_runtime15.jsx)(import_dsh_client_ui_primitives12.IconTrashOutline16, {}) })] })] }, p.id)) }), (0, import_jsx_runtime15.jsx)(ConfirmDialog, { open: toDelete !== null, title: t2("personas.deleteTitle"), description: toDelete ? t2("personas.deleteDesc", { name: toDelete.name }) : "", confirmLabel: t2("action.delete"), danger: true, busy, onCancel: () => setToDelete(null), onConfirm: () => void remove() }), editing && (0, import_jsx_runtime15.jsxs)("fieldset", { disabled: busy, className: "dsh-tavern-editorFields dsh-tavern-card", style: { marginBottom: 12 }, children: [(0, import_jsx_runtime15.jsx)(Field, { label: t2("personas.field.name"), children: (0, import_jsx_runtime15.jsx)("input", { className: "dsh-tavern-input", style: { flex: 1 }, value: editing.name, onChange: (e) => setEditing({ ...editing, name: e.target.value }) }) }), (0, import_jsx_runtime15.jsxs)("div", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime15.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("personas.field.description") }), (0, import_jsx_runtime15.jsx)("textarea", { className: "dsh-tavern-input dsh-tavern-textarea", value: editing.description, onChange: (e) => setEditing({ ...editing, description: e.target.value }) })] }), (0, import_jsx_runtime15.jsx)(Field, { label: t2("personas.field.lorebook"), children: (0, import_jsx_runtime15.jsx)(Select, { width: "100%", value: editing.lorebookId ?? "", onChange: (v) => setEditing({ ...editing, lorebookId: v || null }), options: [
    { value: "", label: t2("personas.lorebookNone") },
    ...lore.state.status === "ready" ? lore.state.value.items.map((n) => ({ value: n, label: n })) : []
  ] }) }), (0, import_jsx_runtime15.jsxs)(SaveBar, { children: [(0, import_jsx_runtime15.jsx)(Btn, { disabled: busy, onClick: () => void save(), primary: true, children: t2("action.save") }), (0, import_jsx_runtime15.jsx)(Btn, { onClick: () => guard.request(() => setEditing(null)), children: t2("action.close") })] })] })] });
}
...
function PresetRegexList(props) {
  const t2 = useT();
  return (0, import_jsx_runtime16.jsxs)("div", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime16.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("presets.regexList.label", { count: props.scripts.length }) }), (0, import_jsx_runtime16.jsx)("div", { className: "dsh-tavern-list", children: props.scripts.map((s, i) => (0, import_jsx_runtime16.jsx)(RegexScriptRow, { script: s, index: i, onToggle: (disabled) => props.onChange(props.scripts.map((x, j) => j === i ? { ...x, disabled } : x)) }, s.id ?? i)) })] });
}
function EntryEditor(props) {
  const { entry } = props;
  const t2 = useT();
  const set = (patch) => props.onChange({ ...entry, ...patch });
  return (0, import_jsx_runtime16.jsxs)("div", { className: "dsh-tavern-entry", children: [(0, import_jsx_runtime16.jsxs)("div", { style: { display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", padding: "12px 16px 8px" }, children: [(0, import_jsx_runtime16.jsx)(Toggle, { checked: entry.enabled, onChange: (enabled) => set({ enabled }), title: entry.enabled ? t2("presets.entry.disable") : t2("presets.entry.enable") }), (0, import_jsx_runtime16.jsx)("input", { className: "dsh-tavern-input", style: { width: 160 }, value: entry.name, placeholder: t2("presets.name"), onChange: (e) => set({ name: e.target.value }) }), (0, import_jsx_runtime16.jsx)(Select, { value: entry.role, onChange: (v) => set({ role: v }), options: [
    { value: "system", label: "system" },
    { value: "user", label: "user" },
    { value: "assistant", label: "assistant" }
  ] }), (0, import_jsx_runtime16.jsx)(Select, { value: entry.position, onChange: (v) => set({ position: v }), options: [
    { value: "relative", label: "relative" },
    { value: "in-chat", label: "in-chat" }
  ] }), entry.position === "in-chat" && (0, import_jsx_runtime16.jsxs)("label", { style: { fontSize: 12 }, children: [t2("presets.entry.depth"), " ", (0, import_jsx_runtime16.jsx)(NumInput, { value: entry.depth, width: 64, onChange: (v) => set({ depth: Math.max(0, Math.round(v)) }) })] }), (0, import_jsx_runtime16.jsxs)("label", { style: { fontSize: 12 }, children: [t2("presets.entry.order"), " ", (0, import_jsx_runtime16.jsx)(NumInput, { value: entry.order, width: 64, onChange: (v) => set({ order: Math.round(v) }) })] }), (0, import_jsx_runtime16.jsxs)("label", { style: { fontSize: 12 }, children: [(0, import_jsx_runtime16.jsx)("input", { type: "checkbox", checked: entry.marker, onChange: (e) => set({ marker: e.target.checked }) }), " ", t2("presets.entry.marker")] }), entry.marker && (0, import_jsx_runtime16.jsx)("input", { className: "dsh-tavern-input", style: { width: 140 }, value: entry.markerId ?? "", placeholder: "markerId", onChange: (e) => set({ markerId: e.target.value }) }), (0, import_jsx_runtime16.jsx)("span", { style: { flex: 1 } }), (0, import_jsx_runtime16.jsx)(IconBtn, { label: t2("presets.entry.delete"), danger: true, onClick: props.onDelete, children: (0, import_jsx_runtime16.jsx)(import_dsh_client_ui_primitives13.IconTrashOutline16, {}) })] }), !entry.marker && (0, import_jsx_runtime16.jsx)("div", { style: { padding: "2px 16px 14px" }, children: (0, import_jsx_runtime16.jsx)("textarea", { className: "dsh-tavern-input dsh-tavern-textarea", style: { minHeight: 60 }, value: entry.content, placeholder: t2("presets.entry.content"), onChange: (e) => set({ content: e.target.value }) }) })] });
}
function PresetsSection(props) {
  ...
  const [toDelete, setToDelete] = (0, import_react16.useState)(null);
  ...
  return (0, import_jsx_runtime16.jsxs)(Section, { title: t2("section.presets"), description: t2("presets.section.desc"), children: [toast.node, guard.confirmation, (0, import_jsx_runtime16.jsxs)("div", { className: "dsh-tavern-toolbar", children: [(0, import_jsx_runtime16.jsx)(FileBtn, { accept: ".json", disabled: busy, onFile: (file) => void onImportFile(file), children: t2("presets.importFile") }), (0, import_jsx_runtime16.jsx)(Btn, { size: "md", onClick: () => guard.request(createNew), children: t2("presets.new") }), (0, import_jsx_runtime16.jsx)(Btn, { size: "md", onClick: reload, disabled: busy, children: t2("action.refresh") }), items.length >= 5 && (0, import_jsx_runtime16.jsx)(SearchInput, { label: t2("presets.searchLabel"), value: query, onChange: setQuery, placeholder: t2("presets.searchPlaceholder"), width: 220 })] }), state.status === "loading" && (0, import_jsx_runtime16.jsxs)("div", { className: "dsh-tavern-list", children: [(0, import_jsx_runtime16.jsx)(Skeleton, { height: 70, radius: 16 }), (0, import_jsx_runtime16.jsx)(Skeleton, { height: 70, radius: 16 }), (0, import_jsx_runtime16.jsx)(Skeleton, { height: 70, radius: 16 })] }), state.status === "error" && (0, import_jsx_runtime16.jsx)(Err, { message: state.message }), (0, import_jsx_runtime16.jsx)(Err, { message: error }), items.length === 0 && state.status === "ready" && (0, import_jsx_runtime16.jsxs)("div", { className: "dsh-tavern-empty", children: [(0, import_jsx_runtime16.jsx)("div", { className: "dsh-tavern-emptyIcon", children: (0, import_jsx_runtime16.jsx)(import_dsh_client_ui_primitives13.IconFolderOpenOutline16, { size: 32 }) }), (0, import_jsx_runtime16.jsx)("div", { className: "dsh-tavern-emptyTitle", children: t2("presets.empty") }), (0, import_jsx_runtime16.jsx)("div", { className: "dsh-tavern-emptyDesc", children: t2("presets.emptyDesc") })] }), q !== "" && filtered.length === 0 && state.status === "ready" && (0, import_jsx_runtime16.jsx)(SearchEmpty, { what: t2("presets.noun"), query: query.trim(), onClear: () => setQuery("") }), (0, import_jsx_runtime16.jsx)("div", { className: "dsh-tavern-list", style: { marginBottom: 12 }, children: filtered.map((item) => (0, import_jsx_runtime16.jsxs)("div", { className: "dsh-tavern-tile", ...clickableProps(() => guard.request(() => void open(item.id))), children: [(0, import_jsx_runtime16.jsx)("span", { className: "dsh-tavern-tileIcon", children: (0, import_jsx_runtime16.jsx)(import_dsh_client_ui_primitives13.IconListPenOutline16, { size: 18 }) }), (0, import_jsx_runtime16.jsxs)("div", { className: "dsh-tavern-tileMain", children: [(0, import_jsx_runtime16.jsxs)("div", { className: "dsh-tavern-tileTitleRow", children: [(0, import_jsx_runtime16.jsx)("span", { className: "dsh-tavern-tileName", children: item.name }), item.regexCount > 0 ? (0, import_jsx_runtime16.jsx)(Badge, { children: t2("presets.regexCount", { count: item.regexCount }) }) : null] }), (0, import_jsx_runtime16.jsx)("span", { className: "dsh-tavern-tileSub", children: item.id })] }), (0, import_jsx_runtime16.jsxs)("div", { className: "dsh-tavern-tileActions", children: [(0, import_jsx_runtime16.jsx)(IconBtn, { label: t2("action.edit"), onClick: () => guard.request(() => void open(item.id)), children: (0, import_jsx_runtime16.jsx)(import_dsh_client_ui_primitives13.IconEditOutline16, {}) }), (0, import_jsx_runtime16.jsx)(IconBtn, { label: t2("presets.export"), disabled: busy, onClick: () => void exportPreset(item.id, item.name), children: (0, import_jsx_runtime16.jsx)(import_dsh_client_ui_primitives13.IconDownloadOutline16, {}) }), (0, import_jsx_runtime16.jsx)(Btn, { size: "sm", disabled: busy, onClick: () => void setAsDefault(item.id), children: t2("presets.setAsDefault") }), (0, import_jsx_runtime16.jsx)(IconBtn, { label: t2("presets.delete"), danger: true, onClick: () => setToDelete(item.id), children: (0, import_jsx_runtime16.jsx)(import_dsh_client_ui_primitives13.IconTrashOutline16, {}) })] })] }, item.id)) }), (0, import_jsx_runtime16.jsx)(ConfirmDialog, { open: toDelete !== null, title: t2("presets.deleteTitle"), description: toDelete ? t2("presets.deleteDesc", { id: toDelete }) : "", confirmLabel: t2("action.delete"), danger: true, busy, onCancel: () => setToDelete(null), onConfirm: () => void remove() }), editing && (0, import_jsx_runtime16.jsxs)("fieldset", { disabled: busy, className: "dsh-tavern-editorFields dsh-tavern-card", style: { marginBottom: 12 }, children: [(0, import_jsx_runtime16.jsx)(Field, { label: t2("presets.name"), children: (0, import_jsx_runtime16.jsx)("input", { className: "dsh-tavern-input", style: { flex: 1 }, value: editing.name, onChange: (e) => setEditing({ ...editing, name: e.target.value }) }) }), (0, import_jsx_runtime16.jsx)(Field, { label: t2("presets.identifier"), children: (0, import_jsx_runtime16.jsx)(Muted, { children: editing.identifier }) }), (editing.regexScripts?.length ?? 0) > 0 && (0, import_jsx_runtime16.jsx)(PresetRegexList, { scripts: editing.regexScripts, onChange: (scripts) => setEditing({ ...editing, regexScripts: scripts }) }), (0, import_jsx_runtime16.jsx)("div", { className: "dsh-tavern-list", style: { margin: "8px 0" }, children: editing.entries.map((entry, i) => (0, import_jsx_runtime16.jsx)(EntryEditor, { entry, onChange: (e2) => setEntry(i, e2), onDelete: () => setEditing({ ...editing, entries: editing.entries.filter((_, j) => j !== i) }) }, entry.identifier)) }), (0, import_jsx_runtime16.jsxs)(SaveBar, { children: [(0, import_jsx_runtime16.jsx)("span", { className: "dsh-tavern-muted", children: JSON.stringify(editing) !== baseline ? t2("draft.unsaved") : "" }), (0, import_jsx_runtime16.jsx)(Btn, { onClick: () => setEditing({ ...editing, entries: [...editing.entries, newEntry2(editing.entries.length * 100 + 100)] }), children: t2("presets.addEntry") }), (0, import_jsx_runtime16.jsx)(Btn, { disabled: busy, onClick: () => void save(), primary: true, children: t2("presets.save") }), (0, import_jsx_runtime16.jsx)(Btn, { onClick: () => guard.request(() => setEditing(null)), children: t2("action.close") })] })] })] });
}
...
function RuleEditor(props) {
  ...
  return (0, import_jsx_runtime17.jsxs)("div", { className: "dsh-tavern-entry", style: { marginBottom: 8 }, children: [(0, import_jsx_runtime17.jsxs)("div", { style: { display: "flex", gap: 10, alignItems: "center", padding: "12px 16px 8px" }, children: [(0, import_jsx_runtime17.jsx)(Toggle, { checked: rule.enabled, onChange: (enabled) => set({ enabled }), title: rule.enabled ? t2("regex.toggleDisable") : t2("regex.toggleEnable") }), (0, import_jsx_runtime17.jsx)("input", { className: "dsh-tavern-input", style: { flex: 1 }, value: rule.name, onChange: (e) => set({ name: e.target.value }) }), (0, import_jsx_runtime17.jsx)(Badge, { children: t2(SOURCE_LABEL_KEY[rule.source]) }), (0, import_jsx_runtime17.jsx)(IconBtn, { label: t2("regex.deleteRule"), danger: true, onClick: props.onDelete, children: (0, import_jsx_runtime17.jsx)(import_dsh_client_ui_primitives14.IconTrashOutline16, {}) })] }), (0, import_jsx_runtime17.jsxs)("div", { style: { padding: "2px 16px 14px" }, children: [(0, import_jsx_runtime17.jsx)(Field, { label: t2("regex.find"), children: (0, import_jsx_runtime17.jsx)("input", { className: "dsh-tavern-input dsh-tavern-codeFont", style: { flex: 1 }, value: rule.find, onChange: (e) => set({ find: e.target.value }) }) }), (0, import_jsx_runtime17.jsx)("div", { className: "dsh-tavern-fieldLabel", style: { margin: "4px 0" }, children: t2("regex.replace") }), (0, import_jsx_runtime17.jsx)("textarea", { className: "dsh-tavern-input dsh-tavern-textarea dsh-tavern-codeFont", style: { minHeight: 40 }, value: rule.replace, onChange: (e) => set({ replace: e.target.value }) }), (0, import_jsx_runtime17.jsxs)("div", { className: "dsh-tavern-fieldRow", style: { margin: "8px 0 4px" }, children: [(0, import_jsx_runtime17.jsxs)("div", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime17.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("regex.scope") }), (0, import_jsx_runtime17.jsx)(CheckChips, { ariaLabel: t2("regex.scope"), options: SCOPES.map((o) => ({ value: o.value, label: t2(o.labelKey) })), selected: rule.scopes, onChange: (scopes) => set({ scopes }) })] }), (0, import_jsx_runtime17.jsxs)("div", { className: "dsh-tavern-field", children: [(0, import_jsx_runtime17.jsx)("span", { className: "dsh-tavern-fieldLabel", children: t2("regex.timing") }), (0, import_jsx_runtime17.jsx)(CheckChips, { ariaLabel: t2("regex.timing"), options: TIMINGS.map((o) => ({ value: o.value, label: t2(o.labelKey) })), selected: rule.timing, onChange: (timing) => set({ timing }) })] })] }), (0, import_jsx_runtime17.jsxs)("div", { style: { display: "flex", gap: 14, flexWrap: "wrap", fontSize: 12, alignItems: "center" }, children: [(0, import_jsx_runtime17.jsxs)("label", { children: [t2("regex.minDepth"), " ", (0, import_jsx_runtime17.jsx)(NullableNumInput, { value: rule.minDepth, width: 64, onChange: (v) => set({ minDepth: v }) })] }), (0, import_jsx_runtime17.jsxs)("label", { children: [t2("regex.maxDepth"), " ", (0, import_jsx_runtime17.jsx)(NullableNumInput, { value: rule.maxDepth, width: 64, onChange: (v) => set({ maxDepth: v }) })] }), (0, import_jsx_runtime17.jsxs)("label", { children: [t2("regex.macroExpand"), " ", (0, import_jsx_runtime17.jsx)(Select, { value: String(rule.substituteRegex), onChange: (v) => set({ substituteRegex: Number(v) }), options: [
...
function RegexSection(props) {
  ...
  const [presetDrafts, setPresetDrafts] = (0, import_react17.useState)(null);
  (0, import_react17.useEffect)(() => {
  ...
  return (0, import_jsx_runtime17.jsxs)(Section, { description: t2("regex.liveNotice"), title: t2("section.regex"), children: [toast.node, (0, import_jsx_runtime17.jsx)(Muted, { children: t2("regex.desc") }), state.status === "loading" && (0, import_jsx_runtime17.jsxs)(import_jsx_runtime17.Fragment, { children: [(0, import_jsx_runtime17.jsx)(Skeleton, { height: 72 }), (0, import_jsx_runtime17.jsx)(Skeleton, { height: 72 }), (0, import_jsx_runtime17.jsx)(Skeleton, { height: 72 })] }), state.status === "error" && (0, import_jsx_runtime17.jsx)(Err, { message: state.message }), (0, import_jsx_runtime17.jsx)(Err, { message: error }), rules !== null && (0, import_jsx_runtime17.jsxs)("div", { className: "dsh-tavern-regexCustom", children: [(0, import_jsx_runtime17.jsx)("div", { className: "dsh-tavern-groupHead", children: t2("regex.customHead") }), current2.map((rule, i) => (0, import_jsx_runtime17.jsx)(RuleEditor, { rule, onChange: (r) => {
    const next = current2.slice();
    next[i] = r;
    setRules(next);
  }, onDelete: () => setRules(current2.filter((_, j) => j !== i)) }, rule.id)), current2.length === 0 && (0, import_jsx_runtime17.jsxs)("div", { className: "dsh-tavern-regexEmpty", children: [(0, import_jsx_runtime17.jsx)("div", { className: "dsh-tavern-emptyTitle", children: t2("regex.emptyTitle") }), (0, import_jsx_runtime17.jsx)("div", { className: "dsh-tavern-emptyDesc", children: t2("regex.emptyDesc") })] }), (0, import_jsx_runtime17.jsxs)(SaveBar, { inline: true, children: [(0, import_jsx_runtime17.jsx)(Btn, { onClick: () => setRules([...current2, newRule()]), children: t2("regex.new") }), (0, import_jsx_runtime17.jsx)(Btn, { disabled: busy, onClick: () => void save(current2), primary: true, children: t2("regex.saveAll") }), (0, import_jsx_runtime17.jsx)(Btn, { onClick: () => {
    setRules(null);
    reload();
  }, children: t2("regex.discard") })] })] }), (0, import_jsx_runtime17.jsxs)("div", { className: "dsh-tavern-regexPresets", children: [(0, import_jsx_runtime17.jsx)("div", { className: "dsh-tavern-groupHead", children: t2("regex.presetHead") }), presetRegex.state.status === "loading" && (0, import_jsx_runtime17.jsxs)(import_jsx_runtime17.Fragment, { children: [(0, import_jsx_runtime17.jsx)(Skeleton, { height: 56 }), (0, import_jsx_runtime17.jsx)(Skeleton, { height: 56 })] }), presetRegex.state.status === "error" && (0, import_jsx_runtime17.jsx)(Err, { message: presetRegex.state.message }), presetDrafts !== null && presetDrafts.length === 0 && (0, import_jsx_runtime17.jsx)(Muted, { children: t2("regex.noPresetRegex") }), (presetDrafts ?? []).map((preset, pi) => (0, import_jsx_runtime17.jsxs)("div", { className: "dsh-tavern-regexPreset", children: [(0, import_jsx_runtime17.jsx)("div", { className: "dsh-tavern-fieldLabel", children: t2("regex.presetCount", { name: preset.name?.trim() || preset.identifier, count: preset.regexScripts.length }) }), (0, import_jsx_runtime17.jsx)("div", { className: "dsh-tavern-list", children: preset.regexScripts.map((s, si) => (0, import_jsx_runtime17.jsx)(RegexScriptRow, { script: s, index: si, disabled: busy, onToggle: (disabled) => void togglePresetScript(pi, si, disabled) }, s.id ?? si)) })] }, preset.identifier))] })] });
}
...
function SettingsSection(props) {
  const { remote } = props;
  const t2 = useT();
  const tabsId = (0, import_react18.useId)();
  ...
  const receivedSettings = (0, import_react18.useRef)(false);
  ...
  const changeLocale = (locale) => {
    ...
    void runAsync(setBusy, setError, async () => {
      ...
      if (r.ok)
        setBaseline((current2) => current2 ? { ...current2, locale } : current2);
      if (!r.ok) {
  ...
  if (state.status === "loading")
    return (0, import_jsx_runtime18.jsxs)(Section, { title: t2("settings.title"), children: [(0, import_jsx_runtime18.jsx)(Skeleton, { height: 56 }), (0, import_jsx_runtime18.jsx)(Skeleton, { height: 56 }), (0, import_jsx_runtime18.jsx)(Skeleton, { height: 56 })] });
  if (state.status === "error")
    return (0, import_jsx_runtime18.jsxs)(Section, { title: t2("settings.title"), children: [(0, import_jsx_runtime18.jsx)(Err, { message: state.message }), (0, import_jsx_runtime18.jsx)(Btn, { size: "md", onClick: reload, children: t2("action.retry") })] });
  if (!draft)
    return null;
  ...
  return (0, import_jsx_runtime18.jsxs)(import_jsx_runtime18.Fragment, { children: [toast.node, (0, import_jsx_runtime18.jsxs)("fieldset", { disabled: busy, className: "dsh-tavern-editorFields", children: [(0, import_jsx_runtime18.jsx)(Tabs, { id: tabsId, panelId: `${tabsId}-panel`, label: t2("settings.title"), items: SUBS.map((s) => ({ id: s.id, label: t2(s.labelKey) })), value: sub, onChange: (id) => {
    lastSub = id;
    setSub(id);
  } }), (0, import_jsx_runtime18.jsxs)("div", { id: `${tabsId}-panel`, role: "tabpanel", "aria-labelledby": `${tabsId}-${sub}`, tabIndex: 0, className: "dsh-tavern-rise", children: [sub === "interface" && (0, import_jsx_runtime18.jsx)(Section, { title: t2("settings.interface.title"), description: t2("settings.interface.desc"), children: (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.interface.language"), description: t2("settings.interface.languageDesc"), children: (0, import_jsx_runtime18.jsx)(Select, { size: "md", value: draft.locale, onChange: (v) => changeLocale(v === "zh" ? "zh" : v === "en" ? "en" : "auto"), options: [
    { value: "auto", label: t2("settings.interface.localeAuto") },
    { value: "en", label: "English" },
    { value: "zh", label: "\u4E2D\u6587" }
  ] }) }) }), sub === "defaults" && (0, import_jsx_runtime18.jsxs)(Section, { title: t2("settings.defaults.title"), description: t2("settings.defaults.desc"), children: [(0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.defaults.preset"), description: t2("settings.defaults.presetDesc"), children: (0, import_jsx_runtime18.jsx)(Select, { size: "md", value: draft.defaults.presetId, onChange: (presetId) => setDefaults({ presetId }), options: [
  ...
  ] }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.defaults.persona"), description: t2("settings.defaults.personaDesc"), children: (0, import_jsx_runtime18.jsx)(Select, { size: "md", value: draft.defaults.personaId, onChange: (personaId) => setDefaults({ personaId }), options: [{ value: "", label: t2("settings.defaults.noPersona") }, ...personaItems.map((p) => ({ value: p.id, label: p.name }))] }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.defaults.mainLore"), description: t2("settings.defaults.mainLoreDesc"), children: (0, import_jsx_runtime18.jsx)(Select, { size: "md", value: draft.defaults.characterLorebookId, onChange: (characterLorebookId) => setDefaults({ characterLorebookId }), options: [{ value: "", label: t2("settings.defaults.embeddedLore") }, ...lorebooks.map((n) => ({ value: n, label: n }))] }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.defaults.globalLore"), description: t2("settings.defaults.globalLoreDesc"), stacked: true, children: lorebooks.length === 0 ? (0, import_jsx_runtime18.jsx)(Muted, { children: t2("settings.defaults.noLorebooks") }) : (0, import_jsx_runtime18.jsx)(CheckChips, { ariaLabel: t2("settings.defaults.globalLore"), options: lorebooks.map((n) => ({ value: n, label: n })), selected: draft.defaults.lorebookIds, onChange: (lorebookIds) => setDefaults({ lorebookIds }) }) }), (0, import_jsx_runtime18.jsx)(SaveBar, { children: (0, import_jsx_runtime18.jsx)(Btn, { primary: true, size: "md", disabled: busy, onClick: () => void save({ defaults: draft.defaults }, t2("settings.defaults.saved")), children: t2("settings.defaults.save") }) })] }), sub === "sampling" && (0, import_jsx_runtime18.jsxs)(Section, { title: t2("settings.sampling.title"), description: t2("settings.sampling.desc"), children: [(0, import_jsx_runtime18.jsx)(SettingsRow, { title: "temperature", description: t2("settings.sampling.temperatureDesc"), children: (0, import_jsx_runtime18.jsx)(NumInput, { step: "0.05", value: draft.sampling.temperature, onChange: (v) => setSampling({ temperature: v }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: "topP", description: t2("settings.sampling.topPDesc"), children: (0, import_jsx_runtime18.jsx)(NumInput, { step: "0.05", value: draft.sampling.topP, onChange: (v) => setSampling({ topP: v }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: "maxTokens", description: t2("settings.sampling.maxTokensDesc"), children: (0, import_jsx_runtime18.jsx)(NumInput, { value: draft.sampling.maxTokens, onChange: (v) => setSampling({ maxTokens: Math.max(0, Math.round(v)) }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: "presencePenalty", children: (0, import_jsx_runtime18.jsx)(NumInput, { step: "0.1", value: draft.sampling.presencePenalty, onChange: (v) => setSampling({ presencePenalty: v }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: "frequencyPenalty", children: (0, import_jsx_runtime18.jsx)(NumInput, { step: "0.1", value: draft.sampling.frequencyPenalty, onChange: (v) => setSampling({ frequencyPenalty: v }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.sampling.thinking"), description: t2("settings.sampling.thinkingDesc"), children: (0, import_jsx_runtime18.jsx)(Select, { size: "md", value: draft.sampling.thinking, onChange: (v) => setSampling({ thinking: v }), options: [
  ...
  ] }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.sampling.stop"), description: t2("settings.sampling.stopDesc"), stacked: true, children: (0, import_jsx_runtime18.jsx)("textarea", { className: "dsh-tavern-input dsh-tavern-textarea dsh-tavern-codeFont", style: { minHeight: 64 }, value: draft.sampling.stop.join("\n"), onChange: (e) => setSampling({ stop: e.target.value.split("\n").map((s) => s.trim()).filter(Boolean) }) }) }), (0, import_jsx_runtime18.jsx)(SaveBar, { children: (0, import_jsx_runtime18.jsx)(Btn, { disabled: busy, onClick: () => void save({ sampling: draft.sampling }, t2("settings.sampling.saved")), primary: true, size: "md", children: t2("settings.sampling.save") }) })] }), sub === "worldinfo" && (0, import_jsx_runtime18.jsxs)(Section, { title: t2("settings.worldinfo.title"), description: t2("settings.worldinfo.desc"), children: [(0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.worldinfo.scanDepth"), children: (0, import_jsx_runtime18.jsx)(NumInput, { value: draft.worldInfo.scanDepth, onChange: (v) => setWorldInfo({ scanDepth: Math.max(0, Math.round(v)) }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.worldinfo.contextPercent"), description: t2("settings.worldinfo.contextPercentDesc"), children: (0, import_jsx_runtime18.jsx)(NumInput, { value: draft.worldInfo.contextPercent, onChange: (v) => setWorldInfo({ contextPercent: v }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.worldinfo.tokenBudget"), description: t2("settings.worldinfo.tokenBudgetDesc"), children: (0, import_jsx_runtime18.jsx)(NumInput, { value: draft.worldInfo.tokenBudget, onChange: (v) => setWorldInfo({ tokenBudget: Math.max(0, Math.round(v)) }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.worldinfo.maxRecursionSteps"), description: t2("settings.worldinfo.maxRecursionStepsDesc"), children: (0, import_jsx_runtime18.jsx)(NumInput, { value: draft.worldInfo.maxRecursionSteps, onChange: (v) => setWorldInfo({ maxRecursionSteps: Math.max(0, Math.round(v)) }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.worldinfo.strategy"), children: (0, import_jsx_runtime18.jsx)(Select, { size: "md", value: String(draft.worldInfo.characterStrategy), onChange: (v) => setWorldInfo({ characterStrategy: Number(v) }), options: [
  ...
  ].map(([key, titleKey, descKey]) => (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2(titleKey), description: descKey ? t2(descKey) : void 0, children: (0, import_jsx_runtime18.jsx)(Toggle, { checked: draft.worldInfo[key], onChange: (on) => setWorldInfo({ [key]: on }) }) }, key)), (0, import_jsx_runtime18.jsx)(SaveBar, { children: (0, import_jsx_runtime18.jsx)(Btn, { disabled: busy, onClick: () => void save({ worldInfo: draft.worldInfo }, t2("settings.worldinfo.saved")), primary: true, size: "md", children: t2("settings.worldinfo.save") }) })] }), sub === "memory" && (0, import_jsx_runtime18.jsxs)(Section, { title: t2("settings.memory.title"), description: t2("settings.memory.desc"), children: [(0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.memory.maxEntries"), description: t2("settings.memory.maxEntriesDesc"), children: (0, import_jsx_runtime18.jsx)(NumInput, { value: draft.memory.maxEntries, onChange: (v) => setMemory({ maxEntries: Math.max(1, Math.round(v)) }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.memory.maxTokens"), description: t2("settings.memory.maxTokensDesc"), children: (0, import_jsx_runtime18.jsx)(NumInput, { value: draft.memory.maxTokens, onChange: (v) => setMemory({ maxTokens: Math.max(0, Math.round(v)) }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.memory.retrievalTopK"), description: t2("settings.memory.retrievalTopKDesc"), children: (0, import_jsx_runtime18.jsx)(NumInput, { value: draft.memory.retrievalTopK, onChange: (v) => setMemory({ retrievalTopK: Math.max(0, Math.round(v)) }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.memory.retrievalTokenBudget"), description: t2("settings.memory.retrievalTokenBudgetDesc"), children: (0, import_jsx_runtime18.jsx)(NumInput, { value: draft.memory.retrievalTokenBudget, onChange: (v) => setMemory({ retrievalTokenBudget: Math.max(0, Math.round(v)) }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.memory.halfLifeDays"), description: t2("settings.memory.halfLifeDaysDesc"), children: (0, import_jsx_runtime18.jsx)(NumInput, { value: draft.memory.halfLifeDays, onChange: (v) => setMemory({ halfLifeDays: Math.max(0, v) }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.memory.dedupScore"), description: t2("settings.memory.dedupScoreDesc"), children: (0, import_jsx_runtime18.jsx)(NumInput, { value: draft.memory.dedupScore, onChange: (v) => setMemory({ dedupScore: Math.max(0, v) }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.memory.compressBatch"), description: t2("settings.memory.compressBatchDesc"), children: (0, import_jsx_runtime18.jsx)(NumInput, { value: draft.memory.compressBatch, onChange: (v) => setMemory({ compressBatch: Math.max(2, Math.round(v)) }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.memory.queryMessages"), description: t2("settings.memory.queryMessagesDesc"), children: (0, import_jsx_runtime18.jsx)(NumInput, { value: draft.memory.queryMessages, onChange: (v) => setMemory({ queryMessages: Math.max(1, Math.round(v)) }) }) }), (0, import_jsx_runtime18.jsx)(SaveBar, { children: (0, import_jsx_runtime18.jsx)(Btn, { disabled: busy, onClick: () => void save({ memory: draft.memory }, t2("settings.memory.saved")), primary: true, size: "md", children: t2("settings.memory.save") }) })] }), sub === "cards" && (0, import_jsx_runtime18.jsxs)(import_jsx_runtime18.Fragment, { children: [(0, import_jsx_runtime18.jsxs)(Section, { title: t2("settings.cards.title"), description: t2("settings.cards.desc"), children: [(0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.cards.cascadeDelete"), description: t2("settings.cards.cascadeDeleteDesc"), children: (0, import_jsx_runtime18.jsx)(Toggle, { checked: draft.cascadeDeleteEmbeddedBook, onChange: (cascadeDeleteEmbeddedBook) => setDraft({ ...draft, cascadeDeleteEmbeddedBook }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.cards.interactiveCards"), description: t2("settings.cards.interactiveCardsDesc"), children: (0, import_jsx_runtime18.jsx)(Toggle, { checked: draft.interactiveCards, onChange: (interactiveCards) => setDraft({ ...draft, interactiveCards }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.cards.triggerLogMax"), children: (0, import_jsx_runtime18.jsx)(NumInput, { value: draft.triggerLogMax, onChange: (v) => setDraft({ ...draft, triggerLogMax: Math.max(10, Math.round(v)) }) }) }), (0, import_jsx_runtime18.jsx)(SettingsRow, { title: t2("settings.cards.whitelist"), description: t2("settings.cards.whitelistDesc"), stacked: true, children: (0, import_jsx_runtime18.jsx)("textarea", { className: "dsh-tavern-input dsh-tavern-textarea dsh-tavern-codeFont", style: { minHeight: 64 }, value: draft.cardNetworkWhitelist.join("\n"), onChange: (e) => setDraft({ ...draft, cardNetworkWhitelist: e.target.value.split("\n").map((s) => s.trim()).filter(Boolean) }) }) }), (0, import_jsx_runtime18.jsx)(SaveBar, { children: (0, import_jsx_runtime18.jsx)(Btn, { primary: true, size: "md", disabled: busy, onClick: () => void save({
  ...
  }, t2("settings.cards.saved")), children: t2("settings.cards.save") }) })] }), (0, import_jsx_runtime18.jsx)("div", { className: "dsh-tavern-groupHead", children: t2("settings.cards.dataHome") }), (0, import_jsx_runtime18.jsx)(Muted, { children: (0, import_jsx_runtime18.jsxs)("span", { style: { wordBreak: "break-all" }, children: [t2("settings.cards.dataHomeDesc"), dataInfo.state.status === "ready" ? dataInfo.state.value.dataHome : "\u2026"] }) })] })] }, sub), (0, import_jsx_runtime18.jsx)(Err, { message: error })] })] });
}
...
function TavernPanel(props) {
  return (0, import_jsx_runtime19.jsx)(PersistentEditor, { remote: props.remote, scope: "panel", children: (0, import_jsx_runtime19.jsx)(PanelContent, { ...props }) });
}
function PanelContent(props) {
  const { remote } = props;
  const t2 = useT();
  const tabsId = (0, import_react19.useId)();
  ...
  (0, import_react19.useLayoutEffect)(() => {
    ...
    const reveal = () => {
      if (!narrow.matches)
        return;
      panelRef.current?.closest('[role="dialog"]')?.querySelector(':scope > nav [aria-current="true"]')?.scrollIntoView({ block: "nearest", inline: "nearest" });
    };
    reveal();
    narrow.addEventListener("change", reveal);
    return () => narrow.removeEventListener("change", reveal);
  }, []);
  return (0, import_jsx_runtime19.jsx)(DraftScope, { children: (request, dirty, busy) => (0, import_jsx_runtime19.jsxs)("div", { ref: panelRef, className: "dsh-tavern-ui dsh-tavern-panel", style: { width: "100%", maxWidth: "100%", fontSize: 14, boxSizing: "border-box" }, children: [dirty && (0, import_jsx_runtime19.jsx)("div", { className: "dsh-tavern-draftStatus", role: "status", children: (0, import_jsx_runtime19.jsx)(Badge, { accent: true, children: t2(busy ? "draft.saving" : "draft.unsaved") }) }), (0, import_jsx_runtime19.jsx)(Tabs, { id: tabsId, panelId: `${tabsId}-panel`, label: t2("settings.label"), items: TABS.map((tab_) => ({ id: tab_.id, label: t2(tab_.labelKey) })), value: tab, onChange: (id) => {
    if (id === tab)
      return;
    request(() => {
      lastTab = id;
  ...
  } }), (0, import_jsx_runtime19.jsxs)("div", { id: `${tabsId}-panel`, role: "tabpanel", "aria-labelledby": `${tabsId}-${tab}`, tabIndex: 0, className: "dsh-tavern-rise", children: [tab === "characters" && (0, import_jsx_runtime19.jsx)(CharactersSection, { remote }), tab === "presets" && (0, import_jsx_runtime19.jsx)(PresetsSection, { remote }), tab === "lorebooks" && (0, import_jsx_runtime19.jsx)(LorebooksSection, { remote }), tab === "personas" && (0, import_jsx_runtime19.jsx)(PersonasSection, { remote }), tab === "regex" && (0, import_jsx_runtime19.jsx)(RegexSection, { remote }), tab === "memory" && (0, import_jsx_runtime19.jsx)(MemorySection, { remote }), tab === "sampling" && (0, import_jsx_runtime19.jsx)(SettingsSection, { remote })] }, tab)] }) });
}
