...
export function TavernAssistantNode(props) {
    ...
    if (binding && text && !hasImages) {
        return (_jsxs("div", { children: [bindingError, _jsx(ReasoningFold, { text: reasoningText, streaming: streaming }), _jsx(SpeechBubble, { remote: remote, sessionId: sessionId, cardId: binding.cardId, name: name || t('assistant.characterFallback'), rawText: text, messageId: node.data.finalNode?.seq, streaming: streaming || node.location?.turn?.status === 'open', interactiveCards: binding.interactiveCards, onMessageBranch: async (branch) => { if (!sessions)
    ...
    return (_jsxs("div", { children: [bindingError, _jsx(NativeAssistantFallback, { ...props, streaming: streaming, interrupted: interrupted, stripMeta: true }), interruptedActions] }));
}
