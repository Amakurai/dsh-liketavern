...
export function TavernAssistantNode(props) {
    ...
    if (binding && text && !hasImages) {
        return (_jsxs("div", { children: [_jsx(ReasoningFold, { text: reasoningText, streaming: streaming }), _jsx(SpeechBubble, { remote: remote, sessionId: sessionId, cardId: binding.cardId, name: name || t('assistant.characterFallback'), rawText: text, streaming: streaming, interactiveCards: binding.interactiveCards, onSwipeGreeting: async (index) => {
                        if (!sessions)
