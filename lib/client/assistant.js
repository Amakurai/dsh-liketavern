...
export function TavernAssistantNode(props) {
    ...
    if (binding && text && !hasImages) {
        return (_jsxs("div", { children: [_jsx(ReasoningFold, { text: reasoningText, streaming: streaming }), _jsx(SpeechBubble, { remote: remote, sessionId: sessionId, cardId: binding.cardId, name: name || t('assistant.characterFallback'), rawText: text, streaming: streaming, onSwipeGreeting: (index) => {
                        if (!sessions)
                            return;
                    ...
                    } }), interrupted && _jsx("div", { className: "dsh-tavern-notice", children: t('assistant.stopped') })] }));
    }
...
function NativeAssistantFallback(props) {
    ...
    for (let i = 0; i < blocks.length; i++) {
        ...
        if (block.kind === 'text') {
            ...
            rendered.push(_jsx(JsonBlock, { label: t('assistant.unknownBlock'), payload: block.block }, i));
        }
    }
