...
function PromptPreviewDialog(props) {
    ...
    return (_jsxs(Dialog, { open: true, title: t('chip.preview.title'), onClose: props.onClose, width: "lg", children: [_jsxs(Muted, { children: [t('chip.preview.budget', { used: wi.used, limit: wi.limit }), wi.overflowed ? ` · ${t('chip.preview.overflowed')}` : '', ' · ', t('chip.preview.assemble', { after: assemble.tokensAfter, before: assemble.tokensBefore }), assemble.trimmedSections.length > 0
                        ? ` · ${t('chip.preview.trimmed', { sections: assemble.trimmedSections.join(t('chip.preview.listSep')) })}`
                        : ''] }), _jsxs("div", { className: "dsh-tavern-filters", style: { margin: '10px 0 12px' }, children: [_jsx(Tabs, { size: "sm", value: tab, onChange: (id) => setTab(id), items: [
                            { id: 'standing', label: 'standing' },
                            { id: 'turn', label: t('chip.preview.tab.turn') },
                            { id: 'full', label: t('chip.preview.tab.full') },
                            { id: 'log', label: t('binding.triggerLog') },
                        ] }), _jsx("span", { style: { flex: 1 } }), _jsx(IconBtn, { label: t('chip.preview.copyView'), onClick: () => void copyBody(), children: _jsx(IconCopyOutline16, {}) })] }), _jsx("pre", { className: "dsh-tavern-modalPre", children: body }), toast.node] }));
}
export function TavernHeaderChip(props) {
    ...
    return (_jsxs("span", { className: "dsh-tavern-ui", style: { display: 'inline-flex' }, children: [_jsx(TavernSeatChip, { label: binding ? (name ?? listedName ?? t('chip.characterFallback')) : t('hero.pickCharacter'), title: t('hero.pickCharacter'), avatarUrl: avatar, open: open, loading: bindingLoader.state.status === 'loading', hasPopup: "dialog", onClick: () => setOpen(!open) }), _jsx(Dialog, { open: open, title: t('chip.dialog.title'), onClose: () => setOpen(false), width: "xl", footer: draft ? (_jsxs("div", { className: "dsh-tavern-footActions", children: [binding ? (_jsx(Btn, { danger: true, size: "md", onClick: () => {
                                setConfirmUnbind(true);
                            }, children: t('chip.unbind.action') })) : null, _jsx("span", { className: "dsh-tavern-footSpacer" }), _jsxs("span", { className: "dsh-tavern-footGroup", children: [_jsx(Btn, { size: "md", onClick: () => void openChatLore(), children: t('chip.chatLore.edit') }), _jsx(Btn, { primary: true, size: "md", onClick: () => void saveBinding(), children: t('binding.save') })] })] })) : undefined, children: _jsxs("div", { className: "dsh-tavern-binding", children: [_jsx(Err, { message: error }), !lists && (_jsxs("div", { className: "dsh-tavern-panelCard", children: [_jsx(Skeleton, { height: 14, width: "24%" }), _jsx(Skeleton, { height: 38, radius: 18 }), _jsx(Skeleton, { height: 38, radius: 18 }), _jsx(Skeleton, { height: 38, radius: 18 })] })), lists && (_jsxs(_Fragment, { children: [draft ? (_jsxs(_Fragment, { children: [_jsxs("div", { className: "dsh-tavern-panelCard", children: [_jsx("div", { className: "dsh-tavern-groupHead", children: t('chip.group.binding') }), _jsx(Field, { label: t('chip.field.character'), children: _jsx(Select, { width: "100%", value: draft.cardId, onChange: (cardId) => {
                                                            if (!cardId)
                                                                return;
                                                    ...
                                                    : ''] })] }))] }))] }) }), toast.node, view && _jsx(PreDialog, { title: view.title, text: view.text, onClose: () => setView(null) }), previewData && _jsx(PromptPreviewDialog, { data: previewData, onClose: () => setPreviewData(null) }), chatLore && (_jsx(Dialog, { open: true, width: "xl", title: t('chip.chatLore.title'), onClose: () => setChatLore(null), children: _jsx(LorebookEditor, { target: { kind: 'chat', cardId: chatLore.cardId, name: t('chip.chatLore.title') }, entries: chatLore.entries, onClose: () => setChatLore(null), onSaved: () => {
                        toast.show(t('chip.chatLore.saved'));
                        setChatLore(null);
                    }, save: (json) => remote.saveChatLorebook({ cardId: chatLore.cardId, json }) }) })), _jsx(ConfirmDialog, { open: confirmUnbind, title: t('chip.unbind.title'), description: t('chip.unbind.desc'), confirmLabel: t('chip.unbind.action'), danger: true, busy: unbindBusy, onCancel: () => {
                    setConfirmUnbind(false);
