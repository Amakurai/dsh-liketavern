...
import { DEFAULT_LOCALE, en, zh } from './locales.js';
let preference = 'auto';
let hostActive = 'en';
...
export function getTavernLocale() {
    return current;
}
/** 设置语言偏好（'auto' 跟随宿主）；立即按当前宿主语言重算生效语言。 */
export function setTavernLocale(id) {
    if (id === preference)
        return;
    preference = id;
    syncEffective();
...
export function setTavernHostLocale(active) {
