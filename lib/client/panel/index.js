...
export function TavernPanel(props) {
    const { remote } = props;
    const t = useT();
    const [tab, setTab] = useState(lastTab ?? 'characters');
    return (_jsxs("div", { className: "dsh-tavern-ui dsh-tavern-panel", style: { width: '100%', maxWidth: '100%', fontSize: 14, boxSizing: 'border-box' }, children: [_jsx(Tabs, { items: TABS.map((tab_) => ({ id: tab_.id, label: t(tab_.labelKey) })), value: tab, onChange: (id) => {
                    lastTab = id;
                    setTab(id);
                } }), _jsxs("div", { className: "dsh-tavern-rise", children: [tab === 'characters' && _jsx(CharactersSection, { remote: remote }), tab === 'presets' && _jsx(PresetsSection, { remote: remote }), tab === 'lorebooks' && _jsx(LorebooksSection, { remote: remote }), tab === 'personas' && _jsx(PersonasSection, { remote: remote }), tab === 'regex' && _jsx(RegexSection, { remote: remote }), tab === 'memory' && _jsx(MemorySection, { remote: remote }), tab === 'sampling' && _jsx(SettingsSection, { remote: remote })] }, tab)] }));
}
