...
export function MemorySection(props) {
    ...
    return (_jsxs(Section, { title: t('section.memory'), description: t('memory.desc'), children: [toast.node, _jsx(SettingsRow, { title: t('memory.character'), description: t('memory.characterDesc'), children: _jsx(Select, { size: "md", value: cardId, disabled: busy, onChange: (value) => { setStoryId(undefined); setCardId(value); }, options: [{ value: '', label: t('memory.pickCharacter') }, ...charItems.map((c) => ({ value: c.cardId, label: c.name }))] }) }), cardId && _jsx(SettingsRow, { title: t('memory.story'), description: t('memory.storyDesc'), children: _jsx(Select, { value: storyId ?? '', disabled: busy, onChange: (value) => setStoryId(value || undefined), options: [{ value: '', label: t('memory.initialState') }, ...(stories.state.status === 'ready' ? stories.state.value.items.map((story) => ({ value: story.id, label: `${story.sessionId} · ${story.createdAt.slice(0, 10)}` })) : [])] }) }), _jsx(Err, { message: stories.state.status === 'error' ? stories.state.message : error }), cardId && (_jsxs(_Fragment, { children: [_jsxs("div", { style: { display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 10, margin: '10px 0 14px' }, children: [_jsxs("div", { className: "dsh-tavern-filters", children: [_jsx("button", { type: "button", className: "dsh-tavern-chip", "data-active": tab === 'memory' ? 'true' : 'false', onClick: () => setTab('memory'), children: t('memory.tab.memory', { count: memoryItems.length }) }), _jsx("button", { type: "button", className: "dsh-tavern-chip", "data-active": tab === 'delta' ? 'true' : 'false', onClick: () => setTab('delta'), children: t('memory.tab.delta', { count: deltaItems.length }) }), _jsx("button", { type: "button", className: "dsh-tavern-chip", "data-active": tab === 'journal' ? 'true' : 'false', onClick: () => setTab('journal'), children: t('memory.tab.journal') })] }), _jsx("span", { style: { flex: 1 } }), tab === 'memory' && _jsx(Btn, { disabled: busy, onClick: () => void compress(), children: t('memory.compressOldest') }), tab === 'delta' && _jsx(Btn, { disabled: busy, onClick: () => void exportBook(), children: t('memory.exportBook') })] }), tab === 'memory' && (_jsxs("div", { className: "dsh-tavern-list", children: [memories.state.status === 'loading' && (_jsxs(_Fragment, { children: [_jsx(Skeleton, { height: 72 }), _jsx(Skeleton, { height: 72 }), _jsx(Skeleton, { height: 72 })] })), memories.state.status === 'error' && _jsx(Err, { message: memories.state.message }), memoryItems.length === 0 && memories.state.status === 'ready' && (_jsxs("div", { className: "dsh-tavern-empty is-compact", children: [_jsx("div", { className: "dsh-tavern-emptyTitle", children: t('memory.emptyMemories') }), _jsx("div", { className: "dsh-tavern-emptyDesc", children: t('memory.emptyMemoriesDesc') })] })), memoryItems.map((m) => (_jsxs("div", { className: "dsh-tavern-memo", children: [_jsxs("div", { className: "dsh-tavern-memoHead", children: [_jsx(Badge, { children: m.id }), m.archived ? _jsx(Badge, { children: t('memory.archived') }) : null, m.tags.map((tag) => (_jsx(Badge, { children: tag }, tag))), _jsx("span", { className: "dsh-tavern-memoMeta", children: m.updated }), _jsxs("span", { className: "dsh-tavern-memoActions", children: [_jsx(IconBtn, { label: editingId === m.id ? t('memory.collapseEdit') : t('action.edit'), onClick: () => setEditingId(editingId === m.id ? null : m.id), children: _jsx(IconEditOutline16, {}) }), _jsx(IconBtn, { label: t('memory.deleteEntry'), danger: true, disabled: busy, onClick: () => void deleteMemory(m.id), children: _jsx(IconTrashOutline16, {}) })] })] }), _jsx("pre", { className: "dsh-tavern-memoBody dsh-tavern-scroll", children: m.body }), editingId === m.id && (_jsx(MemoryEditor, { remote: remote, cardId: cardId, storyId: storyId, entry: m, onDone: () => {
                                            setEditingId(null);
                                            memories.reload();
                                                        ...
                                                        ] })] }), (deltaType === 'update' || deltaType === 'invalidate') && (_jsxs("label", { className: "dsh-tavern-field", children: [_jsx("span", { className: "dsh-tavern-fieldLabel", children: t('memory.deltaRef') }), _jsx("input", { className: "dsh-tavern-input", value: deltaRef, onChange: (e) => setDeltaRef(e.target.value) })] }))] }), _jsx("textarea", { className: "dsh-tavern-input dsh-tavern-textarea", style: { minHeight: 60, marginTop: 8 }, placeholder: t('memory.deltaBodyPlaceholder'), value: deltaContent, onChange: (e) => setDeltaContent(e.target.value) }), _jsxs("label", { className: "dsh-tavern-field", style: { marginTop: 8 }, children: [_jsx("span", { className: "dsh-tavern-fieldLabel", children: t('memory.deltaKeys') }), _jsx("input", { className: "dsh-tavern-input", value: deltaKeys, onChange: (e) => setDeltaKeys(e.target.value) })] }), _jsx("div", { style: { display: 'flex', justifyContent: 'flex-end', marginTop: 8 }, children: _jsx(Btn, { primary: true, disabled: busy || !deltaContent.trim(), onClick: () => void addDelta(), children: t('memory.addDelta') }) })] })] })), tab === 'journal' && (_jsxs("div", { className: "dsh-tavern-list", children: [_jsx(Muted, { children: t('memory.journalHint') }), journal.state.status === 'ready' ? (_jsx("textarea", { className: "dsh-tavern-input dsh-tavern-textarea", style: { minHeight: 180 }, value: journalText, onChange: (e) => setJournalText(e.target.value) })) : journal.state.status === 'error' ? (_jsx(Err, { message: journal.state.message })) : (_jsx(Skeleton, { height: 180 })), _jsx("div", { style: { display: 'flex', justifyContent: 'flex-end' }, children: _jsx(Btn, { primary: true, disabled: busy || journal.state.status !== 'ready' || !cardId, onClick: () => void saveJournal(), children: t('memory.saveJournal') }) })] }))] }))] }));
}
