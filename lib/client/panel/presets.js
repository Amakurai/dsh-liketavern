...
export function PresetsSection(props) {
    ...
    return (_jsxs(Section, { title: t('section.presets'), description: t('presets.section.desc'), children: [toast.node, _jsxs("div", { className: "dsh-tavern-toolbar", children: [_jsx(FileBtn, { accept: ".json", disabled: busy, onFile: (file) => void onImportFile(file), children: t('presets.importFile') }), _jsx(Btn, { size: "md", onClick: createNew, children: t('presets.new') }), _jsx(Btn, { size: "md", onClick: reload, disabled: busy, children: t('action.refresh') }), items.length >= 5 && (_jsx(SearchInput, { label: t('presets.searchLabel'), value: query, onChange: setQuery, placeholder: t('presets.searchPlaceholder'), width: 220 }))] }), state.status === 'loading' && (_jsxs("div", { className: "dsh-tavern-list", children: [_jsx(Skeleton, { height: 70, radius: 16 }), _jsx(Skeleton, { height: 70, radius: 16 }), _jsx(Skeleton, { height: 70, radius: 16 })] })), state.status === 'error' && _jsx(Err, { message: state.message }), _jsx(Err, { message: error }), items.length === 0 && state.status === 'ready' && (_jsxs("div", { className: "dsh-tavern-empty", children: [_jsx("div", { className: "dsh-tavern-emptyIcon", children: _jsx(IconFolderOpenOutline16, { size: 32 }) }), _jsx("div", { className: "dsh-tavern-emptyTitle", children: t('presets.empty') }), _jsx("div", { className: "dsh-tavern-emptyDesc", children: t('presets.emptyDesc') })] })), q !== '' && filtered.length === 0 && state.status === 'ready' && (_jsx(SearchEmpty, { what: t('presets.noun'), query: query.trim(), onClear: () => setQuery('') })), _jsx("div", { className: "dsh-tavern-list", style: { marginBottom: 12 }, children: filtered.map((item) => (_jsxs("div", { className: "dsh-tavern-tile", ...clickableProps(() => void open(item.id)), children: [_jsx("span", { className: "dsh-tavern-tileIcon", children: _jsx(IconListPenOutline16, { size: 18 }) }), _jsxs("div", { className: "dsh-tavern-tileMain", children: [_jsxs("div", { className: "dsh-tavern-tileTitleRow", children: [_jsx("span", { className: "dsh-tavern-tileName", children: item.name }), item.regexCount > 0 ? _jsx(Badge, { children: t('presets.regexCount', { count: item.regexCount }) }) : null] }), _jsx("span", { className: "dsh-tavern-tileSub", children: item.id })] }), _jsxs("div", { className: "dsh-tavern-tileActions", children: [_jsx(IconBtn, { label: t('action.edit'), onClick: () => void open(item.id), children: _jsx(IconEditOutline16, {}) }), _jsx(IconBtn, { label: t('presets.export'), disabled: busy, onClick: () => void exportPreset(item.id, item.name), children: _jsx(IconDownloadOutline16, {}) }), _jsx(Btn, { size: "sm", disabled: busy, onClick: () => void setAsDefault(item.id), children: t('presets.setAsDefault') }), _jsx(IconBtn, { label: t('presets.delete'), danger: true, onClick: () => setToDelete(item.id), children: _jsx(IconTrashOutline16, {}) })] })] }, item.id))) }), _jsx(ConfirmDialog, { open: toDelete !== null, title: t('presets.deleteTitle'), description: toDelete ? t('presets.deleteDesc', { id: toDelete }) : '', confirmLabel: t('action.delete'), danger: true, busy: busy, onCancel: () => setToDelete(null), onConfirm: () => void remove() }), editing && (_jsxs("div", { className: "dsh-tavern-card", style: { marginBottom: 12 }, children: [_jsx(Field, { label: t('presets.name'), children: _jsx("input", { className: "dsh-tavern-input", style: { flex: 1 }, value: editing.name, onChange: (e) => setEditing({ ...editing, name: e.target.value }) }) }), _jsx(Field, { label: t('presets.identifier'), children: _jsx(Muted, { children: editing.identifier }) }), (editing.regexScripts?.length ?? 0) > 0 && (_jsx(PresetRegexList, { scripts: editing.regexScripts, onChange: (scripts) => setEditing({ ...editing, regexScripts: scripts }) })), _jsx("div", { className: "dsh-tavern-list", style: { margin: '8px 0' }, children: editing.entries.map((entry, i) => (_jsx(EntryEditor, { entry: entry, onChange: (e2) => setEntry(i, e2), onDelete: () => setEditing({ ...editing, entries: editing.entries.filter((_, j) => j !== i) }) }, entry.identifier))) }), _jsxs(SaveBar, { children: [_jsx(Btn, { onClick: () => setEditing({ ...editing, entries: [...editing.entries, newEntry(editing.entries.length * 100 + 100)] }), children: t('presets.addEntry') }), _jsx(Btn, { disabled: busy, onClick: () => void save(), primary: true, children: t('presets.save') }), _jsx(Btn, { onClick: () => setEditing(null), children: t('action.close') })] })] }))] }));
}
