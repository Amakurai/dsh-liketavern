...
export function RegexSection(props) {
    ...
    const current = rules ?? [];
    return (_jsxs(Section, { description: t('regex.liveNotice'), title: t('section.regex'), children: [toast.node, _jsx(Muted, { children: t('regex.desc') }), state.status === 'loading' && (_jsxs(_Fragment, { children: [_jsx(Skeleton, { height: 72 }), _jsx(Skeleton, { height: 72 }), _jsx(Skeleton, { height: 72 })] })), state.status === 'error' && _jsx(Err, { message: state.message }), _jsx(Err, { message: error }), rules !== null && (_jsxs("div", { className: "dsh-tavern-regexCustom", children: [_jsx("div", { className: "dsh-tavern-groupHead", children: t('regex.customHead') }), current.map((rule, i) => (_jsx(RuleEditor, { rule: rule, onChange: (r) => {
