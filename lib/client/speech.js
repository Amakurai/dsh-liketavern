...
import { disableInteractiveParts } from '../core/templateDisplay.js';
...
export function SpeechHtmlFrame(props) {
    ...
    useLayoutEffect(() => {
        ...
        const onMsg = (e) => {
            ...
            if (helperRecord(value) && ['iframe-resize', 'resizeIframe'].includes(String(value.type)) && typeof value.height === 'number' && Number.isFinite(value.height) && value.height > 0) {
                legacyHeight = true;
                setFrameH(Math.min(8000, Math.max(24, Math.ceil(value.height))));
                return;
            }
            ...
            if (helperRecord(value) && value.source === 'dsh-tavern-card' && handlers.current.readOnly && typeof value.action === 'string') {
                ...
                const responses = { helperDisplayRefresh: 'helperDisplayResult', helperMessageEdit: 'helperMessageEditResult',
                ...
                if (resultAction) {
                    ...
                    if (!stagedEvent)
                        handlers.current.onReadOnlyViolation?.();
                    return;
                }
                ...
                if (['helperScriptChoices', 'helperScriptDiagnostic', 'helperScriptReady', 'helperMessageEditApplied', 'helperScriptLibraryApplied'].includes(value.action))
                    return;
            }
            if (helperRecord(value) && value.source === 'dsh-tavern-card' && value.action === 'helperDisplayGuardResult' && typeof value.requestId === 'string') {
                ...
                guard.resolve({ check, commit: check, cancel: () => { if (held) {
                        ...
                        if (!heldGuards.size)
                            eventEndpoint?.setHostEventsEnabled(true);
                        unlock(id);
                    } } });
                return;
            }
    ...
    const frameStyle = frameH != null
        ...
        : props.compact
            ? { height: 48, minHeight: 0, overflow: 'auto' }
            : props.widget
 ...
 */
function DisplayProjection(props) {
    ...
    const visibleParts = useMemo(() => {
        ...
        if (!parts.length)
            parts.push({ kind: 'markdown', text: text || ' ' });
        return parts;
    ...
    const cycleRef = useRef({ seen: new Set(), done: false, active: true });
    ...
    const completeDisplay = () => {
        ...
        if (!cycle.active || cycle.done || cycle.seen.size < htmlCount || streaming)
            return;
        cycle.done = true;
        ...
        void (async () => {
            ...
            if (!context || !current())
                throw new HelperDisplayError('stale');
            await emitHelperHostEvent(sessionId, context.storyId, context.currentMessageRole === 'user' ? 'user_message_rendered' : 'character_message_rendered', context.currentMessageRole === 'user' ? [context.currentMessageId] : [context.currentMessageId, 'normal'], current);
        })().then(() => {
            if (current())
    ...
    useEffect(() => { if (!htmlCount)
        completeDisplay(); }, [htmlCount, streaming]);
    ...
    useLayoutEffect(() => { if (props.publishing)
        props.onPublished?.(); }, [props.publishing, props.onPublished]);
    const content = visibleParts.map((part, index) => {
        if (part.kind === 'markdown')
            return _jsx(MarkdownText, { text: part.text, streaming: Boolean(streaming), labels: markdownLabels, fileMentions: props.fileMentions }, `text:${index}`);
        ...
        const frame = _jsx(SpeechHtmlFrame, { onFrameReady: () => { const cycle = cycleRef.current; cycle.seen.add(index); completeDisplay(); }, registerDisplayGuard: props.registerDisplayGuard, srcDoc: srcDoc, title: part.title || props.name, widget: widget, compact: Boolean(!streaming && value.parts) && !fullDocumentCover, readOnly: props.staging || props.refreshing, onReadOnlyViolation: props.staging ? () => props.onFailure(new HelperDisplayError('stale')) : undefined, helperBinding: value.helper ? { sessionId, storyId: value.helper.storyId } : undefined, onSwipeGreeting: props.onSwipe, onMessageBranch: props.onMessageBranch, onMessageEdit: props.messageId === undefined ? undefined : async (request) => {
                const result = await remote.editHelperMessages({ ...request, sessionId, messageId: props.messageId });
                if (!result.ok)
                    throw new Error(result.error.message);
                if (result.value.snapshot)
                    notifyHelperStory(sessionId, result.value.snapshot.storyId);
                return result.value;
                ...
                if (!value.helper)
                    throw new Error(t('speech.helperUnsupported'));
        ...
        return part.title
            ? _jsxs("details", { className: "dsh-tavern-reason", children: [_jsx("summary", { children: part.title }), frame] }, `fold:${index}`)
            : frame;
    });
    return _jsx("div", { className: `dsh-tavern-displayProjection${props.staging ? ' is-staging' : ''}`, "aria-hidden": props.staging || undefined, children: content });
}
function SpeechBubbleSession(props) {
    ...
    const loaded = useLoader(() => remote.renderOutputText({ sessionId, text: rawText, messageId: props.messageId }), [sessionId, rawText, props.messageId, props.interactiveCards, props.bindingRevision], Boolean(rawText) && !streaming);
    const knownCharacterRevision = useRef(undefined);
    useEffect(() => {
        ...
        if (knownCharacterRevision.current === undefined) {
            knownCharacterRevision.current = revision;
            return;
        }
        if (knownCharacterRevision.current === revision)
            return;
        knownCharacterRevision.current = revision;
        avatar.reload();
        loaded.reload();
    ...
    const rendered = { state: useMemo(() => activeOverride
            ? { status: 'ready', value: activeOverride.value }
    ...
    useEffect(() => {
        ...
        const stop = registerHelperDisplay(sessionId, async (request, signal) => {
            ...
            if (currentContext && currentContext.storyId !== request.storyId)
                throw new HelperDisplayError('stale');
            ...
            try {
                ...
                const verified = nextContext ? { ok: true, value: nextContext } : await waitHelperDisplay(remote.getHelperSnapshot({ sessionId, messageId: props.messageId }), signal);
                ...
                const check = () => { if (!alive || renderLatest.current !== current)
                    ...
                    lease.check(); };
                check();
                let resolve, reject, committed = false, record;
                const after = new Promise((yes, no) => { resolve = yes; reject = no; });
                void after.catch(() => { });
                const release = () => { for (const lease of leases)
                    lease.cancel(); };
                const abort = (error, report = committed) => {
                    if (record.settled)
                        return;
                    record.settled = true;
                    ...
                    if (stageRef.current === record) {
                        ...
                        if (alive)
                            setStage(previous => previous?.id === record.id ? null : previous);
                    }
                    reject(error);
                    if (report && alive)
                        setLifecycleError(displayFailure(error, t));
                };
                record = { id: ++projectionSerial.current, base: loaded.state, value: next.value, phase: 'preparing', after, settled: false,
                    abort,
                    ready: () => {
                        if (record.settled || stageRef.current !== record || record.phase !== 'preparing')
                            return;
                        try {
                            check();
                    ...
                    published: () => {
                        if (record.settled || stageRef.current !== record || record.phase !== 'publishing')
                            return;
                        record.settled = true;
                        clearTimeout(record.timer);
                        setDisplay({ base: record.base, value: record.value, id: record.id });
                        stageRef.current = null;
                        setStage(null);
                        resolve();
                    } };
                return { check, after: () => after, current: () => alive && renderLatest.current.status === 'ready' && renderLatest.current.value === next.value,
                    cancel: () => abort(new HelperDisplayError('stale'), false), commit: () => {
                        ...
                        if (stageRef.current)
                            throw new HelperDisplayError('busy');
                        committed = true;
                        record.timer = setTimeout(() => abort(new HelperDisplayError('timeout'), true), 60000);
                        stageRef.current = record;
        ...
        return () => { alive = false; const pending = stageRef.current; if (pending?.base === loaded.state)
            pending.abort(new HelperDisplayError('stale'), false); stop(); };
    ...
    const registerDisplayGuard = (guard) => {
        displayGuards.current.add(guard);
        return () => { displayGuards.current.delete(guard); };
    };
    ...
    if (rendered.state.status === 'ready' && validStage?.phase !== 'publishing') {
        ...
        projectionNodes.push(_jsx(DisplayProjection, { ...props, value: value, staging: false, refreshing: loaded.state.status !== 'ready', registerDisplayGuard: registerDisplayGuard, isCurrent: () => !stageRef.current && renderLatest.current.status === 'ready' && renderLatest.current.value === value, onComplete: () => { }, onFailure: error => setLifecycleError(displayFailure(error, t)), onSwipe: index => void swipeGreeting(index) }, `display:${activeProjectionId}`));
    }
    else if (rendered.state.status !== 'ready') {
        projectionNodes.push(_jsx(MarkdownText, { text: text || ' ', streaming: Boolean(streaming), labels: markdownLabels, fileMentions: props.fileMentions }, `fallback:${baseProjectionId}`));
    }
    if (validStage) {
        const pending = stageRef.current;
        projectionNodes.push(_jsx(DisplayProjection, { ...props, value: validStage.value, staging: validStage.phase === 'preparing', publishing: validStage.phase === 'publishing', registerDisplayGuard: registerDisplayGuard, isCurrent: () => Boolean(pending && !pending.settled && stageRef.current === pending), onComplete: () => pending?.ready(), onFailure: error => pending?.abort(error, true), onPublished: () => pending?.published(), onSwipe: index => void swipeGreeting(index) }, `display:${validStage.id}`));
    }
    return (_jsxs("div", { className: "dsh-tavern-speech dsh-tavern-rise", children: [_jsx(Avatar, { url: avatarUrl, name: name, size: 40, className: "dsh-tavern-speechAvatar" }), _jsxs("div", { className: "dsh-tavern-speechBody", children: [_jsx("div", { className: "dsh-tavern-speechName", children: name }), _jsx(Err, { message: swipeError }), _jsx(Err, { message: lifecycleError }), loaded.state.status === 'error' && _jsxs("div", { children: [_jsx(Err, { message: t('speech.renderFailed', { error: loaded.state.message }) }), _jsx(Btn, { onClick: loaded.reload, children: t('speech.retryRender') })] }), regexDiagnostics && _jsxs("details", { className: "dsh-tavern-reason", children: [_jsx("summary", { children: t('speech.regexFailures', { count: regexDiagnostics.total }) }), _jsx("ul", { children: regexDiagnostics.errors.map((error, index) => _jsxs("li", { children: [error.ruleName || error.ruleId, ": ", error.message] }, index)) }), regexDiagnostics.total > regexDiagnostics.errors.length && _jsx("div", { children: t('speech.regexFailuresMore', { count: regexDiagnostics.total - regexDiagnostics.errors.length }) })] }), projectionNodes, props.media, !streaming && loaded.state.status === 'ready' && rendered.state.status === 'ready' && (rendered.state.value.helper ?? rendered.state.value.helperContext) && _jsx(ScriptChoices, { sessionId: sessionId, context: (rendered.state.value.helper ?? rendered.state.value.helperContext) })] }), _jsx("div", { className: "dsh-tavern-speechCopy", children: _jsx(IconBtn, { label: t('speech.copy'), onClick: () => void onCopy(), children: _jsx(IconCopyOutlineMedium, {}) }) }), toast.node] }));
}
