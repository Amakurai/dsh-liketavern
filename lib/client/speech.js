...
function DisplayProjection(props) {
    ...
    const content = visibleParts.map((part, index) => {
        ...
        const frame = _jsx(SpeechHtmlFrame, { onFrameReady: () => { const cycle = cycleRef.current; cycle.seen.add(index); completeDisplay(); }, registerDisplayGuard: props.registerDisplayGuard, srcDoc: srcDoc, title: part.title || props.name, widget: widget, compact: Boolean(storedParts) && !fullDocumentCover, readOnly: props.staging, onReadOnlyViolation: props.staging ? () => props.onFailure(new HelperDisplayError('stale')) : undefined, helperBinding: value.helper ? { sessionId, storyId: value.helper.storyId } : undefined, onSwipeGreeting: props.onSwipe, onMessageBranch: props.onMessageBranch, onMessageEdit: props.messageId === undefined ? undefined : async (request) => {
