...
export function NumInput(props) {
    const label = useContext(ControlLabel);
    return (_jsx("input", { type: "number", "aria-labelledby": label?.labelId, "aria-describedby": label?.descriptionId, className: "dsh-tavern-input", style: { width: props.width ?? 90 }, value: Number.isFinite(props.value) ? props.value : 0, step: props.step ?? 'any', onChange: (e) => {
