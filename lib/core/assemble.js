...
export function assemblePrompt(input) {
    ...
    const wiMessages = (pos, role) => {
        ...
        if (turn)
            out.push(asTurn(trackedMessage(role, turn, worldInfoPromptMessages)));
        return out;
    };
    const wiText = (pos) => {
        const { standing, turn } = wiChunks(pos);
    ...
    const markerContent = (id, role) => {
        switch (id) {
            ...
            case Marker.DialogueExamples: {
                ...
                if (before.turn)
                    out.push(asTurn(trackedMessage(role, before.turn, worldInfoPromptMessages)));
                ...
                if (after.turn)
                    out.push(asTurn(trackedMessage(role, after.turn, worldInfoPromptMessages)));
                return out;
            }
