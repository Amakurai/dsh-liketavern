...
export function resolveTavernReasoningEffort(thinking, reasoning, current, provider) {
    ...
    if (picked !== undefined)
        return picked;
    if (thinking === 'disabled' && provider !== undefined && DEEPSEEK_OFFICIAL_PROVIDERS.has(provider))
        return 'off';
 ...
 */
export function mergeTavernCallConfig(config, sampling, reasoningEffort, hostMaxTokens) {
    const merged = {
        ...config,
