...
export function parseHelperWorldbook(input) {
    const data = helperJson(input, HELPER_WORLDBOOK_MAX_BYTES);
    if (!Array.isArray(data) || data.length > 2000)
        throw new Error('世界书条目必须是至多 2000 项的数组');
    const ids = new Set(), reserved = new Set();
    for (const value of data)
        if (helperRecord(value) && value.uid !== undefined) {
            if (!Number.isSafeInteger(value.uid) || Number(value.uid) < 0)
                throw new Error('世界书 UID 必须是非负安全整数');
            reserved.add(Number(value.uid));
        }
    let next = 0;
    const record = (value) => { if (value === undefined)
        return {}; if (!helperRecord(value))
        throw new Error('世界书设置必须为对象'); return value; };
    const text = (value, fallback = '', max = 100000) => { if (value === undefined)
        return fallback; if (typeof value !== 'string' || value.length > max)
        throw new Error('世界书文本字段类型无效或超出预算'); return value; };
    const bool = (value, fallback) => { if (value === undefined)
        return fallback; if (typeof value !== 'boolean')
        throw new Error('世界书开关必须为布尔值'); return value; };
    const number = (value, fallback, min = 0, max = Number.MAX_SAFE_INTEGER) => { if (value === undefined)
        return fallback; if (typeof value !== 'number' || !Number.isFinite(value) || value < min || value > max)
        throw new Error('世界书数值字段无效'); return value; };
    const nullable = (value) => value == null ? null : number(value, 0);
    const choice = (value, values, fallback) => { if (value === undefined)
        return fallback; if (!values.includes(value))
        throw new Error('世界书枚举字段无效'); return value; };
    const keys = (value) => { if (value === undefined)
        return []; if (!Array.isArray(value) || value.length > 2000)
        throw new Error('世界书关键字列表无效'); return value.map(item => text(item, '', 500)); };
    return data.map(value => {
        ...
        while (reserved.has(next))
            next++;
        const uid = item.uid === undefined ? next++ : Number(item.uid);
        reserved.add(uid);
        if (ids.has(uid))
            throw new Error('世界书 UID 重复');
        ids.add(uid);
