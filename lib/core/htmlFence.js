...
export function findHtmlFence(text) {
    ...
    for (let token = tokens.exec(text); token; token = tokens.exec(text)) {
        ...
        if (raw.startsWith('<!--'))
            continue;
        if (token[1]) {
            const close = new RegExp('</' + token[1] + '\\s*>', 'gi');
        ...
        if (raw.length < 3 || !/^ {0,3}$/.test(line)) {
            if (raw[0] === '`') {
                const close = new RegExp('(?<!`)`{' + raw.length + '}(?!`)', 'g');
                close.lastIndex = tokens.lastIndex;
                if (close.exec(text))
                    tokens.lastIndex = close.lastIndex;
            }
        ...
        if (!closing)
            return null;
        tokens.lastIndex = close.lastIndex;
        if (!['', 'html', 'text', 'xml'].includes(info[1].trim().toLowerCase()))
            continue;
        const content = text.slice(contentStart, closing.index), trimmed = content.trim();
        if (!findHtmlDocument(trimmed) && !findHtmlFragment(trimmed))
            continue;
