 ...
 */
export const STANDING_PIN_VERSION = 22;
/** 场景值归一化：空/缺省视为 normal（与 assemble.ts 的 generationType 缺省一致）。 */
export function normalizeGenerationType(generationType) {
    const value = (generationType ?? 'normal').trim().toLowerCase();
    return value || 'normal';
}
/** 钉死键：会话 × 生成场景。 */
export function standingPinKey(sessionId, generationType) {
    return `${sessionId}\0${normalizeGenerationType(generationType)}`;
}
export function standingFingerprint(binding, persona, 
 ...
 */
export function stableFingerprintHash(value) {
...
export function pinStandingText(pins, sessionId, fingerprint, computed) {
