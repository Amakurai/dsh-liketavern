...
export function undoWorldDelta(before, after, current) {
    ...
    const byId = (text) => new Map(lines(text).flatMap((line) => {
        const id = idOf(line);
    ...
    for (const id of new Set([...previous.keys(), ...written.keys()])) {
        if (previous.get(id) !== written.get(id) && actual.get(id) === written.get(id))
            restore.set(id, previous.get(id));
    }
    const result = [];
    for (const line of lines(current)) {
        const id = idOf(line);
        if (id === null || !restore.has(id))
            result.push(line);
        else {
    ...
    for (const line of restore.values())
        if (line !== undefined)
            result.push(line);
