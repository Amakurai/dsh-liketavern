...
function matchEntry(entry, texts, settings, macroCtx) {
    const opts = {
...
function applyInclusionGroups(activated, settings, random, log) {
    const ungrouped = [];
    ...
    for (const [group, members] of byGroup) {
        ...
        kept.push(winner);
        for (const member of members) {
            if (member.entry.key === winner.entry.key)
                continue;
...
function pickGroupByScore(pool) {
...
export function evaluateWorldInfo(input) {
    ...
    for (const candidate of activated) {
        if (keptKeys.has(candidate.entry.key))
            continue;
        // 落选条目本轮才写入的 sticky/cooldown 必须清掉，否则下一轮会经 sticky 占组。
        if (!stickyKeysAtStart.has(candidate.entry.key))
    ...
    const rawLimit = settings.tokenBudget > 0
