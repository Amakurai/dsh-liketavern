...
class IssueCounter {
    issues = new Map();
    add(code, severity, count = 1) {
        const current = this.issues.get(code);
        if (current)
            current.count += count;
        else
            this.issues.set(code, { code, severity, count });
    }
    values() {
 ...
 */
async function readBoundedText(path, maxBytes, sharedBudget) {
    let before;
    try {
    ...
    try {
        ...
        while (position <= maxBytes) {
            ...
            if (bytesRead === 0)
                return { status: 'ok', text: Buffer.concat(chunks, position).toString('utf8') };
...
async function readBoundedJson(path, maxBytes = MAX_JSON_BYTES, budget) {
    const read = await readBoundedText(path, maxBytes, budget);
    if (read.status !== 'ok')
        return read;
    try {
        return { status: 'ok', value: JSON.parse(read.text) };
    }
    catch {
        return { status: 'invalid' };
    }
}
async function probeDirectory(path, boundary) {
...
async function readDirectoryEntries(path, budget) {
    if (budget.remaining <= 0) {
        budget.exhausted = true;
        return [];
    }
...
function validStoryMetadata(value, id) {
...
async function validateWalArtifacts(directory, budget) {
    const recordsRead = await readBoundedText(`${directory}/records.jsonl`, MAX_WAL_BYTES, budget);
    if (recordsRead.status === 'oversized' || recordsRead.status === 'budget')
        return 'uninspected';
    if (recordsRead.status === 'unsafe' || recordsRead.status === 'invalid' || recordsRead.status === 'unreadable')
        return 'unhealthy';
    const records = [];
    if (recordsRead.status === 'ok') {
        ...
        for (const line of recordsRead.text.split('\n')) {
            ...
            if (!isObject(record) || !safeWalPath(record.path) || !Number.isSafeInteger(record.seq)
                || record.seq < 1 || record.seq <= previous
                ...
                || (record.afterEncoding !== undefined && record.afterEncoding !== 'utf8' && record.afterEncoding !== 'base64'))
                return 'unhealthy';
            ...
            if ((before !== null && (record.beforeEncoding === 'base64' || before !== record.before) && !isBase64(before))
                || (record.afterEncoding === 'base64' && record.after !== null && !isBase64(record.after)))
                return 'unhealthy';
            previous = record.seq;
            records.push(record);
        }
    ...
    if (progressRead.status === 'missing')
        return 'healthy';
    if (progressRead.status === 'oversized' || progressRead.status === 'budget')
        return 'uninspected';
    if (progressRead.status !== 'ok' || !isObject(progressRead.value))
        return 'unhealthy';
    ...
    if (progress.hash !== hash || !Number.isSafeInteger(progress.next) || progress.next < -1
        || progress.next >= records.length || !isPaths(progress.restored) || !isPaths(progress.preserved)
        || progress.restored.length + progress.preserved.length !== records.length - progress.next - 1)
        return 'unhealthy';
    if ('pending' in progress) {
        const pending = progress.pending;
        if (!isObject(pending) || typeof pending.path !== 'string' || pending.path !== records[progress.next]?.path
            || !(pending.from === null || isBase64(pending.from)) || !(pending.to === null || isBase64(pending.to)))
            return 'unhealthy';
    }
    return 'healthy';
}
async function inspectWalRoot(root, stats, issues, budget, scanBudget) {
    ...
    let entries;
    try {
        entries = await readDirectoryEntries(root, scanBudget);
    }
    catch {
        issues.add('WAL_ROOT_UNREADABLE', 'error');
        return;
    ...
    if (stats.pending > pendingBefore)
        issues.add('WAL_UNCOMMITTED', 'warning', stats.pending - pendingBefore);
}
async function inspectStories(cardRoot, stats, issues, budget, scanBudget) {
    ...
    let entries;
    try {
        entries = await readDirectoryEntries(storiesRoot, scanBudget);
    }
    catch {
    ...
    for (const entry of entries) {
        ...
        let root;
        try {
            root = storyRoot(cardRoot, entry.name);
        }
        catch {
    ...
    if (stats.stories.drafts > draftsBefore)
        issues.add('STORY_DRAFT_PRESENT', 'info', stats.stories.drafts - draftsBefore);
}
async function inspectCharacters(charactersRoot, stats, issues, budget, scanBudget) {
    let entries;
    try {
        entries = await readDirectoryEntries(charactersRoot, scanBudget);
    }
    catch {
    ...
    for (const entry of entries) {
        ...
        if (!healthy)
            issues.add(card.status === 'oversized' ? 'CHARACTER_METADATA_TOO_LARGE' : 'CHARACTER_METADATA_INVALID', 'error');
        ...
        if (archive.status === 'ok') {
            if (validArchiveMetadata(archive.value))
                stats.characters.archived += 1;
            else {
                healthy = false;
...
async function inspectFiles(root, stats, issues, budget, scanBudget) {
    const pending = [{ path: root, depth: 0 }];
    while (pending.length > 0 && !scanBudget.exhausted && !budget.exhausted) {
        const current = pending.pop();
        let entries;
        try {
            entries = await readDirectoryEntries(current.path, scanBudget);
        }
        catch {
            stats.unreadable += 1;
        ...
        try {
            for (const entry of entries) {
                ...
                if (entry.name.toLowerCase().endsWith('.json')) {
                    ...
                    if (json.status === 'oversized')
                        stats.oversizedJson += 1;
    ...
    if (stats.skippedLinks > 0)
        issues.add('FILE_LINK_SKIPPED', 'warning', stats.skippedLinks);
    if (stats.invalidJson > 0)
        issues.add('JSON_SYNTAX_INVALID', 'error', stats.invalidJson);
    if (stats.oversizedJson > 0)
        issues.add('JSON_VALIDATION_LIMIT_EXCEEDED', 'warning', stats.oversizedJson);
    if (stats.unreadable > 0)
        issues.add('FILE_UNREADABLE', 'error', stats.unreadable);
}
...
async function installedDshVersion() {
    try {
...
export function areDshVersionsCompatible(installedVersion, expectedVersion) {
...
export async function runDoctor(options = {}) {
    ...
    if (!nodeSupported)
        issues.add('NODE_VERSION_UNSUPPORTED', 'error');
    ...
    if (pluginVersion === 'unknown')
        issues.add('PLUGIN_VERSION_UNKNOWN', 'warning');
    ...
    if (dshExpectedVersion === 'unknown')
        issues.add('DSH_EXPECTED_VERSION_UNKNOWN', 'warning');
    if (dshInstalledVersion === 'unknown')
        issues.add('DSH_INSTALLED_VERSION_UNKNOWN', 'warning');
    ...
    const budget = { remaining: budgetLimit, exhausted: false };
    ...
    const scanBudget = { remaining: entryLimit, exhausted: false };
    const home = resolveDshHome(options.home, options.env ?? process.env);
    const homeDisplay = dshHomeDisplay(home);
    ...
    const dataRootStatus = homeStatus === 'directory'
    ...
    if (dataRootStatus === 'missing') {
        ...
        for (const [name, path] of Object.entries(names)) {
            ...
            if (directories[name] === 'missing')
                issues.add('STANDARD_DIRECTORY_MISSING', 'warning');
            else if (directories[name] !== 'directory')
                issues.add('STANDARD_DIRECTORY_INVALID', 'error');
        }
        if (directories.characters === 'directory')
            await inspectCharacters(paths.characters, statistics, issues, budget, scanBudget);
...
export function formatDoctorText(report) {
    ...
    for (const issue of report.issues)
        lines.push(`issue: severity=${issue.severity} code=${issue.code} count=${issue.count}`);
    return lines.join('\n') + '\n';
}
export function parseDoctorArguments(args) {
    const parsed = { json: false, help: false, version: false };
...
export async function doctorMain(args = process.argv.slice(2), io = {
    ...
    let parsed;
    try {
        parsed = parseDoctorArguments(args);
    }
    catch {
