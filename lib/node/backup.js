...
import { BACKUP_LIMITS, BackupError, assertAbsent, assertDirectory, assertSeparate, boundedJson, checkedPath, createDirectories, discardTemporary, fail, inventory, isDependencyPath, safeRelative, transferFile } from './backupFiles.js';
function object(value) { return value !== null && typeof value === 'object' && !Array.isArray(value); }
function version(value) { return typeof value === 'string' && /^(?:unknown|\d+\.\d+\.\d+(?:[-+][\w.-]+)?)$/.test(value); }
export function parseBackupManifest(raw) {
    if (!object(raw) || raw.schemaVersion !== 1 || raw.format !== 'dsh-tavern-directory-backup'
        ...
        || !['node', 'plugin', 'dsh', 'expectedDsh'].every(key => version(raw.versions[key]))
        || !Array.isArray(raw.entries) || raw.entries.length > BACKUP_LIMITS.entries || !Array.isArray(raw.exclusions))
    ...
    for (const value of raw.entries) {
        ...
        entries.push({ path: value.path, kind: value.kind, size: value.size, mode: value.mode,
            ...(value.kind === 'file' ? { sha256: value.sha256 } : {}) });
    }
    for (const entry of entries) {
        const parts = entry.path.split('/');
        for (let n = 1; n < parts.length; n++)
            if (!directories.has(parts.slice(0, n).join('/')))
                fail('INVALID_MANIFEST');
    }
    const exclusions = [];
    for (const value of raw.exclusions) {
        if (!safeRelative(value) || !isDependencyPath(value) || exclusions.includes(value)
            || seen.has(value.normalize('NFC').toLocaleLowerCase('en-US')) || entries.some(entry => entry.path.startsWith(value + '/')))
            fail('INVALID_MANIFEST');
        exclusions.push(value);
    }
    return { schemaVersion: 1, format: 'dsh-tavern-directory-backup', scope: 'dsh-home-data-without-installed-dependencies',
        createdAt: raw.createdAt, versionSource: 'backup-runtime', versions: raw.versions, exclusions: exclusions.sort(),
        entries: entries.sort((a, b) => a.path < b.path ? -1 : a.path > b.path ? 1 : 0) };
}
function report(operation, manifest, offlineDeclared, issues) {
...
async function references(home, entries) {
    const files = new Map(entries.filter(entry => entry.kind === 'file').map(entry => [entry.path, entry]));
    const paths = new Set(files.keys());
    const issues = new Map();
    const add = (code) => { const found = issues.get(code); if (found)
        found.count++;
    else
        issues.set(code, { code, severity: 'error', count: 1 }); };
    let budget = 64 * 1024 ** 2;
    for (const entry of entries) {
        if (entry.kind !== 'file' || !/^dsh-tavern\/sessions\/[^/]+\.json$/.test(entry.path))
            continue;
        budget -= entry.size;
        if (budget < 0)
            fail('REFERENCE_CHECK_LIMIT');
        let binding;
        try {
            binding = parseSessionBinding(await boundedJson(checkedPath(home, entry.path), 1024 * 1024));
        }
        catch {
            add('BINDING_INVALID');
            continue;
        ...
        if (expected !== entry.path)
            add('BINDING_IDENTITY_INVALID');
        const card = `dsh-tavern/characters/${binding.cardId}`;
        if (!paths.has(`${card}/card.json`))
            add('BINDING_CARD_MISSING');
        if (binding.storyId) {
            const story = `${card}/stories/${binding.storyId}/story.json`;
            if (!paths.has(story))
                add('BINDING_STORY_MISSING');
            else {
                budget -= files.get(story).size;
                if (budget < 0)
                    fail('REFERENCE_CHECK_LIMIT');
                const metadata = await boundedJson(checkedPath(home, story), 1024 * 1024);
                if (!object(metadata) || metadata.sessionId !== binding.sessionId)
                    add('BINDING_STORY_IDENTITY_INVALID');
            }
        }
        for (const [id, directory] of [[binding.presetId, 'library/presets'], [binding.personaId, 'personas'],
            ...[...binding.lorebookIds, ...(binding.characterLorebookIds ?? []), binding.characterLorebookId].map(id => [id, 'library/lorebooks'])]) {
            if (id && !paths.has(`dsh-tavern/${directory}/${id.replace(/[^A-Za-z0-9_一-鿿.-]/g, '_')}.json`))
                add('BINDING_ASSET_MISSING');
        }
    }
    return [...issues.values()];
}
async function inspect(home, entries) {
    ...
    if (doctor.statistics.files.scanTruncated || doctor.statistics.characters.uninspected || doctor.statistics.stories.uninspected
        || doctor.statistics.wal.uninspected || doctor.statistics.files.oversizedJson)
        fail('STRUCTURE_CHECK_INCOMPLETE');
...
async function checkActivity(home, entries) {
    for (const entry of entries) {
        if (entry.kind !== 'file' || !entry.path.endsWith('.lock') || basename(entry.path) === 'session.lock' || entry.size > 32)
            continue;
        let value;
        try {
            value = await boundedJson(checkedPath(home, entry.path), 32);
        }
        catch (error) {
            if (error instanceof BackupError && error.code === 'INVALID_JSON')
                continue;
            throw error;
        }
...
async function compare(home, expected, exclusions) {
    const observedExclusions = [];
    const actual = await inventory(home, exclusions ? observedExclusions : undefined);
    if (exclusions && JSON.stringify([...exclusions].sort()) !== JSON.stringify(observedExclusions.sort()))
        fail('SOURCE_CHANGED');
    if (actual.length !== expected.length)
        fail('INVENTORY_MISMATCH');
    for (let i = 0; i < actual.length; i++) {
        const a = actual[i], b = expected[i];
        if (a.path !== b.path || a.kind !== b.kind || a.size !== b.size || (process.platform !== 'win32' && a.mode !== b.mode))
            fail('INVENTORY_MISMATCH');
        if (a.kind === 'file' && await transferFile(home, b) !== b.sha256)
            fail('HASH_MISMATCH');
    }
}
async function copy(home, target, entries) {
    ...
    for (const entry of [...entries].reverse())
        if (entry.kind === 'directory')
            await chmod(checkedPath(target, entry.path), entry.mode);
    return result;
}
async function stage(target, action, hooks) {
    ...
    try {
        ...
        if (dirname(resolve(temporary)) !== dirname(resolve(target)))
            fail('UNSAFE_CLEANUP');
...
async function readBackup(backup) {
    await assertDirectory(backup);
    if (JSON.stringify((await readdir(backup)).sort()) !== JSON.stringify(['data', 'manifest.json']))
...
export async function createBackup(options) {
    if (!options.offline)
        fail('OFFLINE_DECLARATION_REQUIRED');
    if (!options.home || !options.backup)
        fail('EXPLICIT_PATH_REQUIRED');
    ...
    await stage(backup, async (temporary) => {
        ...
        await compare(home, saved, exclusions);
        await checkActivity(home, entries);
...
export async function restoreBackup(options) {
    ...
    await stage(target, async (temporary) => {
        ...
        if (JSON.stringify(copied) !== JSON.stringify(manifest.entries))
            fail('HASH_MISMATCH');
        await options.hooks?.afterCopy?.();
