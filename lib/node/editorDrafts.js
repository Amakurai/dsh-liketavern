...
function validateValue(value) {
    ...
    function count(bytes) {
        budget += bytes;
        if (budget > EDITOR_DRAFT_MAX_BYTES)
            invalid('大小不能超过 2 MiB');
    }
    function visit(item, depth) {
        ...
        if (typeof item === 'string') {
            count(Buffer.byteLength(item, 'utf8') + 2);
            return;
        }
        if (typeof item === 'number' && Number.isFinite(item)) {
            count(String(item).length);
            return;
        }
        if (!item || typeof item !== 'object' || (!Array.isArray(item) && !plainObject(item)))
            invalid('只能包含 JSON 数据');
        if (active.has(item))
            invalid('不能包含循环引用');
        if (Object.getOwnPropertySymbols(item).length)
            invalid('不能包含 Symbol 字段');
        active.add(item);
        count(2);
...
function draftPath(root, owner, key) {
    if (typeof owner !== 'string' || !/^[A-Za-z0-9_-]{8,80}$/.test(owner))
        invalid('浏览器标识格式错误');
    if (typeof key !== 'string' || key.length < 1 || key.length > 300 || key.includes('\0'))
        invalid('编辑器标识格式错误');
    const hash = (input) => createHash('sha256').update(input, 'utf8').digest('hex');
    return join(root, 'editor-drafts', hash(owner), `${hash(key)}.json`);
}
export async function getEditorDraft(root, owner, key) {
    ...
    return withWorkspaceLock(path, async () => {
        ...
        try {
            ...
            if (size > EDITOR_DRAFT_MAX_BYTES)
                invalid('已保存草稿大小超过 2 MiB');
            ...
            if (!plainObject(record) || record.version !== 1 || typeof record.updatedAt !== 'string' || !Number.isFinite(Date.parse(record.updatedAt)))
                invalid('已保存草稿格式错误');
...
export async function saveEditorDraft(root, owner, key, value) {
    const path = draftPath(root, owner, key);
    validateValue(value);
    const serialized = JSON.stringify({ version: 1, updatedAt: new Date().toISOString(), value });
    if (Buffer.byteLength(serialized, 'utf8') > EDITOR_DRAFT_MAX_BYTES)
        invalid('大小不能超过 2 MiB');
    await withWorkspaceLock(path, () => atomicWrite(path, serialized));
}
export async function deleteEditorDraft(root, owner, key) {
    const path = draftPath(root, owner, key);
