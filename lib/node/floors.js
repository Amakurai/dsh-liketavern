...
export function sessionPrefixEvents(source, boundaryInclusive) {
 ...
 */
async function forkChildSession(ctx, source, seed) {
    const childId = newChildId();
    const presets = agentPresetsOf(ctx);
    const parent = ctx.agents.get(source.id);
    const named = (parent ? presets.composedPreset(parent.ctx) : undefined) ?? resolveSessionPreset(source) ?? 'tavern';
...
async function rollbackFloors(state, cardId, binding, sessionId, fromTurn) {
    const ws = await state.workspace(cardId);
    const names = floorNamesForLineageRollback((await ws.wal.listFloors()).map((floor) => floor.floor), binding, sessionId, fromTurn);
    if (names.length === 0)
        return [];
...
async function copyTimers(state, cardId, binding, currentSessionId, toSession, boundaryTurn) {
...
export async function regenerate({ ctx, state }, sessionId, messageId) {
...
export async function rollbackToFloor({ ctx, state }, sessionId, messageId) {
    const source = liveSession(ctx, sessionId);
    if (!source)
...
export async function editUserMessage(deps, sessionId, messageId, newText) {
    ...
    if (!firstUserMessageOf(source.events, turn))
...
export function withEditedAssistantMessage(events, messageId, newText) {
    ...
    const replaced = { ...event, data: { ...rest, message } };
...
export async function getFloorAssistantMessage({ ctx }, sessionId, messageId) {
    ...
    if (turn === null)
        throw new FloorError('no-message', '这条消息不在当前会话中（可能已过期）');
    const hit = source.events.find((e) => e.type === 'assistant/message' && e.data.message?.id === messageId);
...
function syncIdleAgentLastTurn(ctx, sessionId) {
    ...
    if (!phase || phase.kind !== 'idle' || typeof phase.lastTurn !== 'number')
        return;
    phase.lastTurn = 1;
}
/**
...
export function retireGreetingOnlyBlankSession(session, ctx) {
    if (!isGreetingOnlyBlank(session.events))
        return false;
 ...
 */
export async function getFloorSiblings({ ctx, state }, sessionId, messageId) {
...
export async function enterGreetingConversation({ ctx, state }, sessionId) {
    ...
    session.append('assistant/message', { turn: 1, step: 1, message: greetingMessage(text) }, { surfaceOp: 'append', sourceEventSeqs: [] });
