...
export async function getHelperEventState(ctx, state, request) {
    ...
    return withWorkspaceLock(ws.fs.root, () => withWorkspaceLock(state.paths.sessions, async () => {
        ...
        if (!binding || binding.cardId !== initial.cardId || binding.storyId !== initial.storyId || request.storyId !== undefined && request.storyId !== binding.storyId)
            throw new Error('事件剧情绑定已经改变');
        if (!state.config.interactiveCards || binding.interactiveCards === false)
            throw new Error('当前剧情未启用交互卡');
        ...
        if (request.closedSeq !== undefined) {
            ...
            if (ending?.type !== 'turn/end')
                throw new Error('事件目标不是已结束的宿主轮次');
            const floor = request.sessionId + '#t' + ending.data.turn, receipt = state.helperTurnClosures.get(floor);
            if (!receipt || receipt.seq !== ending.seq || receipt.storyId !== binding.storyId)
                throw new Error('事件轮次尚无可用的剧情收口回执');
            if (receipt.error !== undefined)
                throw new Error('剧情收口失败：' + receipt.error);
            const meta = await ws.wal.validateFloor(floor);
            if (!meta.committed)
                throw new Error('事件楼层尚未完成 WAL 提交');
            const generation = (await loadTemplateState(ws.fs)).generation;
            if (generation?.sessionId === request.sessionId && generation.turn === ending.data.turn && generation.status === 'prepared')
                throw new Error('事件轮次模板尚未收口');
        }
