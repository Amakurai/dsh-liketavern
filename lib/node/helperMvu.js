...
import { helperMvuData, parseHelperMvuState } from '../core/helperMvu.js';
...
const stateHash = (value) => hash(helperJson({ scopes: value.scopes, extras: value.extras, swipes: value.swipes ?? {}, mvu: parseHelperMvuState(value.mvu) }, 4 * 1024 * 1024));
const scope = (identity) => JSON.stringify(['message', identity]);
const enabled = (state, binding) => !!binding?.storyId && binding.helperMvu === true && state.config.interactiveCards && binding.interactiveCards !== false;
const leaseKey = (request) => JSON.stringify([request.sessionId, request.storyId]);
function leaseMap(state) { let map = leases.get(state); if (!map) {
    map = new Map();
    leases.set(state, map);
} for (const [key, value] of map)
    if (value.expires < Date.now())
        map.delete(key); return map; }
export function revokeHelperMvuLease(state, sessionId, storyId) { leases.get(state)?.delete(leaseKey({ sessionId, storyId })); }
function requestIdentity(request) {
    if (!request.sessionId || !request.storyId || !request.runtimeId || request.runtimeId.length > 96)
        throw new Error('自动 MVU 运行时身份无效');
}
function candidates(events, turn, through = Infinity) {
    const steps = new Map();
    for (const event of events) {
        ...
        if (event.data.chunk.type === 'finish')
            status.finishes++;
        status.last = event.seq;
        status.stop = event.data.chunk.type === 'finish' && event.data.chunk.reason.kind === 'stop';
        steps.set(event.data.step, status);
    }
    return events.filter((event) => {
        if (event.type !== 'assistant/message' || event.data.turn !== turn || event.data.interrupted || event.seq >= through || event.surfaceOp !== undefined && event.surfaceOp !== 'append')
            return false;
        if (!event.data.message.content.some(block => block.type === 'text' && block.text.trim()))
            return false;
        if (isTavernGreetingEvent(event))
            return true;
        const status = steps.get(event.data.step);
        return !!status && status.finishes === 1 && status.stop && status.last < event.seq;
    });
}
function makeJob(event, kind, floor) {
...
function verifyContinuation(events, job, mvu) {
    let target = job;
    for (let depth = 0; depth < 64; depth++) {
        ...
        if (!continued)
            return;
        const previous = [...events].reverse().find((event) => event.type === 'assistant/message' && event.seq < start.seq
            && (event.surfaceOp === undefined || event.surfaceOp === 'append') && event.data.message.content.some(block => block.type === 'text' && block.text.trim()));
        if (!previous)
            throw new Error('自动 MVU 无法核对续写来源，请重新生成完整回复');
        if (mvu.completed.some(receipt => receipt.identity === String(previous.data.message.id)))
            return;
...
async function seedScripts(state, sessionId, storyId, saved) {
    ...
    for (const script of enabledHelperLibraries(context.libraries)) {
        ...
        if (!Object.hasOwn(saved.scopes, key))
            saved.scopes[key] = helperJson(script.data);
    }
}
function floorOwner(binding, sessionId, job) {
    const suffix = '#t' + job.turn;
    if (!job.floor.endsWith(suffix))
        throw new Error('自动 MVU 楼层与消息轮次不一致');
    const owner = job.floor.slice(0, -suffix.length);
    if (owner !== sessionId && !(binding.walLineage ?? []).some(entry => entry.sessionId === owner && entry.throughTurn >= job.turn))
        throw new Error('自动 MVU 楼层不属于当前剧情或继承世系');
}
async function floorReady(state, binding, sessionId, job, allowBegin = false) {
    ...
    if (known?.rolledBack)
        throw new Error('自动 MVU 目标楼层已回滚');
    const open = state.openFloors.get(sessionId);
    if (open && (open.floor !== job.floor || open.cardId !== binding.cardId || open.storyId !== binding.storyId))
        throw new Error('自动 MVU 不能写入已经开启的新楼层');
    if (active.some(item => {
    ...
    if (known)
        await ws.wal.validateFloor(job.floor);
    else if (allowBegin && job.floor === sessionId + '#t' + job.turn)
        await ws.wal.beginFloor(job.floor);
    else
        throw new Error('自动 MVU 目标 WAL 楼层缺失');
    return { ws, open: !!open };
}
function receiptFloor(floor) { return { floor, turn: Number(floor.slice(floor.lastIndexOf('#t') + 2)) }; }
/** 数据回执必须有可验证 WAL；活动楼层交回 turn/end，已关闭但未提交的楼层继续阻止新轮次。 */
async function unfinalizedReceipt(state, binding, sessionId, mvu) {
    const receipt = mvu.completed.at(-1);
    if (!receipt)
        return false;
    const job = receiptFloor(receipt.floor);
    floorOwner(binding, sessionId, job);
    const ws = await state.storyWorkspace(binding.cardId, binding.storyId), floor = await ws.wal.validateFloor(job.floor);
    if (floor.rolledBack)
        throw new Error('自动 MVU 完成回执楼层已回滚');
    if (floor.committed)
        return false;
    const open = state.openFloors.get(sessionId);
...
async function unregisteredFloor(state, binding, sessionId, mvu) {
    ...
    for (const floor of await ws.wal.listFloors()) {
        if (floor.rolledBack || floor.committed || linked.has(floor.floor) || open?.floor === floor.floor && open.cardId === binding.cardId && open.storyId === binding.storyId)
            continue;
        const match = /^(.*)#t([0-9]+)$/.exec(floor.floor);
        if (!match)
            continue;
        const turn = Number(match[2]), owner = match[1];
        if (owner !== sessionId && !(binding.walLineage ?? []).some(entry => entry.sessionId === owner && entry.throughTurn >= turn))
            continue;
...
async function queue(state, sessionId, session, live) {
    if (session.id !== sessionId)
        throw new Error('自动 MVU 结束来源与会话不一致');
    const binding = await state.loadBinding(sessionId);
    if (!enabled(state, binding))
        return;
    const events = session.snapshotEvents(), boundary = [...events].reverse().find(event => event.type === 'turn/start' || event.type === 'turn/end');
    if (!boundary || !live && (boundary.type !== 'turn/end' || boundary.data.reason.kind !== 'completed'))
        return;
    if (live && boundary.type === 'turn/end')
        return queue(state, sessionId, session, false);
    const turn = boundary.data.turn, open = state.openFloors.get(sessionId);
    if (live && (!open || open.cardId !== binding.cardId || open.storyId !== binding.storyId))
        throw new Error('自动 MVU stop 缺少当前楼层');
    ...
    await withWorkspaceLock(ws.fs.root, () => withWorkspaceLock(state.paths.sessions, async () => {
        ...
        if (!enabled(state, current) || hash(current) !== hash(binding))
            throw new Error('自动 MVU 登记期间剧情绑定已改变');
        const saved = await loadHelperState(ws.fs), mvu = parseHelperMvuState(saved.mvu), floor = open?.floor ?? sessionId + '#t' + turn;
        const selected = mvu.initialized ? items : items.slice(-1);
        let added = false;
        for (const item of selected) {
            const job = makeJob(item, mvu.initialized ? 'update' : 'initialize', floor);
            if (mvu.pending.some(row => row.identity === job.identity) || mvu.completed.some(row => row.identity === job.identity))
                continue;
            if (!mvu.initialized && mvu.pending.length)
                continue;
            if (mvu.pending.length >= 64)
                throw new Error('自动 MVU 待处理任务超过 64 项');
            verifyContinuation(events, job, mvu);
            await floorReady(state, current, sessionId, job);
            if (!added) {
                await seedScripts(state, sessionId, current.storyId, saved);
                added = true;
            }
...
export const queueHelperMvuTurn = (state, sessionId, session) => queue(state, sessionId, session, false);
export const queueHelperMvuStop = (state, sessionId, session) => queue(state, sessionId, session, true);
export async function helperMvuPending(state, sessionId, includeInitialization = true) {
    const binding = await state.loadBinding(sessionId);
    if (!enabled(state, binding))
        return false;
    const ws = await state.storyWorkspace(binding.cardId, binding.storyId);
...
export async function assertHelperMvuWritable(state, sessionId) {
    const binding = await state.loadBinding(sessionId);
    if (!enabled(state, binding))
        return;
    const ws = await state.storyWorkspace(binding.cardId, binding.storyId);
    await withWorkspaceLock(ws.fs.root, async () => { const mvu = parseHelperMvuState((await loadHelperState(ws.fs)).mvu); if (mvu.pending.length || await unfinalizedReceipt(state, binding, sessionId, mvu) || await unregisteredFloor(state, binding, sessionId, mvu))
        throw new Error('自动 MVU 正在更新当前剧情，请等待事务完成'); });
}
async function sources(state, binding) {
    const result = [], known = new Set(await state.listLorebooks()), seen = new Set();
    const add = async (id, role) => {
        if (seen.has(id))
            return;
        if (!known.has(id))
            throw new Error('自动 MVU 初始化世界书不存在：' + id);
        seen.add(id);
    ...
    if (binding.characterLorebookId)
        await add(binding.characterLorebookId, 'character');
    else if (binding.useEmbeddedLorebook !== false) {
        const embedded = await state.loadCharacterLorebookRaw(binding.cardId);
        if (embedded)
            result.push({ name: embedded.name, role: 'character', entries: parseLorebook(embedded.json, { source: 'character', sourceRef: binding.cardId }).map(({ uid, comment, content }) => ({ uid, comment, content })) });
    }
    for (const id of binding.characterLorebookIds ?? [])
        await add(id, 'character');
    helperJson(result, 2 * 1024 * 1024);
    return result;
}
function latestCompleted(events) {
    ...
    return [...events].reverse().find((event) => event.type === 'assistant/message' && !event.data.interrupted && (event.surfaceOp === undefined || event.surfaceOp === 'append')
...
async function locked(state, request, run) {
    ...
    return withWorkspaceLock(ws.fs.root, () => withWorkspaceLock(state.paths.sessions, async () => {
        ...
        if (!binding || binding.storyId !== request.storyId || binding.cardId !== first.cardId)
            throw new Error('自动 MVU 剧情绑定已改变');
        return run(binding);
    }));
}
async function scriptRevision(state, request) { const scripts = await state.getSessionHelperScripts(request.sessionId, request.storyId); return hash([scripts.bindingRevision, scripts.libraries.map(({ type, revision }) => [type, revision])]); }
function verifyTarget(events, job) {
    ...
    if (target?.type !== 'assistant/message' || String(target.data.message.id) !== job.identity || target.data.turn !== job.turn || target.data.interrupted || target.surfaceOp !== undefined && target.surfaceOp !== 'append'
        ...
        throw new Error('自动 MVU 目标消息身份或原文已改变');
    if (!isTavernGreetingEvent(target) && !candidates(events, job.turn).some(item => item.seq === job.seq))
        throw new Error('自动 MVU 目标没有真实正常 stop');
}
/** 后台脚本在任何沙箱挂载前取得安全锚点并播种静态 data；不会领取执行租约或执行第三方代码。 */
export async function ensureHelperMvuScriptAnchor(ctx, state, sessionId, storyId) {
    return locked(state, { sessionId, storyId, runtimeId: 'script-bootstrap' }, async (binding) => {
        ...
        if (!job) {
            ...
            if (!mvu.initialized) {
                ...
                if (boundary?.type === 'turn/start' && open?.floor === sessionId + '#t' + boundary.data.turn && open.cardId === binding.cardId && open.storyId === storyId) {
                    ...
                    if (stop && (!latest || stop.seq > latest.seq))
                        latest = stop;
                }
            }
            if (!latest)
                return null;
            const open = state.openFloors.get(sessionId);
            if (!mvu.initialized && open && open.floor !== sessionId + '#t' + latest.data.turn)
                return null;
        ...
        if (stateHash(saved) !== before) {
            ...
            if (!ready.open)
                await ws.wal.commitFloor(job.floor);
        }
        else if (mvu.pending.length)
            await floorReady(state, binding, sessionId, job);
        return job.seq;
    });
}
export async function prepareHelperMvuJob(ctx, state, request) {
    return locked(state, request, async (binding) => {
        const baseWork = { storyId: request.storyId, enabled: enabled(state, binding), status: 'idle' };
        if (!baseWork.enabled) {
            leaseMap(state).delete(leaseKey(request));
            return baseWork;
        }
        const ws = await state.storyWorkspace(binding.cardId, binding.storyId);
        ...
        if (await unfinalizedReceipt(state, binding, request.sessionId, mvu)) {
            ...
            if (!ready.open)
                await ready.ws.wal.commitFloor(job.floor);
        }
        if (!mvu.pending.length && await unregisteredFloor(state, binding, request.sessionId, mvu)) {
            ...
            if (!session)
                throw new Error('自动 MVU 登记恢复缺少当前宿主会话');
            await queue(state, request.sessionId, session, false);
            saved = await loadHelperState(ws.fs);
            mvu = parseHelperMvuState(saved.mvu);
            if (!mvu.pending.length)
                throw new Error('自动 MVU 登记恢复未找到正常完成回复');
        }
        ...
        if (!mvu.pending.length && !mvu.initialized) {
            ...
            if (!latest)
                return { ...baseWork, status: 'waiting' };
            const open = state.openFloors.get(request.sessionId);
            if (open && open.floor !== request.sessionId + '#t' + latest.data.turn)
                return { ...baseWork, status: 'waiting' };
        ...
        if (!job)
            return { ...baseWork, awaitingTurnEnd: state.openFloors.has(request.sessionId) };
        verifyTarget(events, job);
        verifyContinuation(events, job, mvu);
        await floorReady(state, binding, request.sessionId, job);
        const key = leaseKey(request), map = leaseMap(state), old = map.get(key);
        if (old && old.job.id === job.id && !old.committed) {
            if (old.runtimeId !== request.runtimeId)
                return { ...baseWork, status: 'waiting' };
            old.expires = Date.now() + 90000;
            return helperJson(old.work, 4 * 1024 * 1024);
        }
        ...
        if (job.kind === 'update') {
            ...
            for (let at = index - 1; at >= 0; at--) {
                const item = saved.scopes[scope(history[at].identity)];
                if (item && helperRecord(item.stat_data)) {
                    base = helperMvuData(item);
                    break;
                }
            }
            if (!helperRecord(base.stat_data))
                throw new Error('自动 MVU 更新缺少前一条有效消息状态');
        }
        const initialSources = job.kind === 'initialize' ? await sources(state, binding) : undefined;
        const greeting = events.find((event) => event.type === 'assistant/message' && isTavernGreetingEvent(event) && (event.surfaceOp === undefined || event.surfaceOp === 'append'));
        const isGreeting = isTavernGreetingEvent(events.find(event => event.seq === job.seq));
        const token = randomUUID(), work = { ...baseWork, status: 'pending', job, token, base: helperJson(base), snapshot: { ...await getHelperSnapshot(ctx, state, request.sessionId, job.seq), writable: false }, applyText: job.kind === 'update' || !isGreeting && !helperRecord(target.stat_data),
        ...
        if (map.size >= 128)
            throw new Error('自动 MVU 活动运行时超过预算');
        ...
        return helperJson(work, 4 * 1024 * 1024);
    });
}
export async function commitHelperMvuJob(ctx, state, request) {
    const data = helperMvuData(request.data), digest = hash(data);
    return locked(state, request, async (binding) => {
        if (!enabled(state, binding))
            throw new Error('自动 MVU 已关闭');
        const key = leaseKey(request), lease = leaseMap(state).get(key);
        if (!lease || lease.runtimeId !== request.runtimeId || lease.token !== request.token || lease.job.id !== request.jobId)
            throw new Error('自动 MVU 执行租约已失效，请重新准备');
        if (hash(binding) !== lease.binding || await scriptRevision(state, request) !== lease.scripts)
            throw new Error('自动 MVU 绑定或脚本库已改变');
        const events = await readDisplaySessionEvents(ctx, request.sessionId);
        verifyTarget(events, lease.job);
        if (helperHistoryRevision(helperHistoryOf(events, { char: '', user: '' })) !== lease.history)
            throw new Error('自动 MVU 执行期间聊天历史已改变');
        ...
        if (receipt) {
            if (receipt.digest !== digest || receipt.identity !== lease.job.identity)
                throw new Error('自动 MVU 重试结果与完成回执不一致');
            if (hash(saved.scopes[scope(lease.job.identity)]) !== receipt.valueDigest)
                throw new Error('自动 MVU 完成后的消息状态已被修改');
            if (!open)
                await ws.wal.commitFloor(lease.job.floor);
        ...
        if (stateHash(saved) !== lease.before || mvu.pending[0]?.id !== request.jobId)
            throw new Error('自动 MVU 变量或待处理队列已被另一运行时修改');
        saved.scopes[scope(lease.job.identity)] = helperMvuData({ ...(saved.scopes[scope(lease.job.identity)] ?? {}), ...data });
        mvu.pending.shift();
        mvu.completed.push({ id: lease.job.id, identity: lease.job.identity, kind: lease.job.kind, digest, valueDigest: hash(saved.scopes[scope(lease.job.identity)]), floor: lease.job.floor });
        mvu.completed = mvu.completed.slice(-256);
        mvu.initialized = true;
        mvu.lastIdentity = lease.job.identity;
        saved.mvu = mvu;
