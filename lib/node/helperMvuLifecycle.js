...
function gateFor(agent) {
    let gate = gates.get(agent);
    if (!gate) {
        gate = { inbox: { 'next-turn': [], 'next-step': [] }, inboxCursor: 0, removed: new Map(), claimed: new Map(), blocked: new Set(), maintenance: false, abortCleanup: new Map() };
        gates.set(agent, gate);
    }
    return gate;
}
/** 开启配置必须先同步认领宿主真正 idle；不能在新的空 WAL 出现后再尝试初始化旧回复。 */
export async function runHelperMvuEnable(ctx, state, sessionId, save) {
    const message = '请等待当前生成和维护结束后再开启自动 MVU';
    const perform = async (signal) => {
        if (signal)
            await abortable(state.waitForSessionTasks(sessionId), signal);
        else
            await state.waitForSessionTasks(sessionId);
        if (ctx.agents?.get(SessionId(sessionId)) !== agent)
            throw new Error(message);
        if (state.openFloors.has(sessionId))
            throw new Error(message);
    ...
    if (agent.status !== 'idle')
        throw new Error(message);
    try {
        return agent.runMaintenance(signal => perform(signal));
    }
    catch {
        throw new Error(message);
    }
...
export function helperMvuHasAssistant(session) {
    ...
    return events.some(event => {
        if (event.type !== 'assistant/message' || event.data.interrupted || event.surfaceOp !== undefined && event.surfaceOp !== 'append'
            ...
            return false;
        if (isTavernGreetingEvent(event) || event.data.turn === 0)
            return true;
...
export async function waitForHelperMvu(state, agent, signal, includeInitialization = true) {
    let announced = false;
...
export function reserveHelperMvuMaintenance(state, agent, report) {
    const gate = gateFor(agent);
    if (agent.status !== 'idle' || gate.maintenance)
        return;
    try {
...
export async function stopForHelperMvu(state, agent, signal) {
    await abortable(state.waitForSessionTasks(agent.id), signal);
    signal.throwIfAborted();
    ...
    if (assistant?.type !== 'assistant/message' || assistant.data.interrupted)
        return;
    if (!hasNormalAssistantStop(assistant))
        return;
...
export function observeHelperMvuSessionEvent(agent, event) {
    const gate = gateFor(agent);
    if (event.type === 'agent/inbox/spliced') {
        ...
        if (splice.removedCount) {
            for (const message of list.slice(splice.start, splice.start + splice.removedCount)) {
                if (splice.outcome === 'canceled') {
                    gate.claimed.delete(message.id);
                    gate.removed.delete(message.id);
                }
                else if (splice.inserted.length === 0)
                    gate.removed.set(message.id, splice.target);
            }
        }
        ...
        if (gate.blocked.has(event.data.turn))
            restoreHelperMvuInputs(agent, event.data.turn);
        gate.blocked.delete(event.data.turn);
        gate.abortCleanup.get(event.data.turn)?.();
        gate.abortCleanup.delete(event.data.turn);
        for (const [id, claim] of gate.claimed)
            if (claim.turn <= event.data.turn)
                gate.claimed.delete(id);
        gate.removed.clear();
    }
}
export function observeHelperMvuClaim(agent, message, turn) {
    const gate = gateFor(agent), target = gate.removed.get(message.id);
    gate.removed.delete(message.id);
    if (target)
        gate.claimed.set(message.id, { message, target, turn });
}
/** 恢复尚未进入 user/message 的原输入，不 wake；下一条真实输入可以唤醒保留队列。 */
export function restoreHelperMvuInputs(agent, turn, messages) {
    ...
    for (const target of ['next-step', 'next-turn']) {
        ...
        if (restore.length)
            agent.inbox.splice(target, 0, 0, restore);
    }
}
export function isHelperMvuBlocked(agent, turn) { return gateFor(agent).blocked.has(turn); }
/** 在 await next() 之前判断；abort 可能让宿主跳过 pre-step，因此被挡轮次同时安装取消恢复。 */
export async function blockHelperMvuAssembly(state, agent, signal) {
    const turn = currentTurn(agent.session);
    if (turn === undefined)
        return false;
    let blocked;
...
export function registerHelperMvuLifecycle(ctx) {
    ctx.on('session/event', (session, event) => {
        const agent = ctx.agents.get(session.id);
        if (agent?.session === session)
            observeHelperMvuSessionEvent(agent, event);
    });
