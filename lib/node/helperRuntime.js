...
function publicScopes(scopes, history) {
    const result = {};
...
export async function getHelperScriptBundle(ctx, state, sessionId) {
    ...
    const ensureBinding = async () => {
        ...
        if (!current || current.storyId !== binding.storyId || current.cardId !== binding.cardId || current.presetId !== binding.presetId || current.interactiveCards !== binding.interactiveCards || current.helperMvu !== binding.helperMvu)
