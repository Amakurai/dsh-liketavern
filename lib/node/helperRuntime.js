...
import { effectiveHelperSwipes } from '../core/helperSwipes.js';
import { expandIdentityMacros } from '../core/macros.js';
import { createHash } from 'node:crypto';
import { helperJson, helperChanges, helperScopeKey } from '../core/helperRuntime.js';
import { isSyntheticUserText } from '../core/dshPrompt.js';
import { DEFAULT_USER_NAME } from '../core/persona.js';
import { applyHelperChanges, commitHelperChanges, loadHelperScopes, loadHelperState } from '../state/helper.js';
...
const helperHistoryCaches = new WeakMap();
function cachedHelperHistory(state, sessionId, events, bindingKey, names) {
    let cache = helperHistoryCaches.get(state);
    if (!cache) {
        cache = new Map();
        helperHistoryCaches.set(state, cache);
    }
    ...
    const history = helperHistoryOf(events, names), assistantIndexBySeq = new Map(), indexByIdentity = new Map();
    history.forEach((message, index) => { if (!indexByIdentity.has(message.identity))
        indexByIdentity.set(message.identity, index); if (message.role === 'assistant')
        assistantIndexBySeq.set(message.seq, index); });
    let lastBoundary;
    for (let index = events.length - 1; index >= 0; index--) {
        const event = events[index];
        if (event.type === 'turn/start' || event.type === 'turn/end') {
            lastBoundary = { type: event.type, turn: event.data.turn };
            break;
        }
    }
    const entry = { events, eventCount: events.length, lastEvent, lastEventSeq: lastEvent?.seq, bindingKey, charName: names.char, userName: names.user, history,
        historyRevision: helperHistoryRevision(history), assistantIndexBySeq, indexByIdentity, lastBoundary };
    cache.delete(sessionId);
    cache.set(sessionId, entry);
    while (cache.size > HELPER_HISTORY_CACHE_SESSIONS)
        cache.delete(cache.keys().next().value);
    return entry;
}
export function helperHistoryOf(events, names) {
    const messages = [];
    let bytes = 0;
    for (const event of events) {
        ...
        if ('surfaceOp' in event && event.surfaceOp !== undefined && event.surfaceOp !== 'append')
            continue;
        const message = event.type === 'user/message' ? event.data : event.data.message;
        const text = message.content.filter(block => block.type === 'text').map(block => block.text).join('\n');
        if (!text.trim() || event.type === 'user/message' && isSyntheticUserText(text))
            continue;
        bytes += Buffer.byteLength(text);
        if (messages.length >= 4096 || bytes > 4 * 1024 * 1024)
            throw new Error('酒馆助手历史超出快照预算，需要分页加载');
...
function publicScopes(scopes, indexByIdentity) {
    const result = {};
    for (const [key, value] of Object.entries(scopes)) {
        ...
        if (type !== 'message')
            ...
            if (index !== undefined)
                result[JSON.stringify(['message', index])] = value;
        }
    }
    return result;
}
function snapshot(context, scopes, extras = {}, swipes = {}) {
 ...
 */
export async function getHelperDisplayContext(ctx, state, sessionId, messageId) {
    ...
    return withWorkspaceLock(context.ws.fs.root, async () => {
        ...
        if (current.ws.fs.root !== context.ws.fs.root)
            throw new Error('卡面剧情绑定已经改变');
...
export async function getHelperSnapshot(ctx, state, sessionId, messageId) {
    const context = await helperContext(ctx, state, sessionId, messageId);
    return withWorkspaceLock(context.ws.fs.root, async () => {
        const current = await helperContext(ctx, state, sessionId, messageId);
        if (current.ws.fs.root !== context.ws.fs.root)
...
export async function getHelperScriptBundle(ctx, state, sessionId) {
    const binding = await state.loadBinding(sessionId);
    if (!binding?.storyId)
        throw new Error('会话未绑定可用剧情');
    const ensureBinding = async () => {
        const current = await state.loadBinding(sessionId);
        if (!current || current.storyId !== binding.storyId || current.cardId !== binding.cardId || current.presetId !== binding.presetId || current.personaId !== binding.personaId || current.interactiveCards !== binding.interactiveCards || current.helperMvu !== binding.helperMvu)
    ...
    if (!enabled)
        return { ...base, messageId: null };
    try {
        helperJson(libraries, 4 * 1024 * 1024);
        if (!enabledHelperLibraries(libraries).length && !base.helperMvu)
            return { ...base, messageId: null };
    }
    catch (error) {
        return { ...base, messageId: null, runtimeError: error instanceof Error ? error.message : String(error) };
    }
    let messageId;
    if (base.helperMvu)
        messageId = await ensureHelperMvuScriptAnchor(ctx, state, sessionId, binding.storyId);
    else {
        const events = await readDisplaySessionEvents(ctx, sessionId);
    ...
    await ensureBinding();
    if (snapshot.storyId !== binding.storyId)
        throw new Error('脚本剧情绑定已改变');
    const scriptContext = await state.getSessionHelperScripts(sessionId, binding.storyId);
    await ensureBinding();
    if (scriptContext.libraries.some(library => libraries.find(item => item.target.type === library.type)?.revision !== library.revision))
        throw new Error('加载期间脚本库已改变，请重新加载');
    return { ...base, messageId, snapshot, scriptContext };
}
export async function commitHelperVariables(ctx, state, request) {
    ...
    return withWorkspaceLock(context.ws.fs.root, async () => {
        ...
        if (current.binding.storyId !== request.storyId || current.ws.fs.root !== context.ws.fs.root)
            throw new Error('卡面剧情绑定已经改变');
        if (current.historyRevision !== request.historyRevision)
            throw new Error('聊天历史已改变，请刷新卡面后重试');
        if (!state.config.interactiveCards || current.binding.interactiveCards === false)
            throw new Error('交互卡已关闭');
        await assertHelperMvuWritable(state, request.sessionId);
        if (!current.writable)
            throw new Error('生成期间不能修改酒馆助手剧情变量');
        const mapped = changes.map(change => {
            const [type, id] = helperScopeKey(change.key);
            if (type !== 'message')
                return change;
            if (typeof id !== 'number' || !current.history[id])
                throw new Error('目标消息不在当前剧情中');
        ...
        if (floors.some(item => item.floor === floor && item.rolledBack))
            throw new Error('酒馆助手目标楼层已回滚');
        if (floors.some(item => item.floor === floor))
            await current.ws.wal.validateFloor(floor);
        else
            await current.ws.wal.beginFloor(floor);
...
export async function withHelperStoryWrite(ctx, state, sessionId, messageId, storyId, write) {
    const first = await helperContext(ctx, state, sessionId, messageId);
    return withWorkspaceLock(first.ws.fs.root, async () => {
        const current = await helperContext(ctx, state, sessionId, messageId);
        if (current.binding.storyId !== storyId || current.ws.fs.root !== first.ws.fs.root)
            throw new Error('世界书剧情绑定已改变');
        if (!state.config.interactiveCards || current.binding.interactiveCards === false)
            throw new Error('交互卡已关闭');
        await assertHelperMvuWritable(state, sessionId);
        if (!current.writable)
            throw new Error('生成期间不能修改聊天世界书');
