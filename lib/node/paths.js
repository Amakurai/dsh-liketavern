...
export function tavernPaths(home) {
...
export async function ensurePaths(paths) {
    await Promise.all([paths.characters, paths.lorebooks, paths.presets, paths.personas, paths.regexDir, paths.sessions].map((dir) => mkdir(dir, { recursive: true })));
}
/** 会话绑定文件名安全化。 */
