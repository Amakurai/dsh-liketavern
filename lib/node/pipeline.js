...
export function flattenMessages(messages, charName, userName) {
    ...
    for (const m of messages) {
        if (m.role === 'tool' || m.role === 'developer')
            continue;
...
async function runTavernPipelineLocked(input, expected) {
    ...
    };
    {
        ...
        if (queryMessages.trim() && candidateCount > 0 && Number.isFinite(config.memory.retrievalTokenBudget) && config.memory.retrievalTokenBudget >= 1) {
            const hits = await ws.memory.search(queryMessages, { ...memorySearchOptions(candidateCount, config.memory.halfLifeDays), includeSummarySources: true });
