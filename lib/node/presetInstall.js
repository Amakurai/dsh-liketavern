...
export async function installTavernPreset(ctx) {
    const templateDir = join(packageRoot(), 'presets', TAVERN_PRESET_ID);
    const plugins = parse((await readFile(join(templateDir, 'agent.cordis.yml'), 'utf8'))
        .replaceAll('__AGENT_MODULE__', escapeYamlSingleQuoted(agentModulePath())));
    const problem = entryListProblem(plugins);
    if (problem)
        throw new Error(problem);
    const meta = parse(await readFile(join(templateDir, 'preset.yml'), 'utf8'));
    if (typeof meta.name !== 'string' || typeof meta.description !== 'string' || typeof meta.order !== 'number')
        throw new Error('Tavern 预设元数据无效');
    return ctx.agentPresets.register({ id: TAVERN_PRESET_ID, name: meta.name, description: meta.description,
        order: meta.order, plugins: plugins });
}
