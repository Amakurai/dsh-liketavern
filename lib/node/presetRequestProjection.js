...
function stripPlanMetadata(message) {
    if (!('tavernPromptPlan' in message.source))
        return message;
    ...
    for (const original of messages) {
        ...
        if (message.source.kind === 'plugin' && message.source.plugin === HOST_PROMPT_PLUGIN) {
            ...
            if (message.role === 'user' && message.source.form === 'snapshot') {
                ...
                if (sections.length === message.source.sections.length) {
                    result.push(message);
                    continue;
                }
...
export function projectPresetRequest(options, session, projection = {}) {
    ...
    const locate = (anchor) => {
        ...
        while (seq !== undefined && replacements.has(seq)) {
            ...
            if (!event || event.type !== 'user/message' || event.data.source.kind !== 'plugin' || event.data.source.plugin !== 'compact')
                break;
