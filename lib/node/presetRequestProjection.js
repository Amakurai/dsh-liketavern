...
function parsePlan(value) {
    ...
    if (!result.success)
        throw new Error(`Tavern 提示词布局快照无效：${result.error.issues[0]?.message ?? '未知格式'}`);
    ...
    for (const item of [...result.data.layout.history, ...result.data.layout.entries]) {
        ...
        if (bytes > 8 * 1024 * 1024)
            throw new Error('Tavern 提示词布局快照超过 8 MiB 上限');
    }
    if (bytes > 8 * 1024 * 1024)
        throw new Error('Tavern 提示词布局快照超过 8 MiB 上限');
    return result.data;
...
export function createPresetPlanMessage(plan) {
...
export function attachPresetPlanMessage(message, plan) {
...
function planFromMessage(message) {
    ...
    if (message.role !== 'user')
        throw new Error('Tavern 提示词布局载体必须是 user/message');
    return parsePlan(value);
}
/** 接收批次及已持久化日志共用的轮次判定；存在但损坏的元数据仍明确失败。 */
export function hasPresetPlanMessage(message, sessionId, turn) {
...
function stripPlanMetadata(message) {
    if (message.role !== 'user' || !('tavernPromptPlan' in message.source))
        return message;
...
function activeTurn(session) {
    for (const event of [...session.snapshotEvents()].reverse()) {
        if (event.type === 'turn/end')
            throw new Error('Tavern 提示词投影没有正在进行的轮次');
        if (event.type === 'turn/start')
            return event.data.turn;
    }
    throw new Error('Tavern 提示词投影缺少 turn/start');
}
function currentPlan(messages, session) {
    ...
    for (const message of candidates) {
        const plan = planFromMessage(message);
        if (!plan || plan.sessionId !== session.id || plan.turn !== turn)
            continue;
        return { plan, carrier: message };
    }
...
function hostMessages(messages, plan) {
    ...
    const cleanedSystem = freezeMessage({ ...latest, content: [{ type: 'text',
                text: systemText.slice(0, at) + BOUND_DISCIPLINE + systemText.slice(at + plan.standingText.length) }] });
    const result = [cleanedSystem];
    for (const original of messages) {
        if (original.source.kind === 'tavern-prompt-plan')
            continue;
        const message = stripPlanMetadata(original);
        if (message.role === 'system' && message.source.kind === 'system-prompt')
            continue;
        ...
        if (message.role === 'user') {
            ...
            if (snapshot.success) {
                ...
                if (sections.length === snapshot.data.sections.length) {
                    result.push(message);
                    continue;
                }
                const text = joinContextSections(sections);
                if (text)
                    result.push(freezeMessage({ ...message, source: { ...message.source, sections }, content: [{ type: 'text', text }] }));
                continue;
            }
        }
        result.push(message);
...
function pluginMessage(plan, key, role, content) {
    const digest = createHash('sha256').update(JSON.stringify([plan.sessionId, plan.turn, key])).digest('hex');
    const fields = { id: MessageId(`tavern-prompt-${digest}`), content: [{ type: 'text', text: content }] };
    // 仅存在于适配器请求副本；不追加 assistant/message，也不伪造模型历史或重放元数据。
    if (role === 'assistant')
        return freezeMessage({ ...fields, role, source: { kind: 'model', provider: 'tavern-preset', model: 'instruction', tavernProjection: true } });
    if (role === 'system')
        return freezeMessage({ ...fields, role, source: { kind: 'system-prompt', tavernProjection: true } });
...
function applySystemPolicy(messages, projection) {
    if (!projection.model)
        return [...messages];
    const first = messages[0];
    if (!first || first.role !== 'system')
        throw new Error('Tavern 模型请求缺少首条完整系统提示');
    const systems = messages.filter(message => message.role === 'system');
    const sections = [];
    ...
    const complete = (message) => {
        ...
        if (emittedBytes > 16 * 1024 * 1024)
            throw new Error('Tavern 完整系统提示快照累计超过 16 MiB，请减少深度 system 条目');
        sections.push(text);
        return freezeMessage({ ...message, content: [{ type: 'text', text: sections.join('\n\n') }] });
    };
        ...
        for (const message of messages) {
            if (message.role === 'system') {
                if (previous !== undefined) {
                    pending = true;
                    if (previous !== 'user' && previous !== 'tool')
                        incompatiblePosition = true;
                }
            }
            else {
        ...
        for (let index = 0; index < messages.length;) {
            ...
            while (index < messages.length && isTavernSystem(messages[index])) {
                ...
                if (bytes > 16 * 1024 * 1024)
                    throw new Error('Tavern 完整系统提示超过 16 MiB');
                parts.push(text);
            }
    ...
    if (systems.length > 1 && !incompatiblePosition)
        projection.onDiagnostic?.({ kind: 'leading-system-only' });
    ...
    return [freezeMessage({ ...first, content: [{ type: 'text', text: parts.join('\n\n') }] }),
        ...messages.filter(message => message.role !== 'system')];
}
/** 只估可见 text 块与旧式 system；图片、工具参数/结果编码及 provider framing 不在此估算内。 */
function requestTextTokens(options, messages) {
        ...
        if (calls.length) {
            ...
            for (const call of calls) {
                if (pending.has(call.id))
                    throw new Error('Tavern 请求含重复的未完成工具调用 ID，不能安全定位工具配对');
...
export function projectPresetRequest(options, session, projection = {}) {
    if (options.purpose)
        return { ...options, messages: [...options.messages] };
    if (!options.sessionId || options.sessionId !== session.id)
        throw new Error('Tavern 请求与会话身份不匹配');
    const requestMessages = options.messages.map(message => {
        if (message.id === undefined || message.source === undefined)
            throw new Error('Tavern 普通聊天请求缺少持久消息身份，不能投影预设');
        return message;
    });
    const { plan, carrier } = currentPlan(requestMessages, session);
    const entries = plan.layout.entries;
    if (entries.some(entry => entry.compatibilityFallback))
        throw new Error('Tavern 冻结布局缺少模板精确位置，不能用于真实请求；请修订模板后开启下一轮');
    const messages = hostMessages(requestMessages, plan);
    const positions = new Map();
    messages.forEach((message, index) => {
        if (positions.has(message.id))
            throw new Error('Tavern 请求消息 ID 重复，不能可靠定位历史');
        positions.set(message.id, index);
    });
    const anchors = new Map();
    for (const anchor of plan.layout.history) {
        if (anchors.has(anchor.inputIndex))
            throw new Error('Tavern 布局历史 inputIndex 重复');
    ...
    const locate = (anchor) => {
        ...
        if (index !== undefined) {
            if (messages[index]?.role !== anchor.role)
                throw new Error('Tavern 历史锚点角色已变化');
        ...
        while (seq !== undefined && replacements.has(seq)) {
            ...
            if (!event || event.type !== 'user/message' || event.data.source.kind !== 'compact-checkpoint')
                break;
    ...
    let frontierEnd;
    const frontier = () => {
        if (frontierEnd !== undefined)
            return frontierEnd;
        if (carrierIndex < 0)
            throw new Error('Tavern 首次输入边界已被压缩，不能重排本轮尾条；请开启下一轮');
        let end = 1;
    ...
    const safeGaps = { before: new Map(), after: new Map() };
    const safeGap = (gap, side) => {
        ...
        for (const span of spans) {
            if (span.start < result && result <= span.end) {
                if (!span.complete)
                    throw new Error('Tavern 提示词位置穿过未完成的工具调用，拒绝拆开工具配对');
    ...
    for (const entry of entries) {
        if (byKey.has(entry.key))
            throw new Error('Tavern 冻结布局条目 key 重复');
    ...
    const gapFor = (entry) => {
        ...
        if (cached !== undefined)
            return cached;
        if (resolving.has(entry.key))
            throw new Error('Tavern 模板插入位置出现循环引用');
        if (resolving.size >= 512)
            throw new Error('Tavern 模板插入位置嵌套超过 512 层');
        ...
        switch (placement.kind) {
            case 'entry-relative': {
                ...
                if (!target)
                    throw new Error('Tavern 模板插入目标不存在');
                gap = gapFor(target);
                break;
            }
            case 'before-history':
                gap = placement.anchor ? safeGap(locate(placement.anchor).index, 'before') : 1;
                break;
            case 'after-history': {
                if (placement.anchor)
                    locate(placement.anchor);
                gap = safeGap(frontier(), 'after');
                break;
            }
            case 'history-relative':
                gap = safeGap(locate(placement.anchor).index + (placement.side === 'after' ? 1 : 0), placement.side);
                break;
            case 'depth': {
                ...
                if (previous !== undefined && next !== undefined && previous.index >= next.index) {
                    if (previous.index !== next.index || !previous.compacted || !next.compacted)
                        throw new Error('Tavern 冻结历史锚点顺序已变化');
                    gap = safeGap(next.index + 1, 'after');
                }
                else
                    gap = next !== undefined ? safeGap(next.index, 'before') : safeGap(frontier(), 'after');
                break;
            }
        }
    ...
    const append = (gap, message) => {
        const slot = slots.get(gap);
        if (slot)
            slot.push(message);
        else
            slots.set(gap, [message]);
    };
    ...
    for (const entry of entries) {
        if (entry.placement.kind !== 'entry-relative')
            continue;
        ...
        if (siblings)
            ...
            target.set(entry.placement.entryKey, [entry]);
    }
    const emit = (entry, depth = 0) => {
        if (depth >= 512)
            throw new Error('Tavern 模板插入位置嵌套超过 512 层');
        for (const child of before.get(entry.key) ?? [])
            emit(child, depth + 1);
        if (entry.content.trim())
            append(gapFor(entry), pluginMessage(plan, entry.key, entry.role, entry.content));
        for (const child of after.get(entry.key) ?? [])
            emit(child, depth + 1);
    };
    for (const entry of entries)
        if (entry.placement.kind !== 'entry-relative')
...
export function createPresetRequestProjector(ctx, projection = {}) {
    return { project(options, model) {
            if (options.purpose)
                return { ...options, messages: [...options.messages] };
            const session = options.sessionId ? ctx.sessions.get(options.sessionId) : undefined;
            if (!session)
                throw new Error('Tavern 请求找不到当前 Session，不能读取冻结提示词布局');
