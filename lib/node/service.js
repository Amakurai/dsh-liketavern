...
export class TavernService extends TypertRemoteService {
    ...
    async savePreset(request) {
        ...
        try {
            const normalized = parseStPreset(exportStPreset(preset)).preset;
    ...
    conversationStarted(sessionId) {
        return this.ctx.sessions.get(sessionId)?.snapshotEvents().some((e) => e.type === 'turn/start' || e.type === 'assistant/message' || e.type === 'user/message') ?? false;
    }
    ...
    async clearSessionBinding(request) {
        ...
        if (request.onlyIfBlank === true) {
            ...
            return this.state.enqueueSessionTask(request.sessionId, async () => {
                if (this.conversationStarted(request.sessionId))
