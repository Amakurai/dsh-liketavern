...
export class TavernService extends TypertRemoteService {
    ...
    async setSessionBinding(request) {
        ...
        if (!binding?.sessionId || !binding.cardId)
            throw new FloorError('invalid-binding', '绑定缺少 sessionId 或 cardId');
    ...
    regenerate(request) {
        return this.state.enqueueSessionTask(request.sessionId, () => regenerate(this.floorDeps(), request.sessionId, request.messageId));
    }
    rollbackToFloor(request) {
        return this.state.enqueueSessionTask(request.sessionId, () => rollbackToFloor(this.floorDeps(), request.sessionId, request.messageId));
    }
