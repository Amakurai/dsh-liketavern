...
const deferredTurns = new WeakMap();
/** 只在运行时拒绝旧引擎活动日志；普通读取和备份仍允许保存有效旧状态。调用点必须持有剧情锁。 */
function assertActiveTemplateFormatter(stored, sessionId) {
    const carried = resolveTemplateContinuation(stored)?.replay;
    if (carried)
        assertTemplateReplayFormatter(carried);
    if (stored.generation?.status === 'prepared' && stored.generation.sessionId === sessionId)
        assertTemplateReplayFormatter(stored.generation.replay);
}
export async function onTurnStart(state, sessionId, turn, session) {
    state.currentTurns.set(sessionId, turn);
    state.currentSteps.set(sessionId, 1);
    let deferred = deferredTurns.get(state);
    if (!deferred) {
        deferred = new Map();
        deferredTurns.set(state, deferred);
    }
    deferred.set(sessionId, turn);
    if (await helperMvuPending(state, sessionId, !session || helperMvuHasAssistant(session)))
        return;
    ...
    await withWorkspaceLock(ws.fs.root, async () => {
        ...
        if (generation?.sessionId === sessionId) {
            if (generation.cardId !== binding.cardId || generation.storyId !== binding.storyId)
                throw new Error('模板轮次恢复的剧情归属不一致');
            if (generation.turn > turn)
                throw new Error('宿主轮次早于已保存的模板计划，请先回滚剧情');
            if (generation.turn === turn) {
                const original = await ws.wal.validateFloor(floor);
                if (generation.status !== 'prepared' || original.committed)
                    throw new Error('该模板楼层已经结束或缺失，不能重复开始');
...
export async function onTurnEnd(state, sessionId, session) {
    if (session && session.id !== sessionId)
        throw new Error('模板结束事件与目标会话不一致');
    ...
    const record = (error) => {
        ...
        while (state.helperTurnClosures.size > 256)
            state.helperTurnClosures.delete(state.helperTurnClosures.keys().next().value);
    };
    try {
        await finishTurn(state, sessionId, session);
        record();
    }
    catch (error) {
        record(error);
...
async function finishTurn(state, sessionId, session) {
    if (session && session.id !== sessionId)
        throw new Error('模板结束事件与目标会话不一致');
    ...
    let templateError, helperQueueError;
    if (session) {
        try {
        ...
        if (!templateError)
            try {
                await queueHelperMvuTurn(state, sessionId, session);
            }
            catch (error) {
                templateError = error;
                helperQueueError = error;
            }
        if (templateError)
            state.recordTriggerLog(sessionId, [...(state.turnPlans.get(sessionId)?.result.logLines ?? []), `[template:error] ${String(templateError)}`]);
    }
    ...
    if (!entry && session?.id === sessionId && ending?.type === 'turn/end') {
        ...
        if (binding) {
            ...
            if (generation?.sessionId === sessionId && generation.turn === ending.data.turn) {
                if (generation.cardId !== binding.cardId || generation.storyId !== binding.storyId)
                    throw new Error('模板结束恢复的剧情归属不一致');
                await ws.wal.validateFloor(generation.floor);
                ...
                try {
                    ...
                    if (generation?.status === 'prepared' && generation.sessionId === sessionId && generation.cardId === entry.cardId
                        ...
                        const completed = ending?.type === 'turn/end' && ending.data.turn === generation.turn && ending.data.reason.kind === 'completed'
                            && events.some(event => {
                                if (event.type !== 'assistant/message' || event.data.turn !== generation.turn || event.data.interrupted || event.seq >= ending.seq)
                                    return false;
