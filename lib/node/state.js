...
import { pinStandingText } from '../core/standingPin.js';
...
export class TavernState {
    ...
    async salvageLorebookName(base) {
        const sanitize = (s) => s.replace(/[^A-Za-z0-9_一-鿿.-]/g, '_');
    ...
    pinStanding(sessionId, fingerprint, computed) {
    ...
    resolveModelInfoCached(llm, provider, model, signal) {
        ...
        promise.catch(() => {
            if (this.modelInfoCache.get(key) === promise)
