...
export async function prepareContinuedTemplate(Sandbox, context, value) {
    ...
    if (carry?.replay) {
        ...
        let disposed = false;
        try {
            if (JSON.stringify(sandbox.stickyState()) !== JSON.stringify(carry.state))
                throw new Error('sticky 注册表与闭包重放不一致');
...
export function exportTemplateContinuation(sandbox, context, includeReplay = false) {
