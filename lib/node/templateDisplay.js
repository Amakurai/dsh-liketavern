...
import { parseTemplateDisplayParts, splitTemplateDisplay } from '../core/templateDisplay.js';
    ...
    const present = (raw, entry) => {
        ...
        for (const part of splitTemplateDisplay(value, true)) {
            if (part.kind === 'html')
                ...
                if (clean)
                    parts.push({ kind: 'markdown', text: clean });
            }
        }
    };
...
export function presentTemplateDisplay(parts, rules, macroCtx) {
    ...
    for (const part of parseTemplateDisplayParts(parts)) {
        ...
        if (part.kind === 'html')
            output.push({ ...part, text: rendered });
        else
            output.push(...splitTemplateDisplay(rendered));
    }
