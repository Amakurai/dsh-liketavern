...
export function applyTemplatePlacements(assembled, entries, sandbox, macroCtx, budget) {
    const located = entries.flatMap(entry => {
    ...
    if (!located.length)
        return assembled;
    if (!assembled.messageProvenance || !assembled.layout)
        throw new Error('定位注入缺少模拟消息来源索引');
    ...
    let insertionId = 0;
    const boundary = (origin, side) => {
        if (origin?.kind === 'layout')
            return { kind: 'entry-relative', entryKey: origin.entryKey, side };
    ...
    const addLayout = (item, text, at, side = 'before') => {
        ...
        const entry = { key, role, content: text, sourceKeys: [item.entry.key], turnLocal: true, placement };
        if (placement.kind === 'entry-relative') {
            let index = layout.entries.findIndex(value => value.key === placement.entryKey);
            if (index < 0)
                throw new Error('定位注入的布局来源不存在');
            if (placement.side === 'after') {
                let sibling = -1;
                layout.entries.forEach((value, at) => {
                    if (value.placement.kind === 'entry-relative'
                        && value.placement.entryKey === placement.entryKey && value.placement.side === 'after')
                        sibling = at;
                });
                if (sibling >= 0)
                    index = sibling;
    ...
    const live = [], log = [...assembled.log];
    let outputChars = 0;
    const render = (item, index, matched, sourceSuffix = '') => {
        ...
        const prepared = sandbox.transformRegexSources([{ key: source, text: raw, start: 0 }], 'generate', { role, worldinfo: true, depth: 0 }, 'generate-placement').map(part => part.text).join('');
        ...
        if (outputChars > 1024 * 1024)
            throw new Error('定位注入输出超过 1 MiB 上限');
        return result;
    };
    const record = (item, index, text) => {
        if (!text.trim())
            return;
    ...
    const missing = (item) => log.push({ kind: 'template-placement', detail: `${item.entry.comment}：未找到目标消息，未执行模板` });
    const regexp = (pattern, flags = '') => {
        try {
            return new RegExp(pattern, flags);
        }
        catch {
            throw new Error(`无效定位正则：${pattern}`);
        }
    };
    for (const item of located.filter(i => i.placement.kind === 'content')) {
        const placement = item.placement;
        if (placement.kind !== 'content' || placement.mode === 'global')
            continue;
        ...
        for (const index of targets) {
            ...
            if (text.trim()) {
                ...
                if (origin?.kind === 'layout') {
                    ...
                    addLayout(item, text, index, placement.at);
            }
    ...
    const apply = (queue) => {
        ...
        for (const insertion of queue)
            insertion.entryKey = addLayout(insertion.item, insertion.text, insertion.at);
        provenance = Array.from({ length: messages.length + 1 }, (_, index) => [
            ...(groups.get(index) ?? []).map(insertion => ({ kind: 'layout', entryKey: insertion.entryKey })),
            ...(provenance[index] ? [provenance[index]] : []),
        ]).flat();
        messages = Array.from({ length: messages.length + 1 }, (_, index) => [
            ...(groups.get(index) ?? []).map(({ item, text }) => ({ role: item.placement.kind === 'insert' ? item.placement.role : 'system', content: text })),
    ...
    for (const item of located) {
        ...
        if (p.kind !== 'insert' || p.mode === 'regex')
            continue;
        let at;
        if (p.mode === 'pos')
            at = Math.min(messages.length, Math.max(0, p.pos < 0 ? messages.length + p.pos : Math.max(0, p.pos - 1)));
        else {
    ...
    if (tokensAfter > budget)
        throw new Error('定位注入后的模拟提示词超过可用窗口，请缩减注入或提高上下文容量');
