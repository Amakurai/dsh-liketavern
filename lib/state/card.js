...
const DIRECT_BOOK_KEYS = ['character_book', 'lorebook', 'characterBook'];
const EXTENSION_BOOK_KEYS = ['character_book', 'characterBook', 'world'];
/** 仅判定解析器会当作有效内嵌书的非空值，不误删同名的其它厂商扩展。 */
function isEmbeddedBook(value) {
    return (normalizeBook(value)?.entries.length ?? 0) > 0;
}
/** 清理 V1/V2/V3 顶层或 data 直接字段中的世界书落点，不触碰 extensions。 */
function stripDirectBooks(input) {
    const output = { ...input };
    for (const key of DIRECT_BOOK_KEYS) {
        if (isEmbeddedBook(input[key]))
            delete output[key];
    }
    return output;
}
/** 仅清理 pickCharacterBook 实际采用的 extensions 来源中三个受支持的键。 */
function stripExtensionBooks(input) {
    const output = { ...input };
...
function stripRawCharacterBooks(value) {
    if (!isRecord(value))
        return value;
    const hasWrappedData = isRecord(value.data);
    const rawData = hasWrappedData ? value.data : value;
    ...
    if (isRecord(value.extensions))
        root.extensions = stripExtensionBooks(value.extensions);
    if (isRecord(rawData.extensions))
        data.extensions = stripExtensionBooks(rawData.extensions);
    if (hasWrappedData)
        root.data = data;
    return root;
}
 ...
 */
export function withoutEmbeddedCharacterBook(card) {
    ...
    if (raw) {
        ...
        for (const key of ['lorebook', 'characterBook']) {
            if (!isEmbeddedBook(rawData[key]) || !isEmbeddedBook(extensions[key]))
                continue;
        ...
        for (const key of EXTENSION_BOOK_KEYS) {
            if (!extensionSource || !isEmbeddedBook(extensionSource[key]))
                continue;
            // data.world 会在归一化时覆盖同名 extensions.world，但它本身不是世界书落点。
            if (key === 'world' && directKeys.has('world'))
                continue;
            if (isEmbeddedBook(extensions[key]))
                delete extensions[key];
        }
    }
    else {
...
function normalizeCardInternal(json, pngBytes, specHint) {
    ...
    for (const [key, value] of Object.entries(data)) {
        if (!isKnownCharacterDataKey(key, value))
...
function isInflateOverflow(error) {
 ...
 */
function inflateCardText(compressed) {
    try {
...
export function embedCardInPng(pngBytes, json, spec) {
    ...
    if (!sawIend)
        throw new CardParseError('PNG 缺少 IEND 块：文件被截断，无法嵌入角色卡数据');
