...
function sourceRows(input) {
    ...
    if (Array.isArray(rows))
        return new Map(rows.flatMap((item, index) => helperRecord(item) ? [[rawUid(item, index), item]] : []));
...
export function readHelperWorldbook(input) {
    ...
    if (arrayIds && new Set(arrayIds).size !== arrayIds.length)
