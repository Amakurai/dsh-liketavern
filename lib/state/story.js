...
export async function readStory(cardRoot, id) {
    ...
    if (value.version !== 1 || value.id !== id || typeof value.sessionId !== 'string'
        ...
        throw new Error('剧情状态元数据损坏');
    return { id, sessionId: value.sessionId, createdAt: value.createdAt, migrated: value.migrated };
}
export async function listStories(cardRoot) {
    const entries = await readdir(join(cardRoot, 'stories'), { withFileTypes: true }).catch((error) => {
    ...
    for (const entry of entries) {
        if (!entry.isDirectory() || !/^(story-|legacy-)/.test(entry.name))
            continue;
...
export async function snapshotStory(options) {
    ...
    await withWorkspaceLock(options.sourceRoot, async () => {
        const parent = await lstat(join(options.cardRoot, 'stories')).catch((error) => {
            if (error.code === 'ENOENT')
                return null;
            throw error;
        });
        if (parent?.isSymbolicLink())
            throw new Error('剧情状态父目录不能是链接');
        if (await new WorkspaceFs(target, null).exists('story.json'))
            return;
        ...
        try {
            ...
            for (const directory of ['', 'memory', 'assets', 'state', 'state/wi-timers', 'state/wal']) {
                const info = await lstat(join(options.sourceRoot, directory)).catch((error) => {
                    if (error.code === 'ENOENT')
                        return null;
                    throw error;
                });
                if (info?.isSymbolicLink())
                    throw new Error(`剧情快照不支持链接目录：${directory}`);
            }
            await dest.ensureDir();
            const paths = ['journal.md', 'assets/chat-lorebook.json', 'state/world-delta.jsonl'];
            for (const directory of ['memory', 'state/wi-timers', ...(options.includeWal ? ['state/wal'] : [])]) {
                for (const file of await source.list(directory))
                    paths.push(`${directory}/${file}`);
            }
            for (const path of paths) {
                // 外部链接不属于快照；避免本机手改的 junction/symlink 把其它目录带进分支。
                const info = await lstat(join(options.sourceRoot, path)).catch((error) => {
                    if (error.code === 'ENOENT')
                        return null;
                    throw error;
                });
                ...
                if (!info.isFile() || info.isSymbolicLink())
                    throw new Error(`剧情快照不支持链接或目录文件：${path}`);
                const bytes = await source.readBytes(path);
                if (bytes !== null)
                    await dest.writeBytes(path, bytes);
            }
            ...
            if (!resolve(staging).startsWith(base))
                throw new Error('剧情草稿清理路径越界');
            await rm(staging, { recursive: true, force: true });
        }
    });
}
/** 仅供创建分支失败时清理刚创建、尚未绑定的副本，调用方负责复核绑定。 */
export async function discardStory(cardRoot, id, sessionId) {
    const metadata = await readStory(cardRoot, id);
    if (metadata.sessionId !== sessionId || metadata.migrated)
        throw new Error('拒绝清理不属于本次分支的剧情');
    const target = resolve(storyRoot(cardRoot, id));
    if (!target.startsWith(resolve(cardRoot, 'stories') + sep))
        throw new Error('剧情清理路径越界');
