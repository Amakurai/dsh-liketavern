...
// Wal
// ---------------------------------------------------------------------------
function deltaChangesOverlap(a, b) {
    const changed = (rec) => {
...
export class Wal {
    ...
    reopenFloor(floor) {
        return this.enqueue(async () => {
            ...
            if (!meta || meta.floor !== floor)
                throw new Error(`WAL 楼层元数据不匹配：${floor}`);
    ...
    async doRollbackFloor(floor, workspaceRoot) {
        ...
        const saved = await readFile(progressFile, 'utf8').catch((error) => {
            if (error.code === 'ENOENT')
                return null;
            throw error;
        });
        const progress = saved === null ? { hash, next: records.length - 1, restored: [], preserved: [] } : JSON.parse(saved);
        if (progress.hash !== hash || !Number.isInteger(progress.next) || progress.next < -1 || progress.next >= records.length
            || !Array.isArray(progress.restored) || !Array.isArray(progress.preserved))
            throw new Error('WAL 回滚恢复游标损坏');
        ...
        const applyPending = async () => {
            ...
            if (pending.path !== records[progress.next]?.path
                || !(pending.from === null || typeof pending.from === 'string') || !(pending.to === null || typeof pending.to === 'string'))
                throw new Error('WAL 回滚恢复记录损坏');
    ...
    async preflightRollback(floors) {
        ...
        for (const floor of floors)
            changes.push(...await this.readRecords(join(this.rootDir, sanitizeFloor(floor))));
