 ...
 * 唯一例外是 deleteCharacter 的整目录 rm -rf（WorkspaceFs 无递归删除语义）。
 */
...
export async function listCharacters(dataRoot) {
    const dataFs = new WorkspaceFs(dataRoot, null);
    const files = await dataFs.list('.');
    const cardIds = [...new Set(files.map((f) => f.split('/')[0]))];
    const out = [];
