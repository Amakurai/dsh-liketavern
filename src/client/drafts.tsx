...
type DraftStatus = { dirty: boolean; busy: boolean; clearDraft: () => void }
const DraftContext = createContext<((id: string, status: DraftStatus | null) => void) | null>(null)

/** 每个编辑器独立注册，避免一个干净的子编辑器覆盖另一个编辑器的未保存状态。 */
export function useDraftGuard(dirty: boolean, busy = false, clearChildren?: () => void) {
  ...
  const confirmation = <ConfirmDialog open={pending !== null} title={t('draft.leaveTitle')}
    description={t('draft.leaveDesc')} confirmLabel={t('draft.discard')} danger busy={busy}
...
export function DraftScope(props: { children: (request: (action: () => void) => void, dirty: boolean, busy: boolean) => ReactNode }) {
  const [statuses, setStatuses] = useState<Record<string, DraftStatus>>({})
  const report = useCallback((id: string, status: DraftStatus | null) => {
    setStatuses((current) => {
      const next = { ...current }
  ...
  const clearChildren = useCallback(() => {
    for (const status of Object.values(currentStatuses.current)) if (status.dirty) status.clearDraft()
  }, [])
  const guard = useDraftGuard(dirty, busy, clearChildren)
  return <DraftContext.Provider value={report}>
    {props.children(guard.request, dirty, busy)}
    {guard.confirmation}
