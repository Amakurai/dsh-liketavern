...
export const zh = {
  ...
  "settings.scripts.runtime.incompatible": "需要适配",
  "settings.scripts.windowAccess": "脚本尝试访问其它窗口的页面或全局对象，受到浏览器隔离限制。请使用适配版脚本，或在下方脚本管理中暂时停用它；放宽网络白名单无法解决此问题。",
  ...
  'settings.prompts.title': '角色卡提示词',
  'settings.prompts.desc': '这些偏好适用于所有会话，默认开启。保存后从下一轮生效；预设条目禁止角色卡覆盖时，仍使用预设内容。',
  'settings.prompts.preferCharacterPrompt': '优先使用角色卡主提示词',
  'settings.prompts.preferCharacterPromptDesc': '开启时，角色卡填写的主提示词覆盖预设主提示词；关闭或角色卡未填写时使用预设。',
  'settings.prompts.preferCharacterInstructions': '优先使用角色卡历史后指令',
  'settings.prompts.preferCharacterInstructionsDesc': '开启时，角色卡填写的历史后指令覆盖预设对应条目；关闭或角色卡未填写时使用预设。',
  'settings.prompts.save': '保存提示词设置',
  'settings.prompts.saved': '已保存提示词设置，下一轮生效',
  ...
  'settings.memory.retrievalTopKDesc': '每轮最多注入的记忆条数；0 = 不检索。先取最多 4 倍候选（上限 200），再按去重和预算筛选，实际条数可能更少。',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  "settings.scripts.runtime.incompatible": "Adaptation required",
  "settings.scripts.windowAccess": "This script tried to access another window's page or globals, which browser isolation blocks. Use an adapted script, or temporarily disable it in script management below. Allowing more network hosts will not resolve this issue.",
  ...
  'settings.prompts.title': 'Character prompts',
  'settings.prompts.desc': 'These preferences apply to all sessions and are enabled by default. Saved changes apply next turn. Preset entries that forbid character overrides continue using their own content.',
  'settings.prompts.preferCharacterPrompt': 'Prefer character main prompt',
  'settings.prompts.preferCharacterPromptDesc': 'When enabled, a non-empty character main prompt replaces the preset main prompt. Disable this, or leave the character prompt empty, to use the preset.',
  'settings.prompts.preferCharacterInstructions': 'Prefer character post-history instructions',
  'settings.prompts.preferCharacterInstructionsDesc': 'When enabled, non-empty character post-history instructions replace the corresponding preset entry. Disable this, or leave the character instructions empty, to use the preset.',
  'settings.prompts.save': 'Save prompt settings',
  'settings.prompts.saved': 'Prompt settings saved; applies next turn',
  ...
  'settings.memory.retrievalTopKDesc': 'Maximum memories injected per turn; 0 disables retrieval. Fetches up to 4× candidates (capped at 200), then applies deduplication and budget limits; fewer may fit.',
