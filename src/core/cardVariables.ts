...
export function installCardVariables(labels: CardVariableLabels, styles = '', messageId?: number, messageCount?: number, showTools = false): void {
  type Table = Record<string, unknown>
  type Holder = { scopes: Record<string, Table>; generation?: number }
  ...
  type Schema = { safeParseAsync(value: unknown): unknown }
  ...
  function scope(option?: unknown): string {
    ...
    if (typeof type !== 'string' || !['chat', 'character', 'global', 'preset', 'message', 'script', 'extension'].includes(type)) throw new Error('Unsupported variable scope')
    ...
    if(type==='message' && count!==undefined) {
      if(opts.message_id===undefined||id==='latest') id=count-1
      else if(id==='current') id=currentId??count-1
      else if(typeof id==='number'&&id<0) id=count+id
      if(typeof id!=='number'||!Number.isSafeInteger(id)||id<0||id>=count) throw new Error('Message variable index out of range')
    } else if (type === 'message' && (id === 'latest' || id === 'current' || id === -1)) id = messageId ?? 'current'
  ...
  function updateVariablesWith(updater: (value: Table) => Table | Promise<Table>, option?: unknown): Table | Promise<Table> {
  ...
  function deleteVariable(path: string, option?: unknown) {
    if (typeof path !== 'string' || path.length > 4096) throw new Error('Invalid variable path')
    const lodash = root._ as { toPath(value: string): string[] }
    const parts = lodash.toPath(path)
    if (!parts.length || parts.length > 64 || parts.some(key => ['__proto__', 'constructor', 'prototype'].includes(key))) throw new Error('Invalid variable path')
    const value = getVariables(option)
    let parent: unknown = value
    for (const key of parts.slice(0, -1)) {
      if ((!record(parent) && !Array.isArray(parent)) || !Object.hasOwn(parent, key)) { parent = null; break }
      parent = (parent as Table)[key]
    }
    const key = parts[parts.length - 1]!
    const occurred = (record(parent) || Array.isArray(parent)) && Object.hasOwn(parent, key)
    if (occurred) delete (parent as Table)[key]
    return { variables: occurred ? replaceVariables(value, option) : value, delete_occurred: occurred }
  }
  function getAllVariables(): Table {
    const currentId=(root.__dshTavernSnapshot as {currentMessageId:number}|undefined)?.currentMessageId??messageId
    const result: Table = {}
    for (const type of ['global', 'character', 'chat']) merge(result, getVariables({ type }), false)
    // 按当前快照顺序合入到本卡面为止的消息变量。
    const messages = Object.entries(holder.scopes).map(([key, value]) => ({ parts: JSON.parse(key) as [string, string | number], value }))
    ...
    for (const message of messages) merge(result, clone(message.value), false)
    return clone(result)
  }
  function registerVariableSchema(schema: unknown, option: unknown): void {
    if (holder.generation !== generation) throw new Error('Card variable runtime has been disposed')
    if (!record(schema) || typeof schema.safeParseAsync !== 'function') throw new Error('Variable schema must provide safeParseAsync')
    if (!record(option) || typeof option.type !== 'string' || !['global', 'preset', 'character', 'chat', 'message'].includes(option.type)
      || Object.keys(option).some(key => key !== 'type')) throw new Error('Invalid variable schema scope')
    schemas.set(String(option.type), schema as Schema)
    requestBackup()
  }
  ...
  function mountBackup() {
    if (!showTools || !variablesUsed || !document.body) return
    const existing = document.getElementById('dsh-tavern-variable-backup')
    if (existing) { mountEditor(existing); return }
  ...
  function mountEditor(container: HTMLElement) {
    ...
    const panel = document.createElement('fieldset'); panel.id = 'dsh-tavern-variable-editor'
    const legend = document.createElement('legend'); legend.textContent = labels.editor ?? 'Edit variables'
    const scopeBox = document.createElement('fieldset'), scopeTitle = document.createElement('legend')
    scopeTitle.textContent = labels.scope ?? 'Scope'; scopeBox.append(scopeTitle)
    let selection = 'chat'
    const scopeInputs: { input: HTMLInputElement; value: string }[] = []
    for (const [value, text] of [['global', labels.scopeGlobal ?? 'Global'], ['preset', labels.scopePreset ?? 'Preset'],
      ...
      option.type = 'radio'; option.name = 'dsh-tavern-variable-scope'; option.value = value!; option.checked = value === selection
      label.textContent = text!; option.setAttribute('aria-label', text!); label.append(option); scopeBox.append(label)
      scopeInputs.push({ input: option, value: value! })
    }
    const targetLabel = document.createElement('label'); targetLabel.textContent = labels.messageId ?? 'Message ID or latest'
    const target = document.createElement('input'); target.type = 'text'; target.value = 'latest'; target.setAttribute('aria-label', labels.messageId ?? 'Message ID or latest'); targetLabel.append(target)
    const input = document.createElement('textarea'); input.maxLength = maxBytes; input.setAttribute('aria-label', labels.editorText ?? 'Variables JSON')
    const status = document.createElement('p'); status.setAttribute('role', 'status')
    const button = (text: string) => { const node = document.createElement('button'); node.type = 'button'; node.textContent = text; return node }
    ...
    panel.append(legend, scopeBox, targetLabel, load, input, validate, save, discard, status); container.append(panel)
    type Draft = { key: string; option: Table; before: string; initialText: string; snapshotGeneration: unknown }
    let draft: Draft | undefined, busy = false, failed = false
    ...
    const controls = () => {
      load.disabled = busy || dirty(); discard.disabled = busy || !dirty()
      target.disabled = busy || dirty()
      for (const radio of scopeInputs) { radio.input.disabled = busy || dirty(); radio.input.checked = radio.value === selection }
    ...
    const selectedOption = (): Table => {
      const option: Table = { type: selection }
    ...
    const loadDraft = () => {
      try {
        if (holder.generation !== generation || root.__dshTavernDisplayLocked) throw conflict()
        const key = scope(selectedOption()), [type, id] = JSON.parse(key) as [string, string | number]
        const option: Table = { type, ...(type === 'message' ? { message_id: id } : {}) }
        ...
        input.value = text; failed = false; status.textContent = ''; status.setAttribute('role', 'status')
      } catch (error) { noteFailure(error) }
      controls()
    }
    load.onclick = () => { if (!busy && !dirty()) loadDraft() }
    discard.onclick = () => { if (!busy) loadDraft() }
    const changeScope = () => { draft = undefined; input.value = ''; status.textContent = ''; controls() }
    for (const radio of scopeInputs) radio.input.onchange = () => {
      if (!busy && !dirty()) { selection = radio.value; changeScope() }
      else controls()
    }
    target.onchange = () => { if (!busy && !dirty()) changeScope() }
    input.oninput = () => { controls(); if (dirty()) status.textContent = labels.unsaved ?? 'Draft not saved' }
    /** 转换结果必须仍是有界纯 JSON；拒绝静默丢失 function/undefined、循环和非 JSON 类实例。 */
    const jsonTable = (value: unknown): Table => {
      const seen = new Set<object>()
      let bytes = 0
      const walk = (item: unknown, depth: number): unknown => {
        if (depth > 64) throw new Error('Variable JSON exceeds 64 levels')
        let result: unknown
        ...
        else if (typeof item === 'string') { bytes += new TextEncoder().encode(JSON.stringify(item)).length; result = item }
        else {
          if (!item || typeof item !== 'object') throw new Error('Variable schema must return JSON data')
          const prototype: unknown = Object.getPrototypeOf(item)
          if (!Array.isArray(item) && prototype !== null && prototype !== Object.prototype) throw new Error('Variable schema must return plain JSON objects')
          if (seen.has(item) || Object.getOwnPropertySymbols(item).length) throw new Error('Invalid variable JSON')
          seen.add(item); bytes += 2
          const entries = Object.getOwnPropertyDescriptors(item)
          if (Array.isArray(item)) {
            if (Object.keys(entries).length !== item.length + 1) throw new Error('Variable arrays must be continuous')
            result = Array.from({ length: item.length }, (_, index) => {
              const entry = entries[String(index)]
              if (!entry || !('value' in entry) || !entry.enumerable) throw new Error('Invalid variable array')
              bytes++; return walk(entry.value, depth + 1)
            })
          } else {
            const output: Table = {}
            for (const [key, entry] of Object.entries(entries)) {
              if (['__proto__', 'constructor', 'prototype'].includes(key) || !('value' in entry) || !entry.enumerable) throw new Error('Invalid variable field')
              bytes += new TextEncoder().encode(JSON.stringify(key)).length + 2; output[key] = walk(entry.value, depth + 1)
            }
            result = output
          }
          seen.delete(item)
        }
        if (bytes > maxBytes) throw new Error('Card variables exceed 1 MiB')
        return result
      }
      const result = walk(value, 0)
      if (!record(result)) throw new Error('Variables must be an object')
      return result
    }
    const check = (loaded: Draft, text: string, schema: Schema | undefined) => {
      if (holder.generation !== generation || draft !== loaded || root.__dshTavernDisplayLocked
        ...
        || JSON.stringify(holder.scopes[loaded.key] ?? {}) !== loaded.before || schemas.get(String(loaded.option.type)) !== schema) throw conflict()
    }
    const run = async (saving: boolean) => {
      ...
      try {
        ...
        if (new TextEncoder().encode(text).length > maxBytes) throw new Error('Card variables exceed 1 MiB')
        let value = jsonTable(JSON.parse(text) as unknown)
        if (schema) {
          const outcome: unknown = await new Promise((resolve, reject) => {
          ...
          if (!record(outcome) || typeof outcome.success !== 'boolean') throw new Error('Invalid variable schema result')
          if (!outcome.success) throw outcome.error ?? new Error('Variable schema validation failed')
          value = jsonTable(outcome.data)
        }
        check(loaded, text, schema)
        if (!saving) { failed = false; status.textContent = labels.valid ?? 'Valid'; status.setAttribute('role', 'status'); return }
        ...
        if (holder.generation !== generation || draft !== loaded || root.__dshTavernSnapshotGeneration !== loaded.snapshotGeneration
          || JSON.stringify(holder.scopes[loaded.key] ?? {}) !== loaded.before) throw conflict()
        loaded.initialText = JSON.stringify(value, null, 2)
        if (input.value === text) input.value = loaded.initialText
        failed = false
      ...
      } catch (error) { failed = true; noteFailure(error) }
