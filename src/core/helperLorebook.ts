...
export interface HelperLorebookEntry {
  uid:number;display_index:number;comment:string;enabled:boolean;type:HelperWorldbookEntry['strategy']['type']
  position:string;depth:number|null;order:number;probability:number;keys:string[];logic:HelperWorldbookEntry['strategy']['keys_secondary']['logic'];filters:string[]
  scan_depth:'same_as_global'|number;case_sensitive:'same_as_global'|boolean;match_whole_words:'same_as_global'|boolean;use_group_scoring:'same_as_global'|boolean
  automation_id:string|null;exclude_recursion:boolean;prevent_recursion:boolean;delay_until_recursion:boolean|number;content:string
  group:string;group_prioritized:boolean;group_weight:number;sticky:number|null;cooldown:number|null;delay:number|null
}
export type HelperLorebookCodec=ReturnType<typeof createHelperLorebookCodec>
export function createHelperLorebookCodec(json:(value:unknown,maxBytes:number)=>unknown) {
  ...
  function list(input:unknown):Record<string,unknown>[] {
    ...
    if(!Array.isArray(values)||values.length>2000||values.some(value=>!record(value)))throw new Error('旧世界书条目必须是至多 2000 项的对象数组')
    for(const value of values)for(const key of Object.keys(value))if(!fields.includes(key))throw new Error('未知旧世界书字段：'+key)
    return values as Record<string,unknown>[]
  }
  function uid(value:unknown):number {if(typeof value!=='number'||!Number.isSafeInteger(value)||value<0)throw new Error('旧世界书 UID 必须为非负安全整数');return value}
  function toLegacy(entry:HelperWorldbookEntry,index=0):HelperLorebookEntry {
    const extra=entry.extra,inherit=(value:unknown):'same_as_global'|boolean=>typeof value==='boolean'?value:'same_as_global'
  ...
  function convert(input:Record<string,unknown>,base?:HelperWorldbookEntry,partial=false):unknown {
    const value=partial&&base?{...toLegacy(base),...input}:input
    const text=(key:string,fallback='',max=100000)=>{const v=value[key];if(v===undefined)return fallback;if(typeof v!=='string'||v.length>max)throw new Error('旧世界书文本字段无效：'+key);return v}
    const number=(key:string,fallback:number,min=0,max=Number.MAX_SAFE_INTEGER)=>{const v=value[key];if(v===undefined)return fallback;if(typeof v!=='number'||!Number.isFinite(v)||v<min||v>max)throw new Error('旧世界书数值字段无效：'+key);return v}
    const bool=(key:string,fallback=false)=>{const v=value[key];if(v===undefined)return fallback;if(typeof v!=='boolean')throw new Error('旧世界书开关无效：'+key);return v}
    const inherit=(key:string)=>{const v=value[key];if(v===undefined||v==='same_as_global')return null;if(typeof v!=='boolean')throw new Error('旧世界书继承开关无效：'+key);return v}
    const nullable=(key:string)=>value[key]==null?null:number(key,0)
    const enumValue=(key:string,choices:string[],fallback:string)=>{const v=text(key,fallback);if(!choices.includes(v))throw new Error('旧世界书枚举无效：'+key);return v}
    const keys=(key:string)=>{const v=value[key]??[];if(!Array.isArray(v)||v.length>2000||v.some(item=>typeof item!=='string'||item.length>500))throw new Error('旧世界书关键字无效：'+key);return v}
    ...
    if(value.automation_id!==undefined&&value.automation_id!==null&&typeof value.automation_id!=='string')throw new Error('旧世界书 automation_id 无效')
  ...
  function replace(input:unknown,before:HelperWorldbookEntry[]=[]):unknown[] {
  ...
  function patch(input:unknown,before:HelperWorldbookEntry[]):unknown[] {
    const values=list(input),existing=new Set(before.map(entry=>entry.uid)),byId=new Map<number,Record<string,unknown>>()
    for(const value of values){const id=uid(value.uid);if(byId.has(id))throw new Error('旧世界书更新 UID 重复');if(!existing.has(id))throw new Error('旧世界书更新 UID 不存在');byId.set(id,value)}
    return before.map(entry=>byId.has(entry.uid)?convert(byId.get(entry.uid)!,entry,true):clone(entry))
  }
  function ids(input:unknown):number[]{
    const values=json(input,8*1024*1024)
    if(!Array.isArray(values)||values.length>2000)throw new Error('旧世界书删除 UID 列表无效')
    return values.map(uid)
  }
  function filter(entries:HelperLorebookEntry[],input:unknown):HelperLorebookEntry[]{
    if(input===undefined||input==='none')return entries
    const wanted=list([input])[0]!
    const same=(left:unknown,right:unknown):boolean=>Array.isArray(left)?Array.isArray(right)&&left.length===right.length&&left.every((v,index)=>same(v,right[index])):left===right
    return entries.filter(entry=>Object.entries(wanted).every(([key,value])=>same(value,entry[key as keyof HelperLorebookEntry])))
  }
