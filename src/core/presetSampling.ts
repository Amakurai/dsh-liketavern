...
export function parsePresetSampling(value: unknown): PresetSamplingSettings {
  const result = safeParse(samplingSchema, value)
  if (!result.success) throw new Error(`预设采样参数无效：${result.error.message}`)
  ...
  if (sampling.stop?.some(stop => stop.length === 0)) throw new Error('预设采样 stop 不能包含空字符串')
  return sampling
}
...
function importedNumber(value: unknown): unknown {
  if (typeof value === 'string' && value.trim() !== '') return Number(value)
  return value
}

/** 顶层 stop 是兼容扩展；也接收随文件附带的 ST custom_stopping_strings JSON 数组字符串。 */
export function importStPresetSampling(raw: Record<string, unknown>, warnings: string[]): PresetSamplingSettings | undefined {
  ...
  for (const [source, target] of ST_NUMBER_FIELDS) {
    if (raw[source] !== undefined) candidate[target] = importedNumber(raw[source])
  }
  if (raw.stop !== undefined) {
    ...
    try { candidate.stop = encoded === '' ? [] : JSON.parse(encoded) }
    catch { throw new Error('预设采样 custom_stopping_strings 不是有效的 JSON 数组') }
  }
  if (Object.keys(candidate).length === 0) return undefined
  const sampling = parsePresetSampling(candidate)
  const retained = ST_NUMBER_FIELDS.filter(([source, target]) =>
    (target === 'topP' || target === 'presencePenalty' || target === 'frequencyPenalty') && raw[source] !== undefined)
    .map(([source]) => source)
  if (retained.length > 0) warnings.push(`已保留 ${retained.join('、')}；当前宿主不透传这些采样参数`)
...
export function exportStPresetSampling(value: PresetSamplingSettings | undefined): Record<string, unknown> {
  if (value === undefined) return {}
  const sampling = parsePresetSampling(value)
  const result: Record<string, unknown> = {}
  for (const [target, source] of ST_NUMBER_FIELDS) {
    if (sampling[source] !== undefined) result[target] = sampling[source]
  }
  if (sampling.stop !== undefined) result.stop = [...sampling.stop]
  return result
}

/** 每轮首次组装合并并复制一次；冻结计划之后沿用这份采样快照。 */
export function resolvePresetSampling(global: SamplingSettings, preset: PresetSamplingSettings | undefined): SamplingSettings {
