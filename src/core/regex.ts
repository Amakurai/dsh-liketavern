...
function skipCharClass(source: string, i: number): number {
  i++
  if (source[i] === '^') i++
  if (source[i] === ']') i++ // 首位置的 ] 是字面量
...
function stripEscapesAndClasses(text: string): string {
  let out = ''
  let i = 0
  while (i < text.length) {
    const ch = text[i]!
...
function findGroupClose(source: string, open: number): number {
  let depth = 0
  let i = open
  while (i < source.length) {
    const ch = source[i]!
...
function groupFollowedByQuantifier(source: string, close: number): boolean {
...
function bodyHasOverlappingBranches(body: string): boolean {
  ...
  for (let a = 0; a < branches.length; a++) {
    for (let b = a + 1; b < branches.length; b++) {
      const x = branches[a]!
      const y = branches[b]!
      if (x === '' || y === '') continue // 空分支（(a|)）至多制造一次短路，不构成指数回溯
  ...
  while (i < source.length) {
    ...
    if (ch === '(') {
      ...
      if (groupFollowedByQuantifier(source, close)) {
        ...
        if (bodyHasNestedQuantifier(body)) return `嵌套量词 ${source.slice(i, close + 2)}`
        if (bodyHasOverlappingBranches(body)) return `交叠分支 ${source.slice(i, close + 2)}`
      }
...
function assertSafePattern(source: string): void {
 ...
 */
function isWellFormedRule(rule: RegexRule): boolean {
  const r = rule as Partial<Record<keyof RegexRule, unknown>> | null | undefined
  return (
    typeof r === 'object' &&
    r !== null &&
...
function ruleIdOf(rule: RegexRule): string {
  const id = (rule as { id?: unknown } | null | undefined)?.id
...
export function applyRegexToMessages(
  ...
  for (const r of rules) {
    ...
    if (!r.enabled || !r.scopes.includes(filter.scope) || !r.timing.includes(filter.timing)) continue
