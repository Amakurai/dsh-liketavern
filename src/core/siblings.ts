...
export function normalizeSiblingForks(raw: unknown): SiblingFork[] {
  if (!Array.isArray(raw)) return []
  const out: SiblingFork[] = []
...
export function recordSiblingFork(forks: readonly SiblingFork[], entry: SiblingFork): SiblingFork[] {
  if (entry.parentSessionId === entry.childSessionId) return [...forks]
  if (forks.some((f) => f.childSessionId === entry.childSessionId)) return [...forks]
  return [...forks, entry]
}

/** 剪掉悬空记录（父或子会话已不存在）；changed = 是否有记录被剪掉。 */
export function pruneSiblingForks(
  forks: readonly SiblingFork[],
  exists: (sessionId: string) => boolean,
): { forks: SiblingFork[]; changed: boolean } {
 ...
 */
export function siblingSwipe(
  forks: readonly SiblingFork[],
  sessionId: string,
  turn: number,
  exists?: (sessionId: string) => boolean,
): SiblingSwipe | null {
  const edges = forks.filter(
    (f) =>
      f.turn === turn &&
  ...
  if (!adj.has(sessionId)) return null
  ...
  for (const e of edges) {
    if (!seen.has(e.childSessionId)) continue
    const prev = created.get(e.childSessionId)
    if (prev === undefined || e.createdAt < prev) created.set(e.childSessionId, e.createdAt)
  }
  component.sort((a, b) => {
    const ca = created.get(a)
    const cb = created.get(b)
    if (ca === undefined && cb === undefined) return a < b ? -1 : 1
