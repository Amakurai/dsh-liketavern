...
export function templateAvatarPng(input:Uint8Array):Uint8Array|null {
  if(input.length>32*1024*1024 || input.length<8 || signature.some((value,index)=>input[index]!==value)) return null
  ...
  while(offset+12<=input.length) {
    ...
    if(type==='IHDR') {
      if(header || length!==13 || !view.getUint32(offset+8) || !view.getUint32(offset+12)) return null
      header=true
    }
    if(type==='IDAT') image=true
    if(type==='IEND') { if(length || !image) return null; ended=true }
    if(retained.has(type)) {
      size+=length+12
      if(size>TEMPLATE_AVATAR_BYTES) return null
      chunks.push(input.subarray(offset,end))
    }
    offset=end
    if(ended) break
  }
  if(!header || !image || !ended) return null
...
export function isTemplateAvatarUrl(value:unknown):value is string {
