...
export function normalizeTemplateLore(entry: WorldInfoEntry): WorldInfoEntry {
  if (!entry.content.startsWith('@@') || entry.content.startsWith('@@@')) return entry
  ...
  if (modes.length > 1) throw new Error(`世界书「${comment}」包含冲突的模板位置装饰器`)
  if (modes[0]) {
    ...
    if(mode==='generate_before'||mode==='generate_after') {
      ...
      if(argument) {
        if(/^-?\d+$/.test(argument)) position=`[GENERATE:${argument}:${side}]`
        else if(/^REGEX:/i.test(argument)) position=`[GENERATE:${side}:REGEX:${argument.slice(6)}]`
        else throw new Error(`世界书「${comment}」的 @@${mode} 参数需要整数下标或 REGEX:模式`)
  ...
  if (enabled && !/^\[RENDER:(BEFORE|AFTER)\]/i.test(comment) && (decorators.has('iframe') || decorators.has('message_formatting'))) throw new Error(`世界书「${comment}」的 iframe/message_formatting 需要 RENDER 位置`)
  const condition = decorators.get('if')
  if (condition !== undefined && !condition) throw new Error(`世界书「${comment}」的 @@if 条件为空`)
  if (condition && /^\[InitialVariables\]/i.test(comment)) throw new Error('InitialVariables 暂不支持 @@if 条件')
  let content = lines.join('\n')
  // 每条目本来就有独立词法上下文；显式块同时保护多个 RENDER 条目合并后的声明。
  if (decorators.has('private')) content = `<% { %>${content}<% } %>`
...
export function templateLoreEntries(entries: WorldInfoEntry[]): WorldInfoEntry[] {
  return entries.map(entry => {
    if (!entry.enabled) return entry
    const label = entry.comment
    if (entry.templateOnlyPreload || /^\[InitialVariables\]/i.test(label) || /^\[RENDER:(BEFORE|AFTER)\]/i.test(label)) return { ...entry, enabled:false }
    const placement=parseTemplatePlacement(label)
    if (placement) return {...entry,constant:placement.kind==='insert'||entry.constant,content:`<% /* positioned template */ %>${entry.content}`}
