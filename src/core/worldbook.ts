...
function compileKey(
  ...
  if (trimmed.length > MAX_WI_KEY_CHARS) return null
  const literal = REGEX_KEY_RE.exec(trimmed)
  if (literal) {
    // 灾难性回溯构造（(a+)+$ 类）在主事件循环上会冻结整个 host：与非法正则同口径，永不命中
    if (findUnsafeRegexConstruct(literal[1]!) !== null) return null
    try {
...
function compileEntry(
  ...
  const rawKeys = Array.isArray(entry.keys) ? entry.keys.filter((k): k is string => typeof k === 'string') : []
  const rawSecondary = Array.isArray(entry.secondaryKeys)
    ? entry.secondaryKeys.filter((k): k is string => typeof k === 'string')
    : []
...
export function evaluateWorldInfo(input: WIEngineInput): WIEngineResult {
  ...
  const groupLosers = new Set<string>()
   ...
   */
  const adjudicateGroups = (fresh: Candidate[]): void => {
    const newcomers = new Map<string, Candidate[]>()
    for (const c of fresh) {
      const name = typeof c.entry.group === 'string' ? c.entry.group.trim() : ''
      if (!name || groupLosers.has(c.entry.key)) continue
    ...
    for (const [group, members] of newcomers) {
      ...
      for (const member of pool) {
        if (winnerKeys.has(member.entry.key) || groupLosers.has(member.entry.key)) continue
        groupLosers.add(member.entry.key)
        log.push({
          kind: 'group-skip',
          entryKey: member.entry.key,
          detail:
            sticky.length > 0 ? `组「${group}」由 sticky 延续占用` : `组「${group}」已选 ${winners[0]!.entry.key}`,
        })
      }
    }
  }
  ...
  for (const candidate of activated) {
    ...
    if (candidate.via === 'sticky' || !stickyKeysAtStart.has(candidate.entry.key)) delete timer.stickyLeft[candidate.entry.key]
