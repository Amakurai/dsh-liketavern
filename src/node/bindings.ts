...
function sameRegularFile(left: Stats, right: Stats): boolean {
...
async function readBoundedBinding(
  ...
  while (position <= MAX_SESSION_BINDING_BYTES) {
    ...
    if (bytesRead === 0) return { ok: true, text: Buffer.concat(chunks, position).toString('utf8') }
    position += bytesRead
    chunks.push(chunk.subarray(0, bytesRead))
    if (position > MAX_SESSION_BINDING_BYTES) return { ok: false }
  }
  return { ok: false }
}

export async function listBindingReferencesForCard(paths: TavernPaths, cardId: string): Promise<BindingReferenceScan> {
  assertValidCardId(cardId)
  // Windows 的角色目录同样大小写不敏感；旧绑定可以保留调用方传入的大小写，
  // 尚未迁移成 story 时只能靠此处的绑定引用阻止误删。
  const isTargetCard = (value: unknown): boolean => typeof value === 'string'
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') return { sessionIds: [], corruptFiles: [] }
    throw error
  }
  ...
  for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    let parsed: SessionBinding | null = null
    try { parsed = parseSessionBinding(raw) } catch {
      corruptFiles.push(file)
    ...
    if (!sameFile) corruptFiles.push(file)
    if (isTargetCard(parsed.cardId)) references.push(parsed.sessionId)
  }
