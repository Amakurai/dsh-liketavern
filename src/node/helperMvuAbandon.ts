/** 显式放弃自动 MVU 待处理任务：关闭当前剧情绑定，在最新所属 WAL 清队列，保留所有已保存变量和完成回执。 */
...
type Request = { sessionId: string; storyId: string }
export const HELPER_MVU_ABANDON_PATH = 'state/helper-mvu-abandon.json'
const digest = (saved: HelperState) => createHash('sha256').update(JSON.stringify(saved)).digest('hex')
type Recovery = { version: 1; sessionId: string; storyId: string; floor: string; resultDigest: string }
function parseRecovery(raw: string | null): Recovery | null {
  if (raw === null) return null
  if (raw.length > 2048) throw new Error('自动 MVU 放弃恢复记录超限')
  ...
  if (!helperRecord(value) || value.version !== 1 || typeof value.sessionId !== 'string' || !value.sessionId || value.sessionId.length > 256
    ...
    || Object.keys(value).some(key => !['version', 'sessionId', 'storyId', 'floor', 'resultDigest'].includes(key))) throw new Error('自动 MVU 放弃恢复记录损坏')
  return value as Recovery
}
/** 复用分支回滚的顺序：先按 turn，再按祖先到当前会话排序，同轮选择当前分支。 */
function orderOf(floor: string, binding: SessionBinding): { turn: number; owner: number } | null {
  ...
  if (match[1] === binding.sessionId) return { turn, owner: lineage.length }
  const owner = lineage.findIndex(item => item.sessionId === match[1] && item.throughTurn >= turn)
  return owner < 0 ? null : { turn, owner }
}

export async function abandonHelperMvu(state: TavernState, request: Request): Promise<{ disabled: true; abandoned: number }> {
  if (!request.sessionId || request.sessionId.length > 256 || !request.storyId || request.storyId.length > 256) throw new Error('自动 MVU 剧情身份无效')
  ...
  return withWorkspaceLock(workspace.fs.root, () => withWorkspaceLock(state.paths.sessions, async () => {
    ...
    if (!binding || binding.cardId !== first.cardId || binding.storyId !== request.storyId) throw new Error('自动 MVU 剧情绑定已改变')
    ...
    if (generation?.status === 'prepared' && !(open?.floor === generation.floor && open.cardId === generation.cardId && open.storyId === generation.storyId && generation.sessionId === request.sessionId)) throw new Error('模板楼层尚未收口，请先恢复宿主生成；不能通过放弃 MVU 绕过')
    ...
    if (floors.length > 4096) throw new Error('自动 MVU 恢复楼层数量超过预算')
    ...
    for (const floor of owned) await workspace.wal.validateFloor(floor.floor)
    const known = new Set(owned.map(item => item.floor))
    for (const job of mvu.pending) if (!known.has(job.floor)) throw new Error('自动 MVU 待处理任务的所属 WAL 缺失或已回滚')
    if (open && (open.cardId !== binding.cardId || open.storyId !== binding.storyId || open.floor !== owned.at(-1)?.floor)) throw new Error('自动 MVU 当前楼层与剧情恢复目标不一致')
    const receipt = mvu.completed.at(-1)
    if (receipt) {
      if (!known.has(receipt.floor)) throw new Error('自动 MVU 完成回执的所属 WAL 缺失或已回滚')
      const floor = await workspace.wal.validateFloor(receipt.floor)
      if (!floor.committed && open?.floor !== receipt.floor) await workspace.wal.commitFloor(receipt.floor)
    }
    const target = owned.at(-1)?.floor
    if (mvu.pending.length && !target) throw new Error('自动 MVU 没有可写入的所属 WAL 楼层')
    const recovering = recovery && known.has(recovery.floor) ? await workspace.wal.validateFloor(recovery.floor) : null
    if (recovery && !recovering) throw new Error('自动 MVU 放弃恢复记录的 WAL 缺失或已回滚')
    if (recovering && !recovering.committed && open?.floor !== recovering.floor) {
      if (recovery!.sessionId !== request.sessionId || recovery!.storyId !== request.storyId) throw new Error('自动 MVU 放弃恢复记录属于其它剧情')
      if (!mvu.pending.length || recovering.floor !== target) {
        if (digest(saved) !== recovery!.resultDigest) throw new Error('自动 MVU 放弃恢复记录与当前变量不一致，不能标记已完成')
    ...
    if (mvu.pending.length && targetInfo && !targetInfo.committed && !open && target !== receipt?.floor && !mvu.pending.some(job => job.floor === target)
      && !(recovery !== null && recovery.floor === target && recovery.sessionId === request.sessionId && recovery.storyId === request.storyId)) throw new Error('最新楼层存在未收口的宿主事务，不能通过放弃 MVU 提交')
    ...
    try {
      if (mvu.pending.length && target) {
        ...
        await workspace.fs.withFloor(target).writeText(HELPER_MVU_ABANDON_PATH, JSON.stringify({ version: 1, ...request, floor: target, resultDigest: digest(next) } satisfies Recovery))
        await workspace.wal.reopenFloor(target)
        await saveHelperState(workspace.fs.withFloor(target), next)
        if (!open) await workspace.wal.commitFloor(target)
      }
    } catch (cause) {
