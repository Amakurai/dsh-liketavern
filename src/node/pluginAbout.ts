...
export interface PluginAboutMetadata { plugin: unknown; host: unknown; sourceCheckout: boolean }
...
interface SemVer { value: string; core: string[]; prerelease: string[] }

function object(value: unknown): value is Record<string, unknown> { return value !== null && typeof value === 'object' && !Array.isArray(value) }
function semver(value: unknown, message: string): SemVer {
  if (typeof value !== 'string' || value.length > 256) throw new Error(message)
  const match = VERSION.exec(value)
  if (!match) throw new Error(message)
  const prerelease = match[4]?.split('.') ?? []
  if (prerelease.some(part => /^\d+$/.test(part) && part.length > 1 && part.startsWith('0'))) throw new Error(message)
  return { value, core: [match[1]!, match[2]!, match[3]!], prerelease }
}
function numeric(left: string, right: string): number { return left.length !== right.length ? Math.sign(left.length - right.length) : left === right ? 0 : left < right ? -1 : 1 }
/** 按 SemVer 优先级比较，不把大整数转为浮点数；build metadata 不参与排序。 */
function compare(left: SemVer, right: SemVer): number {
  for (let index = 0; index < 3; index++) { const order = numeric(left.core[index]!, right.core[index]!); if (order) return order }
  ...
  for (let index = 0; index < Math.max(left.prerelease.length, right.prerelease.length); index++) {
    ...
    if (aNumeric && bNumeric) return numeric(a, b)
    if (aNumeric !== bNumeric) return aNumeric ? -1 : 1
    return a < b ? -1 : 1
  }
  return 0
}
function packageVersions(value: unknown, error: string): { version: SemVer; host: SemVer } {
  if (!object(value) || value.name !== 'dsh-liketavern' || !object(value.peerDependencies)) throw new Error(error)
...
async function localJson(path: string): Promise<unknown> {
  ...
  try {
    ...
    if (!info.isFile() || info.size > MAX_RESPONSE_BYTES) throw new Error(METADATA_ERROR)
    const chunks: Buffer[] = []
    let length = 0
    while (length <= MAX_RESPONSE_BYTES) {
      ...
      if (!bytesRead) break
      length += bytesRead
      if (length > MAX_RESPONSE_BYTES) throw new Error(METADATA_ERROR)
...
export async function readHostEntryMetadata(entry: string | undefined): Promise<unknown | null> {
  ...
  try {
    ...
    for (let depth = 0; depth < 8; depth++) {
      ...
      try { manifest = await localJson(join(directory, 'package.json')) }
      catch (error) {
      ...
      if (!object(manifest) || manifest.name !== '@deepseek-ai/dsh' || !object(manifest.bin)
        || typeof manifest.bin.dsh !== 'string' || !manifest.bin.dsh || isAbsolute(manifest.bin.dsh)) return null
      ...
      if (normalize(actualEntry) !== normalize(actualBin)) return null
...
function cancel(body: ReadableStream<Uint8Array> | null): void { if (body) void body.cancel().catch(() => {}) }
async function responseJson(response: Response, signal: AbortSignal): Promise<unknown> {
  if (response.status !== 200 || response.redirected) {
    cancel(response.body)
    if (response.status === 404) throw new Error('GitHub 尚无可用正式发布，或发布文件不存在')
    if (response.status === 429) throw new Error('GitHub 更新检查请求过于频繁，请稍后重试')
    throw new Error(NETWORK_ERROR)
  }
  const length = response.headers.get('content-length')
  if (length !== null && (!/^\d+$/.test(length) || Number(length) > MAX_RESPONSE_BYTES)) {
    cancel(response.body); throw new Error('GitHub 更新信息超过 256 KiB 上限')
  }
  if (!response.body) throw new Error(INVALID_RELEASE_ERROR)
  ...
  try {
    for (;;) {
      if (signal.aborted) throw new Error(TIMEOUT_ERROR)
      const result = await reader.read()
      if (result.done) break
      bytes += result.value.byteLength
      if (bytes > MAX_RESPONSE_BYTES) throw new Error('GitHub 更新信息超过 256 KiB 上限')
      chunks.push(result.value)
    }
    if (signal.aborted) throw new Error(TIMEOUT_ERROR)
    try { return JSON.parse(Buffer.concat(chunks).toString('utf8')) as unknown } catch { throw new Error(INVALID_RELEASE_ERROR) }
...
export function createPluginAboutReader(options: PluginAboutReaderOptions = {}): PluginAboutReader {
  ...
  const getPluginAbout = async (): Promise<PluginAbout> => {
    try {
  ...
  const check = async (): Promise<PluginUpdate> => {
    ...
    if (current.hostVersion === 'unknown') throw new Error('无法确认当前运行宿主版本，请通过 dsh 正常启动后重试')
    ...
    const request = async (url: string): Promise<unknown> => {
      try {
        ...
        if (controller.signal.aborted) { cancel(response.body); throw new Error(TIMEOUT_ERROR) }
    ...
    const operation = async (): Promise<PluginUpdate> => {
      const release = await request(RELEASE_API)
      if (!object(release) || release.draft !== false || release.prerelease !== false || typeof release.tag_name !== 'string') throw new Error(INVALID_RELEASE_ERROR)
      const tag = release.tag_name, latest = semver(tag.startsWith('v') ? tag.slice(1) : tag, INVALID_RELEASE_ERROR)
      if (latest.prerelease.length) throw new Error(INVALID_RELEASE_ERROR)
      const target = packageVersions(await request(`${RAW_PACKAGE_ROOT}${encodeURIComponent(tag)}/package.json`), INVALID_RELEASE_ERROR)
      if (target.version.value !== latest.value) throw new Error(INVALID_RELEASE_ERROR)
...
export const getPluginAbout = (): Promise<PluginAbout> => installedReader.getPluginAbout()
export const checkPluginUpdate = (): Promise<PluginUpdate> => installedReader.checkPluginUpdate()
