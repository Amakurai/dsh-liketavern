...
import {
  archiveCharacter as archiveCharacterOnDisk,
  ...
  restoreCharacter as restoreCharacterOnDisk,
  rebuildIndex,
  type ArchivedCharacterSummary,
...
import { deleteBinding, listBindingReferencesForCard, loadBinding, saveBinding, type SessionBinding } from './bindings.js'
...
import { discardStory, legacyStoryId, newStoryId, readStory, snapshotStory, storyRoot, listStories, type StorySummary } from '../state/story.js'
...
export class CharacterInUseError extends RemoteError<'tavern/character-in-use'> {
  constructor(readonly references: CharacterReferences) {
    ...
    super(
      ...
      `角色仍有${reasons}，请先清理后再永久删除`,
      { sessionCount: sessions, storyCount: stories, corruptBindingCount: corrupt },
    )
  }
}

/** 永久删除只接受当前仍持有有效收纳标记的角色，阻断旧确认框与直接 RPC。 */
export class CharacterNotArchivedError extends RemoteError<'tavern/character-not-archived'> {
...
export class CharacterArchivedError extends RemoteError<'tavern/character-archived'> {
  constructor() {
...
export class TavernState {
   ...
   */
  readonly wiCache = new Map<
    string,
    { turn: number; wi: WIEngineResult; memories: string[]; deltas: WorldDelta[]; lastCharMessage: string; journalText: string }
  >()
  ...
  async listArchivedCharacters(): Promise<ArchivedCharacterSummary[]> {
    return listArchivedCharacters(this.paths.characters)
  }

  async archiveCharacter(cardId: string): Promise<void> {
  ...
  async restoreCharacter(cardId: string): Promise<void> {
  ...
  async deleteCharacter(cardId: string): Promise<{ salvagedLorebook: string | null }> {
    ...
    return withWorkspaceLock(this.paths.sessions, () => withWorkspaceLock(cardRoot, async () => {
      let archived: Awaited<ReturnType<typeof readCharacterArchiveMetadata>> = null
      let archiveFailure: unknown
      try { archived = await readCharacterArchiveMetadata(this.paths.characters, cardId) } catch (error) { archiveFailure = error }
      if (!archived) throw new CharacterNotArchivedError(archiveFailure)
      ...
      const storageKey = (key: string) => process.platform === 'win32' ? key.toLowerCase() : key
      const deletedKey = storageKey(cardId)
      for (const key of [...this.workspaces.keys()]) {
        const normalized = storageKey(key)
        if (normalized === deletedKey || normalized.startsWith(`${deletedKey}/`)) this.workspaces.delete(key)
      }
      for (const key of this.cardCache.keys()) {
        if (storageKey(key) === deletedKey) this.cardCache.delete(key)
      }
      for (const key of this.cardRegexCache.keys()) {
        if (storageKey(key) === deletedKey) this.cardRegexCache.delete(key)
      }
  ...
  ): Promise<{ cardId: string; name: string; revision: string }> {
    assertValidCardId(cardId)
    return withWorkspaceLock(join(this.paths.characters, cardId), async () => {
      const charWs = await this.loadCharacter(cardId)
      if (!charWs) throw new Error(`角色 ${cardId} 不存在`)
  ...
  async getSessionHelperScripts(sessionId:string,storyId:string):Promise<HelperScriptContext> {
    return withWorkspaceLock(this.paths.sessions,async()=>{
      ...
      const types:HelperScriptType[]=['global',...(hasPreset?['preset' as const]:[]),'character']
  ...
  async saveBinding(binding: SessionBinding): Promise<void> {
    ...
    return withWorkspaceLock(this.paths.sessions, () => withWorkspaceLock(cardRoot, async () => {
      ...
      if (!ws) throw new Error(`角色 ${binding.cardId} 不存在`)
   ...
   */
  pinStanding(sessionId: string, generationType: string, fingerprint: string, computed: string): { text: string; reused: boolean } {
