...
export function createTemplateGeneration(
  ...
  return {
    ...
    processTemplateSequence:(sequence:TemplateSequenceMessage[])=> {
      ...
      const log:AssembleLogEntry[]=[],turnContext:string[]=[],tailInsertions:string[]=[],matched=new Set<Hook>()
      ...
      for(let index=0;index<sequence.length;index++) {
        ...
        if(item.history) item.historyInsertions={before:checked(leading+localBefore),after:localAfter}
      ...
      if(last) {
        ...
        if(last.history && last.historyInsertions) last.historyInsertions.after=checked(last.historyInsertions.after+after)
