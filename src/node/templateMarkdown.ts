 ...
 */
import MarkdownIt, { type Delimiter, type Env, type MarkdownIt as MarkdownParser, type StateBlock, type StateInline, type Token } from 'markdown-it'
...
interface ImageDimensions { width: string; height: string }
interface SizedDestination extends ImageDimensions { href: string; title?: string; end: number }

function skipSpace(source: string, start: number, max: number): number {
  let position = start
  while (position < max && (source.charCodeAt(position) === 32
    || source.charCodeAt(position) === 9 || source.charCodeAt(position) === 10)) position++
  return position
}

function sizedDestination(source: string, start: number, max: number, markdown: MarkdownParser): SizedDestination | null {
  ...
  if (!markdown.validateLink(href)) return null
  let position = skipSpace(source, destination.pos, max)
  if (position === destination.pos) return null
  ...
  if (titleStart > position) {
    ...
    if (parsed.ok) { title = parsed.str; position = skipSpace(source, parsed.pos, max) }
    else position = titleStart
  }
...
function sizedImage(state: StateInline, silent: boolean): boolean {
  const start = state.pos
  if (state.src.charCodeAt(start) !== 33 || state.src.charCodeAt(start + 1) !== 91) return false
  ...
  if (state.src.charCodeAt(position) !== 40) return false
  ...
  if (position >= state.posMax || state.src.charCodeAt(position) !== 41) return false
  if (!silent) {
    ...
    token.attrSet('alt', '')
    if (destination.title !== undefined) token.attrSet('title', destination.title)
...
function sizedReference(state: StateBlock, line: number, endLine: number, silent: boolean,
  dimensions: WeakMap<Env, Map<string, ImageDimensions>>): boolean {
  ...
  while (position < max && state.src.charCodeAt(position) !== 93) {
    if (state.src.charCodeAt(position) === 91) return false
    if (state.src.charCodeAt(position) === 92) position++
    position++
  }
  if (position >= max || state.src.charCodeAt(position + 1) !== 58) return false
  ...
  if (!destination || destination.end !== max) return false
  let nextLine = line + 1
  ...
  if (!silent) {
    const references = state.env.references ??= Object.create(null) as NonNullable<Env['references']>
    if (!Object.hasOwn(references, label)) {
      ...
      if (!entries) { entries = new Map(); dimensions.set(state.env, entries) }
...
function underline(state: StateInline, silent: boolean): boolean {
  if (state.src.charCodeAt(state.pos) !== 95 || silent) return false
...
function finishUnderline(state: StateInline): void {
  const apply = (delimiters: Delimiter[]): void => {
    for (const opening of delimiters) {
      if (opening.marker !== UNDERLINE_MARKER || opening.end < 0) continue
      ...
      for (const [index, nesting] of [[opening.token, 1], [closing.token, -1]] as const) {
        const token = state.tokens[index]!
        token.markup = token.content
        token.type = nesting === 1 ? 'tavern_u_open' : 'tavern_u_close'
        token.tag = 'u'; token.nesting = nesting; token.content = token.markup.slice(width)
      }
    }
  }
  apply(state.delimiters)
  for (const metadata of state.tokens_meta) if (metadata?.delimiters) apply(metadata.delimiters)
}

/** raw inline code 作为单个 HTML token 保留，避免其中的 emoji、图片和 Markdown 再解析。 */
function rawCode(state: StateInline, silent: boolean, missingClosers: WeakMap<StateInline, { start: number; max: number }>): boolean {
  const start = state.pos
  if (state.src.slice(start, start + 5).toLowerCase() !== '<code'
    || !/[\s>]/.test(state.src[start + 5] ?? '')) return false
  const missing = missingClosers.get(state)
  if (missing?.max === state.posMax && start >= missing.start) return false
  ...
  if (!silent) state.push('html_inline', '', 0).content = state.src.slice(start, closing.lastIndex)
  state.pos = closing.lastIndex
  return true
}

/** 不暴露 parser 或插件对象，避免模板修改其它格式化调用的引擎配置。 */
export function createMessageFormatter(): (text: string) => string {
  ...
  const referenceDimensions = new WeakMap<Env, Map<string, ImageDimensions>>()
  const missingCodeClosers = new WeakMap<StateInline, { start: number; max: number }>()
  ...
  markdown.block.tokenize = (state, startLine, endLine): void => {
    ...
    try { tokenizeBlocks(state, startLine, endLine) } finally {
  ...
  markdown.block.ruler.before('reference', 'tavern_image_reference_dimensions',
    (state, line, endLine, silent) => sizedReference(state, line, endLine, silent, referenceDimensions))
  ...
  markdown.renderer.rules.image = (tokens, index, options, env, renderer) => {
    ...
    if (dimensions) { token.attrSet('width', dimensions.width); token.attrSet('height', dimensions.height) }
