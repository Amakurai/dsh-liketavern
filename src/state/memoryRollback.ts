...
export async function expandAffectedMemories(root: string, paths: readonly string[]): Promise<void> {
  ...
  const expand = async (id: string): Promise<void> => {
    ...
    if (visiting.has(id)) throw new Error('记忆归并来源存在循环，已停止回滚')
    visiting.add(id)
    ...
    for (const source of sources) await fs.delete(`memory/archive/${source}.md`)
    await fs.delete(`memory/${id}.md`)
    await fs.delete(`memory/archive/${id}.md`)
    expanded.add(id)
    for (const source of sources) if (invalid.has(source)) await expand(source)
    visiting.delete(id)
  }
  // 从当前活跃摘要向下展开；归档摘要由父级恢复，未受影响的归并保持原样。
  for (const id of invalid) if (await fs.exists(`memory/${id}.md`)) await expand(id)
}
