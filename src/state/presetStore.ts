...
function parsePresetFormatting(value: unknown): NonNullable<PromptPreset['formatting']> {
  ...
  if (!result.success) throw new Error(`预设格式字段无效：${result.error.message}`)
  return result.data
}
...
export function parseStoredPreset(value: unknown): PromptPreset {
  ...
  for (const entry of preset.entries) validateInChatDepth(entry)
...
function validateInChatDepth(entry: Pick<PresetEntry, 'identifier' | 'position' | 'depth'>): void {
