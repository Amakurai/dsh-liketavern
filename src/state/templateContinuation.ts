...
import { templatePreloadAssets,type TemplateContinuation } from '../core/templateContinuation.js'
import type { TemplateContext } from '../core/template.js'
import { validateTemplateJson } from '../core/template.js'
import { templateReplayGenerationContext, type TemplateReplay } from '../core/templateReplay.js'
...
export function templatePreloadRevision(context:TemplateContext):string {
...
export function templateStickyHasClosures(state:TemplateStickyState):boolean {
...
function validateContinuationReplayRevision(revision:string,replay:TemplateReplay):void {
  if(revision!==templatePreloadRevision(templateReplayGenerationContext(replay))) throw new Error('模板跨轮资产指纹与执行日志不一致')
}

export function parseTemplateContinuation(value:unknown):TemplateContinuation {
  validateTemplateJson(value)
  if(!value || typeof value!=='object' || Array.isArray(value) || value.version!==1
    || typeof value.preloadRevision!=='string' || !/^[a-f0-9]{64}$/.test(value.preloadRevision)
    || Object.keys(value).some(key=>!['version','preloadRevision','state','replay'].includes(key))) throw new Error('模板跨轮状态损坏')
  ...
  if(replay && !templateStickyHasClosures(state)) throw new Error('模板跨轮状态包含无用的闭包日志')
  if(replay) validateContinuationReplayRevision(value.preloadRevision,replay)
  return {version:1,preloadRevision:value.preloadRevision,state,...(replay ? {replay} : {})}
}

/** 仅在当前剧情内解析引用；不会因为回执缺失而重新执行某个旧生成阶段。 */
export function resolveTemplateContinuation(stored:TemplateState):TemplateContinuation|undefined {
  const carry=stored.continuation
  if(!carry) return undefined
  if(!templateStickyHasClosures(carry.state)) return carry
  const replay=carry.replay ?? (stored.generation?.status==='prepared' ? stored.generation.replay : undefined)
  if(!replay) throw new Error('跨轮闭包缺少可恢复的执行日志')
  validateContinuationReplayRevision(carry.preloadRevision,replay)
  if(!isDeepStrictEqual(replay.variables,stored.variables)) throw new Error('模板跨轮执行日志与已提交变量不一致')
  ...
  if(context.historyIdentities) {
    ...
    if(fingerprint!==replay.messageVariablesHash) throw new Error('模板跨轮历史消息变量与已提交消息快照不一致')
  }
  return carry.replay ? carry : {...carry,replay}
}

/** 关闭 prepared 回执前保留仍活跃的共享词法环境；不存在回调时释放日志，计数与过期状态继续保留。 */
export function closeTemplateGenerationState(stored:TemplateState,status:'completed'|'terminated'):void {
  if(!stored.generation) return
  const carry=resolveTemplateContinuation(stored)
  if(carry) stored.continuation=carry
