...
import { TEMPLATE_REPLAY_LIMIT, templateReplayGenerationContext, type TemplateReplay } from '../core/templateReplay.js'
...
const context: z.ZodType<TemplateContext> = z.object({variables:scope,char:text,user:text,card:z.record(text,z.unknown()),entries:z.array(entry),
...
export type PreparedTemplateGeneration = Extract<TemplateGeneration,{status:'prepared'}>

export function parseTemplateReplay(value:unknown):TemplateReplay {
  validateTemplateJson(value)
  if(!value || typeof value!=='object' || Array.isArray(value) || value.version!==2) throw new Error('模板重放版本不兼容，需要完成或回滚旧版本楼层')
  if(JSON.stringify(value).length>TEMPLATE_REPLAY_LIMIT) throw new Error('模板重放快照超过 4 MiB 上限')
  return replay.parse(value)
}

export function parseTemplateGeneration(value:unknown):TemplateGeneration {
  validateTemplateJson(value)
  if(value && typeof value==='object' && !Array.isArray(value) && value.status==='prepared'
    && value.replay && typeof value.replay==='object' && !Array.isArray(value.replay) && value.replay.version!==2) throw new Error('模板重放版本不兼容，需要完成或回滚旧版本楼层')
  const parsed = generation.parse(value)
  if (parsed.floor!==`${parsed.sessionId}#t${parsed.turn}`) throw new Error('模板恢复楼层与会话轮次不一致')
  if (parsed.status==='prepared') {
    if (JSON.stringify(parsed.replay).length>TEMPLATE_REPLAY_LIMIT) throw new Error('模板重放快照超过 4 MiB 上限')
    const current=templateReplayGenerationContext(parsed.replay)
    if (current.sessionId!==parsed.sessionId || current.cardId!==parsed.cardId) throw new Error('模板重放来源与剧情绑定不一致')
  }
  return parsed
}

/** 收口保留少量归属回执，删除大段闭包重放数据；中途重启也能幂等完成楼层提交。 */
export function closeTemplateGeneration(value:TemplateGeneration,status:'completed'|'terminated'):TemplateGeneration {
...
export function templateGenerationContext(value:PreparedTemplateGeneration):TemplateContext {
