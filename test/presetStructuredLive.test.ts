...
type Wire = { messages: WireMessage[]; temperature?: number; max_tokens?: number; stop?: string[]; tools?: unknown[] }
...
const slot = (wire: Wire, text: string) => wire.messages.findIndex(message => wireText(message).split('\n').at(-1) === text)
...
function completion(tool: boolean, index: number): Response {
  const frames = tool ? [
    { choices: [{ index: 0, delta: { tool_calls: [{ index: 0, id: 'structured-tool', type: 'function',
      function: { name: 'factory_edit_preset', arguments: '{}' } }] }, finish_reason: null }] },
    { choices: [{ index: 0, delta: {}, finish_reason: 'tool_calls' }], usage: { prompt_tokens: 80, completion_tokens: 5, total_tokens: 85 } },
  ] : [
    { choices: [{ index: 0, delta: { content: `FACTORY-ANSWER-${index}` }, finish_reason: null }] },
    { choices: [{ index: 0, delta: {}, finish_reason: 'stop' }], usage: { prompt_tokens: 80, completion_tokens: 5, total_tokens: 85 } },
  ]
...
beforeEach(async () => {
  ...
  vi.stubGlobal('fetch', vi.fn<typeof fetch>(async (input, init) => {
    ...
    if (url.endsWith('/files') && init?.method === 'POST') {
      ...
      return Response.json({ id: 'factory-file', object: 'file', bytes: file.size, filename: file.name, purpose: 'user_data',
        created_at: Math.floor(Date.now() / 1000), expires_at: Math.floor(Date.now() / 1000) + 604800 })
    }
    if (url !== 'https://structured.invalid/v1/chat/completions') throw new Error(`Unexpected factory URL: ${url}`)
  ...
  applyDeepSeek(ctx, { baseURL: 'https://structured.invalid/v1', apiKeyEnv: 'STRUCTURED_KEY', maxTokens: 1800,
  ...
  await vi.waitFor(() => expect(settings.get('llm-deepseek')).toBeDefined())
...
it('自动选择适配器，真实 wire 保留 system/user/assistant 深度、SDK 前缀和用户图片', async () => {
  ...
  expect(wires[0]!.messages[0]!.role).toBe('system')
  expect(wireText(wires[0]!.messages[0]!)).toContain('FACTORY-SDK-PREFIX')
  expect(wireText(wires[0]!.messages[0]!)).toContain('OTHER-HOST-SECTION')
  expect(wireText(wires[0]!.messages[0]!)).not.toContain('PRESET-MAIN')
  for (const [text, role] of [['PRESET-MAIN', 'system'], ['BEFORE-USER', 'user'], ['DEPTH-TWO', 'system'],
    ['DEPTH-ONE', 'user'], ['DEPTH-ZERO', 'assistant'], ['AFTER-HISTORY', 'system']]) {
    expect(wires[0]!.messages.filter(message => wireText(message).split('\n').at(-1) === text)).toEqual([expect.objectContaining({ role })])
  }
  const systems = wires[0]!.messages.filter(message => message.role === 'system')
  for (const system of systems) {
    expect(wireText(system)).toContain('FACTORY-SDK-PREFIX')
    expect(wireText(system)).toContain('OTHER-HOST-SECTION')
  }
  // 宿主声明最新 system 完整生效：末尾必须同时保留工具纪律与此前的系统预设。
  for (const text of ['PRESET-MAIN', 'DEPTH-TWO', 'AFTER-HISTORY']) expect(wireText(systems.at(-1)!)).toContain(text)
  const inputAt = wires[0]!.messages.findIndex(message => wireText(message).includes('INPUT-ONE'))
  expect(slot(wires[0]!, 'DEPTH-TWO')).toBeLessThan(inputAt)
  expect(slot(wires[0]!, 'DEPTH-ONE')).toBeLessThan(inputAt)
  expect(slot(wires[0]!, 'DEPTH-ZERO')).toBeGreaterThan(inputAt)
  expect(slot(wires[0]!, 'AFTER-HISTORY')).toBeGreaterThan(slot(wires[0]!, 'DEPTH-ZERO'))
  expect(wires[0]!.messages[inputAt]!.content).toEqual(expect.arrayContaining([expect.objectContaining({ type: 'file', file_id: 'factory-file' })]))
...
it('模型只支持首条 system 时真实传输合并全部系统规则，其它角色仍按布局送出', async () => {
  ...
  expect(wires[0]!.messages[0]).toBe(systems[0])
  for (const text of ['FACTORY-SDK-PREFIX', 'OTHER-HOST-SECTION', 'PRESET-MAIN', 'DEPTH-TWO', 'AFTER-HISTORY']) {
    expect(wireText(systems[0]!)).toContain(text)
  }
...
it('真实 wire 合并相邻系统规则，跨 user 与 assistant 保留位置，最新 system 仍是完整提示', async () => {
  ...
  const wire = wires[0]!, systems = wire.messages.filter(message => message.role === 'system')
  const ending = (text: string) => systems.find(message => wireText(message).endsWith(text))!
  expect(wireText(ending('COALESCE-B'))).toContain('COALESCE-A\n\nCOALESCE-B')
  expect(systems.some(message => wireText(message).endsWith('COALESCE-A'))).toBe(false)
  expect(wireText(ending('COALESCE-E'))).toContain('COALESCE-D\n\nCOALESCE-E')
  expect(systems.some(message => wireText(message).endsWith('COALESCE-D'))).toBe(false)
  expect(slot(wire, 'COALESCE-B')).toBeLessThan(slot(wire, 'USER-BOUNDARY'))
  expect(slot(wire, 'COALESCE-C')).toBeGreaterThan(slot(wire, 'USER-BOUNDARY'))
  expect(slot(wire, 'COALESCE-C')).toBeLessThan(slot(wire, 'COALESCE-INPUT'))
  expect(slot(wire, 'COALESCE-E')).toBeGreaterThan(slot(wire, 'COALESCE-INPUT'))
  expect(slot(wire, 'COALESCE-E')).toBeLessThan(slot(wire, 'ASSISTANT-BOUNDARY'))
  expect(slot(wire, 'COALESCE-F')).toBeGreaterThan(slot(wire, 'ASSISTANT-BOUNDARY'))
  const latest = wireText(systems.at(-1)!)
  for (const text of ['FACTORY-SDK-PREFIX', 'OTHER-HOST-SECTION', ...'ABCDEF'.split('').map(letter => `COALESCE-${letter}`)]) expect(latest).toContain(text)
  expect(latest.match(/FACTORY-SDK-PREFIX/g)).toHaveLength(1)
...
it('两步骤保持同轮布局和工具配对，下一轮才重读资产且每轮载体只接收一次', async () => {
  ...
  const callAt = wires[1]!.messages.findIndex(message => message.tool_calls?.some(call => call.id === 'structured-tool'))
  expect(wires[1]!.messages[callAt + 1]).toMatchObject({ role: 'tool', tool_call_id: 'structured-tool', content: 'FACTORY-TOOL-RESULT' })
  expect(slot(wires[1]!, 'AFTER-HISTORY')).toBeLessThan(callAt)
  const toolMessages = requests[1]!.messages.filter(message => message.content.some(block => block.type === 'tool-call' || block.type === 'tool-result'))
  ...
  expect(slot(wires[2]!, 'UPDATED-PRESET-MAIN')).toBeGreaterThan(0)
  expect(slot(wires[2]!, 'UPDATED-AFTER-HISTORY')).toBeGreaterThan(slot(wires[2]!, 'INPUT-TWO'))
  expect(slot(wires[2]!, 'PRESET-MAIN')).toBe(-1)
  expect(slot(wires[2]!, 'DEPTH-TWO')).toBeLessThan(slot(wires[2]!, 'FACTORY-ANSWER-2'))
...
it('实际请求日志经过 JSON 文件与真实 Session 恢复后仍能重建同一布局，解绑回原路由', async () => {
  ...
  const replay = projectPresetRequest({ ...requests[0]!, messages: restored.deriveMessages() }, restored, { model: { systemPromptUpdate: 'in-history' } })
...
it('恢复时旧 header 属于其它供应商，新 AgentOptions 选择官方路由仍携带本轮布局', async () => {
  ...
  expect(requests[0]!.provider).toBe(PRESET_ADAPTER_PROVIDER)
  expect(planMessages(requests[0]!.messages)).toHaveLength(1)
  expect(slot(wires[0]!, 'PRESET-MAIN')).toBeGreaterThan(0)
...
it('request middleware 等 next 后最终切入 Tavern，pre-step 不依赖提前猜测路由', async () => {
  ...
  expect(planMessages(requests[0]!.messages)).toHaveLength(1)
  expect(slot(wires[0]!, 'PRESET-MAIN')).toBeGreaterThan(0)
  expect(slot(wires[0]!, 'AFTER-HISTORY')).toBeGreaterThan(slot(wires[0]!, 'MIDDLEWARE-TAVERN-INPUT'))
  expect(otherRequests).toHaveLength(0)
})
